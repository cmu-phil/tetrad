...
/**
 *
 * Nov 17, 2025 11:15:57 PM
 *
 * @author Kevin V. Bui (kvb2univpitt@gmail.com)
 */
public class FullCovarianceDataFileReaderTest {

    private final Delimiter delimiter = Delimiter.SPACE;

    private final Path[] dataFiles = {
        new File(getClass().getResource("/data/covariance/spartina_full.txt").getFile()).toPath(),
        new File(getClass().getResource("/data/covariance/quotes_spartina_full.txt").getFile()).toPath()
    };

    public FullCovarianceDataFileReaderTest() {
    }

    /**
     * Test of readInData method, of class FullCovarianceDataFileReader.
     *
     * @throws IOException
     */
    @Test
    public void testReadInData() throws IOException {
        for (Path dataFile : this.dataFiles) {
            CovarianceDataReader dataFileReader = new FullCovarianceDataFileReader(dataFile, this.delimiter);
