 ...
 */
public class TabularContinuousData extends AbstractDatasetValidation implements DataValidation {

    private static final Logger LOGGER = LoggerFactory.getLogger(TabularContinuousData.class);

    public TabularContinuousData(Set<String> excludedVariables, Path dataFile, char delimiter) {
                        ...
                        if (variable != null) {
                            ...
                            if (value.length() > 0) {
                                try {
                                    ...
                                    String errMsg = String.format("Unable to parse data '%s'for variable '%s' at line %d column %d.", value, variable, row + 2, colCount + 1);
