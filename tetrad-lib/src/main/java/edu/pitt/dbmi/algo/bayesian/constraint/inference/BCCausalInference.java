...
public class BCCausalInference {
    ...
    public BCCausalInference(int[] nodeDimension, int[][] cases) {
        ...
        this.cases = cases;
        numberOfNodes = nodeDimension.length - 2;
        numberOfCases = cases.length - 1;
        maximumNodes = numberOfNodes;
        maximumCases = cases.length - 1;
        maximumValues = Arrays.stream(nodeDimension).max().getAsInt();
        maximumParents = maximumNodes - 2;
        maximumCells = maximumParents * maximumValues * maximumValues * maximumCases;
        logFactorial = this.computeLogFactorial(maximumCases, maximumValues);
        scoreFn = 1;  // right now we just fixed it to 1
    }
    ...
    public double probConstraint(OP constraint, int x, int y, int[] z) {
        ...
        double lnMarginalLikelihood_X = this.scoreNode(x, 1, countsTracker);  // the 1 indicates the scoring of X
        parents[y][0] = n;
        for (int i = 1; i <= n; i++) {
            parents[y][i] = z[i];
        }
        double lnMarginalLikelihood_Y = this.scoreNode(y, 2, countsTracker);  // the 2 indicates the scoring of Y
        ...
        probability = this.priorIndependent(x, y, z); // p should be in (0, 1), and thus, not 0 or 1.
        ...
        double lnMarginalLikelihood_XY = this.scoreNode(xy, 3, countsTracker);  // the 3 indicates the scoring of XY, which assumes X and Y are dependent given Z;
    ...
    private double scoreNode(int node, int whichList, CountsTracker countsTracker) {
        ...
        while (instancePtr < countsTracker.countsPtr) {
            double score;
            if (scoreFn == 1) {
                score = this.scoringFn1(node, instancePtr, q, PESS_VALUE, countsTracker);
            } else {
                score = this.scoringFn2(node, instancePtr, countsTracker);
            }
    ...
    private double scoringFn1(int node, int instancePtr, double q, double pess, CountsTracker countsTracker) {
        ...
        for (int k = 0; k <= (r - 1); k++) {
            ...
            Nij += Nijk;
            scoreOfSum += this.gammln(Nijk + pessDivQR) - lngammPessDivQR;
        }

        return this.gammln(pessDivQ) - this.gammln(Nij + pessDivQ) + scoreOfSum;
    }
    ...
    private double gammln(double xx) {
        if (xx == 1) {
            ...
            if (xx > 1) {
                ...
                double z = 1 - xx;
                return Math.log(Math.PI * z) - this.gammlnCore(1 + z) - Math.log(Math.sin(Math.PI * z));
            }
        }
    }
