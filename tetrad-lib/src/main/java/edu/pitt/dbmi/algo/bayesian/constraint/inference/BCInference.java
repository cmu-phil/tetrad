...
//        this.maxCases = max(numberOfCases, 1);
        ...
        maxValues = this.findMaxValue(nodeDimension);
        maxLogFact = (2 * maxCases) + maxValues;

        scoreFn = 1;

        logfact = new double[maxLogFact + 1];
...
//            maxCells = maxParents * maxValues * maxCases ;
        ...
        scores = new double[maxCases + 1][4];
    }
...
//            maxCells = maxParents * maxValues * maxCases ;
        ...
        double lnMarginalLikelihood_X = this.scoreNode(x, 1);  // the 1 indicates the scoring of X
        parents[y][0] = n;
        for (int i = 1; i <= n; i++) {
            parents[y][i] = z[i];
        }
        double lnMarginalLikelihood_Y = this.scoreNode(y, 2);  // the 2 indicates the scoring of Y
        ...
        p = this.priorIndependent(x, y, z); // p should be in (0, 1), and thus, not 0 or 1.
...
//            cases[casei][xy] = (xValue - 1) * nodeDimension[x] + yValue;  // a value in the Cartesian product of X and Y
        ...
        double lnMarginalLikelihood_XY = this.scoreNode(xy, 3);  // the 3 indicates the scoring of XY, which assumes X and Y are dependent given Z;
        //Note: lnMarginalLikelihood_XY is not used, but the above call to ScoreNode creates scores^[*, 3], which is used below
        numberOfNodes--;
        double lnTermPrior_X_Y = log(p) / numberOfScores;  // this is equal to ln(p^(1/numberOfScores))
        ...
        for (int i = 1; i <= numberOfScores; i++) {
            scoreAll += this.lnXpluslnY(lnTermPrior_X_Y + (scores[i][1] + scores[i][2]), lnTermPrior_XY + scores[i][3]);
        }
    ...
    private double scoreNode(int node, int whichList) {
        ...
        while (instancePtr < countsPtr) {
            double score;
            if (scoreFn == 1) {
                score = this.scoringFn1(node, instancePtr, q, priorEquivalentSampleSize);
            } else {
    ...
    private double gammln(double xx) {
        if (xx == 1) {
            ...
            if (xx > 1) {
                ...
                double z = 1 - xx;
                return log(Math.PI * z) - this.gammlnCore(1 + z) - log(Math.sin(Math.PI * z));
            }
        }
    }
