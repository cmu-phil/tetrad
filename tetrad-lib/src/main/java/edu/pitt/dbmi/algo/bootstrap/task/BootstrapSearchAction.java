...
public class BootstrapSearchAction extends RecursiveAction {
    ...
    public BootstrapSearchAction(int dataSetId, int workLoad, BootstrapAlgName algName, Parameters parameters,
    ...
    public Graph learnGraph(DataSet dataSet) {
        ...
        if (algName == BootstrapAlgName.GFCI || algName == BootstrapAlgName.RFCI) {
            if (dataSet.isContinuous()) {
                ...
                independenceTest = new IndTestConditionalGaussianLRT(dataSet,
                        parameters.getDouble("alpha", 0.5),
                        true);
            }
        }
    ...
    public void compute() {
        if (workLoad < 2) {
            ...
            BootstrapSearchAction task1 = new BootstrapSearchAction(dataSetId, workLoad / 2, algName, parameters,
                    bootstrapSearch, verbose);
            BootstrapSearchAction task2 = new BootstrapSearchAction(dataSetId + workLoad / 2, workLoad - workLoad / 2,
                    algName, parameters, bootstrapSearch, verbose);
