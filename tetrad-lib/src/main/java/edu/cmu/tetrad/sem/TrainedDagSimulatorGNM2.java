 ...
 * </ul>
 */
public final class TrainedDagSimulatorGNM2 {
     ...
     */
    public TrainedDagSimulatorGNM2(DataSet data, Graph dag, Params params) {
    ...
    private static final class MlpRegressor {
        ...
        void initHe(Random rng) {
            double s1 = TMath.sqrt(2.0 / TMath.max(1, din));
            for (int i = 0; i < hidden; i++) {
                for (int j = 0; j < din; j++) W1[i][j] = rng.nextGaussian() * s1;
                b1[i] = 0.0;
            }
            double s2 = TMath.sqrt(2.0 / TMath.max(1, hidden));
    ...
    private static final class MlpSoftmaxClassifier {
        ...
        private static double[] softmaxInto(double[] z, double[] out) {
            double max = z[0];
            for (int i = 1; i < z.length; i++) max = TMath.max(max, z[i]);
            double sum = 0.0;
    ...
    private final class ContinuousMechanism extends Mechanism implements TetradSerializable {
        ...
        void fit(DataSet data, Random rng, Params p) {
            ...
            for (int ep = 0; ep < p.epochs; ep++) {
                ...
                for (int start = 0; start < tr.n; start += p.batchSize) {
                    int end = TMath.min(tr.n, start + p.batchSize);
        ...
        private int discreteSignatureFromDataRow(DataSet data, int row, InputEncoder enc) {
            ...
            for (int j = 0; j < enc.parentIdx.length; j++) {
                ...
                int v = TrainedDagSimulatorGNM2.safeGetInt(data, row, col);
                h = 31 * h + v;
            }
            return h;
        }
    ...
    private final class DiscreteMechanism extends Mechanism implements TetradSerializable {
        ...
        void fit(DataSet data, Random rng, Params p) {
            ...
            for (int i = 0; i < tr.n; i++) {
                ...
                xent += -TMath.log(TMath.max(1e-300, probs[y]));
                used++;
            }
