...
public class MixedDataBox implements DataBox {
    ...
    public MixedDataBox(List<Node> variables, int numRows) {
        ...
        continuousData = new double[variables.size()][];
        discreteData = new int[variables.size()][];
    ...
    public Number get(int row, int col) {
        if (col >= continuousData.length || row >= this.numRows()) {
            return null;
        }
    ...
    @Override
    public DataBox copy() {
        MixedDataBox box = new MixedDataBox(variables, this.numRows());
