 ...
 */
public class CorrelationMatrixOnTheFly2 implements ICovarianceMatrix {
     ...
     */
    public CorrelationMatrixOnTheFly2(DataSet dataSet) {
        this(dataSet, false, Runtime.getRuntime().availableProcessors());
    }

    public CorrelationMatrixOnTheFly2(DataSet dataSet, int parallelism) {
        this(dataSet, false, parallelism);
    }

    public CorrelationMatrixOnTheFly2(DataSet dataSet, boolean verbose) {
        this(dataSet, verbose, Runtime.getRuntime().availableProcessors());
    }

    public CorrelationMatrixOnTheFly2(DataSet dataSet, boolean verbose, int parallelism) {
...
//                        v /= (sampleSize - 1);
     ...
     * order.
     */
    public final CorrelationMatrixOnTheFly2 getSubmatrix(String[] submatrixVarNames) {
        throw new UnsupportedOperationException();
    }
     ...
     */
    private final double getValueInernal(int i, int j) {
        if (i == j) {
            return variances[i];
        }
...
//        v /= (sampleSize - 1);
    ...
    public final double getValue(int i, int j) {
        return getValueInernal(i, j) / sqrt(getValueInernal(i, i) * getValueInernal(j, j));
    }
