...
public class RegressionCovariance implements Regression {
    ...
    public RegressionResult regress(Node target, List<Node> regressors) {
        try {
            ...
            Matrix rX = allCorrelations.view(xIndices, xIndices).matrix();
            Matrix rY = allCorrelations.view(xIndices, new int[]{yIndex}).matrix();
            ...
            Matrix r = allCorrelations.view(allIndices, allIndices).matrix();
