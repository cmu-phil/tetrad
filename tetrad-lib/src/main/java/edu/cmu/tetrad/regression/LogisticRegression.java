...
public class LogisticRegression implements TetradSerializable {
    ...
    private Result regress(int[] target, String targetName, double[][] regressors, List<String> regressorNames) {
        ...
        for (int j = 1; j <= numRegressors; j++) {
            xMeans[j] /= nc;
            xStdDevs[j] /= nc;
            xStdDevs[j] = TMath.sqrt(TMath.abs(xStdDevs[j] - xMeans[j] * xMeans[j]));
        }
        xMeans[0] = 0.0;
        xStdDevs[0] = 1.0;
        ...
        par[0] = TMath.log((double) ny1 / (double) ny0);
        for (int j = 1; j <= numRegressors; j++) {
            par[j] = 0.0;
        }
        ...
        while (TMath.abs(llP - ll) > 1e-7) {   /// 1e-7
        ...
        for (int j = 1; j <= numRegressors; j++) {
            ...
            parStdErr[j] = TMath.sqrt(arr[j][j]) / xStdDevs[j];
            par[0] = par[0] - par[j] * xMeans[j];
            double zScore = par[j] / parStdErr[j];
            double prob = norm(TMath.abs(zScore));

            pValues[j] = prob;
        }

        parStdErr[0] = TMath.sqrt(arr[0][0]);
    ...
    private double norm(double z) {
        double q = z * z;
        final double piOver2 = TMath.PI / 2.0;

        if (TMath.abs(q) > 7.0) {
            return (1.0 - 1.0 / q + 3.0 / (q * q)) * TMath.exp(-q / 2.0) /
                   (TMath.abs(z) * TMath.sqrt(piOver2));
        } else {
