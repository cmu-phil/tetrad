...
public class BooleanGlassGeneIm implements SessionModel {
    ...
    public BooleanGlassGeneIm(BooleanGlassGenePm genePm, Parameters parameters) {
        try {
            ...
            glassFunction = new BooleanGlassFunction(genePm.getLagGraph());
            initializer = new BasalInitializer(glassFunction, 0, 1);
            history = new GeneHistory(initializer, glassFunction);
            simulator = new MeasurementSimulatorParams(parameters);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    public List<String> getFactors() {
        ...
        IndexedLagGraph connectivity = glassFunction.getIndexedLagGraph();
    ...
    public List<String> getParents(int factor) {
        IndexedLagGraph connectivity = this.getBooleanGlassFunction()
                .getIndexedLagGraph();
...
//        System.out.println(measuredDataSet);
        ...
        if (measuredData[0][0].length == 1) {
            dataModelList.add(0, this.asTimeSeriesData(measuredData, 0, factors));
        }
