...
public class Matrix implements TetradSerializable {
    ...
    public Matrix(double[][] data) {
        if (data.length == 0) {
            apacheData = new Array2DRowRealMatrix();
        } else {
            apacheData = new BlockRealMatrix(data);
        }

        m = data.length;
        n = m == 0 ? 0 : data[0].length;
    }
    ...
    public Matrix copy() {
        if (this.zeroDimension()) return new Matrix(this.rows(), this.columns());
    ...
    public Matrix times(Matrix m) {
        if (zeroDimension() || m.zeroDimension())
            return new Matrix(rows(), m.columns());
        else {
    ...
    public Matrix inverse() throws SingularMatrixException {
        if (!this.isSquare()) throw new IllegalArgumentException("I can only invert square matrices.");
    ...
    public Matrix transpose() {
        if (this.zeroDimension()) return new Matrix(this.columns(), this.rows());
    ...
    public Vector sum(int direction) {
        if (direction == 1) {
            Vector sums = new Vector(this.columns());
            ...
            Vector sums = new Vector(this.rows());
