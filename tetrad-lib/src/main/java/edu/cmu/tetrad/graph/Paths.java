...
// --- Public API: Recursive (mask-based) adjustment ---
    ...
    public List<Set<Node>> adjustmentSetsSlow(Node source, Node target,
        ...
        Set<Node> forbidden = getForbiddenForAdjustment(graph, source, target);
...
//                            Set<Node> ancOfZmin = possibleAncestorsOf(Zmin, graphType);
    ...
    public List<Set<Node>> adjustmentSetsFast(Node X, Node Y, String graphType,
        ...
        Set<Node> forbidden = getForbiddenForAdjustment(graph, X, Y);
...
// Distance scheme matches your j=1 ⇒ {second-from-start, second-from-end}, etc.
    ...
    public RAEnumerate.AdjSummary recursiveAdjustment(
        ...
        Set<Node> forbGAC;
        try {
            forbGAC = new LinkedHashSet<>(getForbiddenForAdjustment(graph, source, target));
        } catch (Throwable t) {
