...
public final class Identifiability implements ManipulatingBayesUpdater {
   ...
   /**
    ...
    public double getJointMarginal(int[] sVariables, int[] sValues) {
		...
		{
			sNodes.add(bayesIm.getNode(sVariables[i]));
		}
		...
		List<List<Node>> cComponentNodes = new LinkedList<List<Node>>();
		...
		for (int i = 0; i < nCComponents; i++)
		{
			...
			QList qV = new QList(nNodes, probTermV, true);  
	...
	{
		int size = cComponents.length;
		
		int currentMax = -1;
		for (int i = 0; i < size; i++)
		{
			if (cComponents[i] > currentMax)
			{
				currentMax = cComponents[i];
			}
		}
