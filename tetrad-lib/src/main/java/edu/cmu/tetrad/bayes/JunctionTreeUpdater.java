    ...
    public class JunctionTreeUpdater implements ManipulatingBayesUpdater {
        ...
        public void setEvidence(Evidence evidence) {
            ...
            // In this implementation the "updated" IM is the manipulated IM for querying marginals/joints.
...
//            }
            ...
            syncJtiEvidenceFromCurrentEvidence();
        }
        ...
        public double getMarginal(int variable, int category) {
            ...
            // Query inference engine using dstVar (updatedBayesIm shares indices with manipulatedBayesIm)
            return this.jti.getMarginal(dstVar, category);
        }

        private void syncJtiEvidenceFromCurrentEvidence() {
            if (this.jti == null || this.manipulatedBayesIm == null) return;
...
//            // Hard singleton evidence (optional)
//            for (int i = 0; i < ev2.getNumNodes(); i++) {
//                int only = getOnlyAllowedCategoryOrMinusOne(p2, i, ev2.getNumCategories(i));
//                // IMPORTANT: clear if not singleton, so old singleton doesn't stick
//                this.jti.setEvidence(i, only); // your setEvidence clears on negative
//            }
        }
        ...
        public double[] calculatePriorMarginals(int nodeIndex) {
            ...
            for (int c = 0; c < k; c++) {
                marginals[c] = getMarginal(nodeIndex, c); // maps correctly
            }
