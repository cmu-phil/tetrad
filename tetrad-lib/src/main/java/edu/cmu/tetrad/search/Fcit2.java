...
public final class Fcit2 implements IGraphSearch {
    ...
    private static final Comparator<Edge> EDGE_ORDER =
            Comparator.comparing(Fcit2::minName).thenComparing(Fcit2::maxName);
    /**
     ...
     *
     * <p>Rationale, by analogy with PC's depth loop: FCIT's per-edge search
     * already prefers small candidate sets, but the ordering is per edge, not
     * global. With a single full-depth pass, an edge scanned early can be
     * discharged by a large separator before an edge scanned later would have
     * been discharged by a small one -- and the large-separator removal is the
     * less reliable of the two, both because it is tested with less power and
     * because it commits the legality gate to a state the small removal would
     * have reached differently. Deepening globally orders removals by separator
     * size, so every edge that a size-k separator can discharge is discharged
     * before any size-(k+1) separator is consulted, and the higher levels then
     * run against a sparser view.</p>
     *
     ...
     * unless {@link #iterativeDeepening} is true. Raising this skips levels
     * that are cheap but, on dense problems, rarely productive.
     */
    ...
    public Graph search() throws InterruptedException {
        ...
        // ---------------------------------------------------------------------
        // With deepening off this is the singleton schedule {depth}, so the
        // loop below collapses to exactly one call of the historical main loop.
        ...
        for (int level = 0; level < schedule.size(); level++) {
            boolean finalLevel = (level == schedule.size() - 1);
            this.activeDepth = schedule.get(level);
            strategy.setDepth(this.activeDepth);

            // The sweep-exhaustion memo records "no separator at this graph
            // version"; that verdict is relative to the cap in force when it was
            // taken, so it must not survive a cap change or the deeper levels
            // are silently skipped for exactly the pairs they exist to serve.
            // (foundSepsets is NOT cleared: a separator found at a lower level
            // has size <= that level's cap, so it stays valid at every later
            // one, and re-deriving it could only find a different valid set.)
            sweepFailedAt.clear();
            ...
            // Detection is deferred to the final level. It is the expensive
            // phase, its verdicts feed back into removal, and any leg it would
            // confirm early is still available at full depth -- so running it
            // at every level buys nothing and costs a scan per level.
            NongenuineScan scan = runMainLoop(finalLevel && detectionEnabled,
                    excludeSelectionBias, initialColliders, rounds);
    ...
    /**
     * Mutable round counter, so that round numbering is continuous across the
     * levels of a deepening schedule rather than restarting at each level.
     */
     ...
     * single-edge removal phase to fixpoint, the saturating pass, and -- when
     * {@code runDetection} -- the non-genuine-leg scan, iterated to a joint
     * fixpoint. This is the body that ran once inline before deepening was
     * added; with deepening off it is still called exactly once.
     *
     * @param runDetection          whether to run the detection scan in this loop
     * @param excludeSelectionBias  passed through to the removal gate
     * @param unshieldedTriples     the frozen initial colliders
     * @param rounds                shared round counter for logging
     * @return the last scan performed, or an empty scan if detection was off
     */
    private NongenuineScan runMainLoop(boolean runDetection, boolean excludeSelectionBias,
                                       Set<Triple> unshieldedTriples, RoundCounter rounds)
        ...
        do {
            ...
            do {
                TetradLogger.getInstance().log("\nRound: " + rounds.next());
            } while (removeEdgesRecursively(excludeSelectionBias, unshieldedTriples));

            if (saturatingPass(excludeSelectionBias, unshieldedTriples)) {
                progressed = true;
                lastScan = null;
                continue;
            }
    ...
    private Set<Node> quickCommonSubsetSepset(Node x, Node y, long deadline, Set<Set<Node>> tried)
        ...
        if (this.activeDepth >= 0) max = Math.min(max, this.activeDepth);
     ...
     * changes. See {@link #setDeepeningStart(int)} and
     * {@link #setUnlimitedRampCeiling(int)} for the schedule.</p>
     *
