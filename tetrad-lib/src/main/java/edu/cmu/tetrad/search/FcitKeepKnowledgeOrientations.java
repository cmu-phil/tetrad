 ...
 * equality with the canonical PAG of the implied Zhang MAG) rejects any graph carrying
 * knowledge marks beyond the invariant marks, so committed graphs are always canonical.
 * Changes from Fcit (2026-8): (1) redoGfciOrientation applies fciOrientbk immediately
 * after the reorient-to-circles step, so every candidate reorientation carries the
 * knowledge marks; (2) both commit gates (single-edge and saturating) use the
 * modulo-knowledge certificate legalPagModuloKnowledge instead of the strict one: the
 * candidate must equal the canonical PAG of its implied Zhang MAG after that canonical
 * PAG is re-refined with fciOrientbk and re-closed under the final rules, so every mark
 * is either invariant or knowledge-forced. When knowledge is empty the certificate
 * degenerates to exactly PagLegalityCheck.isLegalPag and this class behaves like Fcit.
 * The illegal-state memo remains sound: knowledge is a fixed input for the run, so the
 * certificate is still a deterministic function of (skeleton, sepsets). As in the Star
 * variant, when knowledge conflicts with data-derived invariant structure the
 * knowledge-refined candidate fails the certificate and the removal is reverted --
 * knowledge loses conservatively; that is a deliberate, contestable modeling choice.</p>
 *
 ...
 * @author josephramsey
 */
public final class FcitKeepKnowledgeOrientations implements IGraphSearch {
    ...
    private static final Comparator<Edge> EDGE_ORDER =
            Comparator.comparing(FcitKeepKnowledgeOrientations::minName).thenComparing(FcitKeepKnowledgeOrientations::maxName);
    /**
     ...
     */
    public FcitKeepKnowledgeOrientations(IndependenceTest test, Score score) {
        if (test == null) {
    ...
    private static void redoGfciOrientation(Graph pag, FciOrient fciOrient, Knowledge knowledge,
        ...
        // Change from Fcit: apply background-knowledge orientations before the collider
        // seeds and the final rules, mirroring StarFciKeepKnowledgeOrientations'
        // gfciOrientPag order (circles -> bk -> colliders -> closure). Without this the
        // candidate reorientation is knowledge-blind and the knowledge marks present in
        // the initial dagToPag graph are erased at the first committed removal.
     ...
     * StarFciKeepKnowledgeOrientations#legalPagModuloKnowledge. When knowledge is empty it delegates to
     * {@link PagLegalityCheck#isLegalPag(Graph, Set)} (the strict certificate Fcit uses). Otherwise: all nodes must
     * be measured; the implied Zhang MAG must exist and be a legal MAG (so the structural protections -- cycles,
     * almost-cycles, non-maximality -- are fully retained); and the candidate must equal the canonical PAG of that
     * MAG after the canonical PAG is re-refined with fciOrientbk and re-closed under the complete final rules by
     * {@link #refineWithKnowledge}. Every mark in an accepted graph is thereby either invariant in the equivalence
     * class or forced by knowledge (directly or via rule propagation); a graph that is "between a MAG and a PAG"
     * for any other reason still fails, exactly as under the strict certificate.
     * <p>
     * The refinement runs through {@code this.fciOrient}, whose strategy is bound at trial time to the trial's
     * scratch sepset map, so R4's data-driven discriminating-path resolutions are reproduced and its sepset
     * recordings land in the scratch map -- adopted on commit, discarded on revert (the P1 discipline).
     */
    private PagLegalityCheck.LegalPagRet legalPagModuloKnowledge(Graph pag, Set<Node> selection) {
        ...
        Graph pag2k = refineWithKnowledge(pag2);
    ...
    /**
     * Refines a graph with background knowledge and closes under the complete FCI final rules: fciOrientbk for the
     * required/forbidden-edge marks, then finalOrientation to propagate them. Works on a copy; identity when
     * knowledge is empty. On a graph that is already a knowledge-refined fixed point this is a no-op.
     */
    private Graph refineWithKnowledge(Graph graph) {
        if (knowledge == null || knowledge.isEmpty()) {
            return graph;
        }

        Graph g = graph.copy();
        fciOrient.fciOrientbk(knowledge, g, g.getNodes(), excludeSelectionBias);
        fciOrient.finalOrientation(g, excludeSelectionBias);
        return g;
    }
    ...
    private boolean tryToModifyGraph(Node x, Node y, Set<Node> b, String type, boolean excludeSelectionBias,
        ...
        if (!orientationFailed) {
            // Modulo-knowledge certificate (strict when knowledge is empty). The refinement
            // engine is this.fciOrient, whose strategy is currently bound to the trial's
            // scratch sepset map, so R4 recordings made during the certificate follow the
            // P1 discipline: adopted on commit, discarded on revert.
            legalPagQuiet = legalPagModuloKnowledge(this.pag, new HashSet<>(selection));
        }
    ...
    private boolean saturatingPass(boolean excludeSelectionBias, Set<Triple> initialColliders) {
        ...
        if (!orientationFailed) {
            ...
            legal = legalPagModuloKnowledge(this.pag, new HashSet<>(selection));
        }
