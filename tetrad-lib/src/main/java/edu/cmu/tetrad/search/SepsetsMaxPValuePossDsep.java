...
//        List<Node> adjk = graph.getAdjacentNodes(k);
        List<Node> adji = new ArrayList<>(SepsetsMaxPValuePossDsep.possibleDsep(i, k, this.graph, this.maxPathLength));
        List<Node> adjk = new ArrayList<>(SepsetsMaxPValuePossDsep.possibleDsep(k, i, this.graph, this.maxPathLength));
        adji.remove(k);
        adjk.remove(i);
