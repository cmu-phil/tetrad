...
public final class FcitSl2 implements IGraphSearch {
    ...
    /**
     * Whether the GUIDED WITNESS CONSTRUCTION replaces the configured generator (class walk /
     * staged / closure-cover) for the within-class passes. Motivation ("Bryan's brain"): an
     * expert reading a PAG does not enumerate the class to find a MAG hosting a deletion; they
     * pick the collider the blocking needs and mentally propagate what that choice FORCES --
     * no new unshielded collider anywhere (R1), no directed cycle (R2) -- and either arrive at
     * a witness in a handful of decisions or see a contradiction and know none exists down
     * that road. This generator does the same mechanically, and that is what changes the
     * scaling story: the class walk's cost grows with the SIZE OF THE CLASS (711 members at
     * six variables in PKE11; combinatorial at 15+), while this search's cost grows with the
     * number of BLOCKING DECISIONS on the bounded x..y path closure, which is local.
     * <p>
     * Search space: restricted PAGs -- the interim PAG's own marks plus committed circle
     * orientations -- rather than fully-resolved MAGs. This is the structural fix for the
     * closure-cover search's documented weakness ("searches MAG space and tests class
     * membership at the end ... can generate exactly the right moves and still never land in
     * class"): here the untouched circles are never prematurely resolved to the Zhang
     * orientation, so the leaf completion ({@link GraphTransforms#zhangMagFromPag} of the
     * restricted PAG) re-derives them COHERENTLY with the committed marks instead of
     * inheriting stale ones, and in-class-ness is aimed at by construction rather than
     * repaired after the fact.
     * <p>
     * Three sound in-class filters run DURING the search: (i) moves that would create an
     * unshielded collider the PAG does not carry are not generated (an unshielded
     * non-collider of the PAG is a non-collider in EVERY class member -- so collider status
     * is only ever a CHOICE at shielded triples); (ii) after each move, R1/R2 propagation
     * commits every mark the move entails, failing the branch when an entailed mark collides
     * with an invariant or a stamp-compatibility pin; (iii) a post-propagation consistency
     * check prunes new unshielded colliders and directed cycles. All three are necessary
     * conditions only -- propagation is partial (no R3/R4/R8-R10), and the discriminating-path
     * side of Markov equivalence is not propagated -- so class membership is still CERTIFIED
     * at the leaf by MagToPag equality, and every candidate still passes the gates
     * byte-for-byte ({@link #closureGateAndCommit}). As everywhere else in this file, the flag
     * changes only which candidates are proposed and in what order, never what is accepted.
     * <p>
     * False by default so existing baselines are untouched.
     */
    private boolean useGuidedConstruction = false;
    /**
     * Whether a pair the guided construction cannot host is handed to the configured legacy
     * generator (closure-cover if {@link #useClosureCoverSearch}, else staged / class walk)
     * rather than left uncommitted. True by default: with it on, the guided generator's reach
     * is a superset of the legacy path's, and {@code guidedFallbackCommits} keeps the residual
     * rate visible. Set false to measure the guided search's UNAIDED reach.
     */
    private boolean guidedFallbackToLegacy = true;
    /**
     * Safety cap on how many completed leaf candidates the guided search may emit per
     * deletion attempt. Leaves are few by design (each is a full blocking assignment, not a
     * class member), so this cap exists to bound the pathological case, not the common one.
     * Negative means uncapped.
     */
    private int guidedMaxLeaves = 512;
    /**
     * Guided-construction telemetry. {@code guidedCommits}: commits hosted by a guided leaf.
     * {@code guidedLeavesEmitted}: completed leaf candidates evaluated.
     * {@code guidedClassPrunes} / {@code guidedCyclePrunes}: branches abandoned by the
     * consistency checks (a new unshielded collider / a directed cycle among committed marks)
     * -- sound prunes, since both conditions fail in every class member consistent with the
     * branch's commitments. {@code guidedPropagationPrunes}: branches abandoned because R1/R2
     * propagation collided with an invariant mark or a stamp pin. {@code guidedPropagatedMarks}:
     * total marks committed on restricted PAGs, moves and propagation together; against the
     * move budget this measures how much of each witness the rules derived for free.
     * {@code guidedLeafMisseps}: leaves
     * that blocked every enumerated path but failed exact m-separation (an active path beyond
     * the enumeration bounds); each such failure attempts counterexample extraction, and
     * {@code guidedCounterexamplePaths} counts the extracted paths appended to the universe.
     * {@code guidedLeafIllegal} / {@code guidedLeafOffClass} /
     * {@code guidedLeafGateRefusals}: leaves rejected at the legality, class-certification,
     * and gate stages respectively. {@code guidedStampObstructions}: stamp-compatibility
     * requirements colliding with an invariant arrowhead, as in the closure search.
     * {@code guidedFallbackAttempts} / {@code guidedFallbackCommits}: pairs handed to the
     * legacy generator, and how many committed there.
     */
    private long guidedCommits = 0, guidedLeavesEmitted = 0, guidedClassPrunes = 0,
            guidedCyclePrunes = 0, guidedPropagationPrunes = 0, guidedPropagatedMarks = 0,
            guidedLeafMisseps = 0, guidedLeafIllegal = 0, guidedLeafOffClass = 0,
            guidedLeafGateRefusals = 0, guidedLeafDedups = 0, guidedStampObstructions = 0,
            guidedFallbackAttempts = 0, guidedFallbackCommits = 0, guidedCounterexamplePaths = 0;
    /**
    ...
    public Graph search() throws InterruptedException {
        ...
        if (useGuidedConstruction) {
            TetradLogger.getInstance().log("Guided construction: " + guidedCommits
                    + " commit(s), " + guidedLeavesEmitted + " leaf candidate(s) emitted, "
                    + guidedPropagatedMarks + " mark(s) committed (moves + propagation), "
                    + guidedPropagationPrunes + " propagation-conflict prune(s), "
                    + guidedClassPrunes + " new-collider prune(s), "
                    + guidedCyclePrunes + " cycle prune(s), "
                    + guidedLeafMisseps + " beyond-bound active-path miss(es), "
                    + guidedCounterexamplePaths + " counterexample path(s) extracted, "
                    + guidedLeafIllegal + " illegal completion(s), "
                    + guidedLeafOffClass + " off-class completion(s), "
                    + guidedLeafGateRefusals + " gate refusal(s), "
                    + guidedLeafDedups + " duplicate completion(s), "
                    + guidedStampObstructions + " invariant stamp obstruction(s).");
            TetradLogger.getInstance().log("Guided fallback: " + guidedFallbackAttempts
                    + " pair(s) handed to the legacy generator, " + guidedFallbackCommits
                    + " committed there"
                    + (guidedFallbackToLegacy ? "." : " (fallback DISABLED; unaided reach)."));
        }
    ...
    private boolean removeEdgesRecursively(boolean excludeSelectionBias, boolean escape)
        ...
        while (from < edgeList.size()) {
            ...
            boolean didChange = useGuidedConstruction
                    ? tryToModifyGraphGuided(x, y, h.cond, h.pValue(), excludeSelectionBias, escape)
                    : useClosureCoverSearch
...
//        System.out.println("MAGs to return = " + out);
    ...
    // ==================== GUIDED WITNESS CONSTRUCTION ====================
    //
    // A third CANDIDATE GENERATOR behind the SAME commit gates. See the javadoc at
    // {@link #useGuidedConstruction} for the full rationale; in one line: search over
    // RESTRICTED PAGs (committed circle orientations) with R1/R2 propagation and in-class
    // pruning after every move, so a hosting representative is CONSTRUCTED from the blocking
    // requirements the deletion imposes, never found by enumerating the class.
    //
    // Per (x, y, S): every x..y skeleton path must be blocked by S in the witness minus the
    // edge. The blocking options at an interior node are exactly the expert's: an S-node must
    // be a definite non-collider (a tail at it on a path edge), a non-S node blocks only as a
    // definite collider with no possibly-directed route into S. Each option is a small mark
    // commitment on the restricted PAG; propagation then commits everything the choice
    // entails and the consistency checks refuse choices no class member can realize. A state
    // in which every enumerated path is DEFINITELY blocked by committed marks alone is
    // completed to a MAG by zhangMagFromPag -- the untouched circles resolved coherently with
    // the commitments -- confirmed exactly, certified in-class by MagToPag equality, and
    // handed to closureGateAndCommit.
    //
    // Proved-vs-conjectured ledger: the collider filter, the propagation-conflict prune, and
    // the consistency prunes are all SOUND (each violated condition fails in every class
    // member consistent with the branch); their incompleteness costs only wasted branches.
    // Nothing here changes what is certified or committed.

    /**
     * Guided-construction replacement for {@link #tryToModifyGraph}: same contract, same
     * gates, different candidate generator. Stage 1 (the plain Zhang MAG) is preserved
     * verbatim and counted in {@code zhangCommits}, so the common case costs what it costs
     * today and the provenance ledger stays comparable across generators.
     * <p>
     * ESCAPE mode is delegated to the legacy path: this generator is in-class by design
     * (it never orients against the PAG's invariant marks and prunes new unshielded
     * colliders), so it has nothing to offer pass 3.
     */
    private boolean tryToModifyGraphGuided(Node x, Node y, Set<Node> b, double pValue,
                                           boolean excludeSelectionBias, boolean escape)
            throws InterruptedException {
        if (escape) {
            return legacyTryToModify(x, y, b, pValue, excludeSelectionBias, true);
        }
        ...
        // Stage 1: the canonical Zhang MAG, exactly as the other generators try it first.
        if (closureGateAndCommit(new EdgeListGraph(base), x, y, b, pValue, excludeSelectionBias,
                _removed, _edge, () -> zhangCommits++) == GateStatus.COMMITTED) {
            return true;
        }

        // Restricted-PAG state: the interim PAG's own marks. Stamp-compatibility TAILs are
        // committed as marks (so propagation sees their consequences up front); the stamp's
        // own arrowheads are held as PINS only, never committed -- classification is
        // pre-stamp, exactly as in the closure search.
        Graph W = new EdgeListGraph(_pag);
        Map<String, Endpoint> pins = new HashMap<>();
        applyStampCompatPinsGuided(_pag, base, W, pins, x, y, b);

        boolean committed = false;

        List<Object[]> trail0 = new ArrayList<>();
        boolean rootOk = guidedPropagate(W, pins, trail0);
        if (rootOk && !guidedNoNewColliders(W, _pag)) {
            guidedClassPrunes++;
            rootOk = false;
        }
        if (rootOk && W.paths().existsDirectedCycle()) {
            guidedCyclePrunes++;
            rootOk = false;
        }
        ...
        if (rootOk) {
            ...
            Set<String> leafTried = new HashSet<>();
            // NOTE: the Stage-1 Zhang key is deliberately NOT pre-seeded. When the root state
            // is already all-def-blocked its completion IS the Zhang MAG, and re-adjudicating
            // it here is what lets the m-sep confirm fail visibly so a counterexample path
            // (one beyond the enumeration bounds) can be extracted and targeted. Pre-seeding
            // silently deduplicated that leaf and dead-ended the whole search (the 15:3+3
            // X11--X17 miss that motivated the counterexample loop).
            int[] leafBudget = {guidedMaxLeaves < 0 ? Integer.MAX_VALUE : guidedMaxLeaves};

            committed = guidedDfs(W, pins, paths, budget, _pag, basePag, x, y, b, pValue,
                    excludeSelectionBias, _removed, _edge, visited, leafTried, leafBudget, deadline);
        }

        if (!committed && guidedFallbackToLegacy) {
            // Same shape as closureFallbackToStaged: nothing has been mutated at this point
            // (interimPags and sepsets are touched only via closureGateAndCommit, which runs
            // only on success), so the legacy generator sees exactly the state it would have
            // seen had it run alone.
            guidedFallbackAttempts++;
            committed = legacyTryToModify(x, y, b, pValue, excludeSelectionBias, false);
            if (committed) guidedFallbackCommits++;
        }

        if (!committed && verbose) {
            TetradLogger.getInstance().log("\tGuided construction: no candidate hosted " + _edge
                    + ", sepset = " + b);
        }
    ...
    /**
     * The generator the guided search defers to: closure-cover when
     * {@link #useClosureCoverSearch} is set, else the staged / class-walk search.
     */
    private boolean legacyTryToModify(Node x, Node y, Set<Node> b, double pValue,
                                      boolean excludeSelectionBias, boolean escape)
            throws InterruptedException {
        return useClosureCoverSearch
                ? tryToModifyGraphClosure(x, y, b, pValue, excludeSelectionBias, escape)
                : tryToModifyGraph(x, y, b, pValue, excludeSelectionBias, escape);
    }

    /**
     * Guided analog of {@link #applyStampCompatPins}. Two deliberate differences: the
     * okWithArrow probe reads the ZHANG MAG (as the original reads its Zhang-seeded
     * candidate -- probing the restricted PAG instead would pin TAIL far too often, the
     * over-tight-pin failure documented at the original), while the compat TAILs are
     * committed INTO the restricted PAG so propagation can derive their consequences
     * immediately rather than discovering them one refused branch at a time.
     */
    private void applyStampCompatPinsGuided(Graph pag, Graph base, Graph W, Map<String, Endpoint> pins,
                                            Node x, Node y, Set<Node> b) {
        List<Node> common = pag.getAdjacentNodes(x);
        common.retainAll(pag.getAdjacentNodes(y));
        ...
        for (Node c : common) {
            ...
            for (Node d : pag.getAdjacentNodes(c)) {
                ...
                boolean okWithArrow = (!unshX || base.getEndpoint(x, c) == Endpoint.ARROW)
                        && (!unshY || base.getEndpoint(y, c) == Endpoint.ARROW);
                if (okWithArrow) continue;

                Endpoint atC = pag.getEndpoint(d, c);

                if (atC == Endpoint.ARROW) {
                    // Invariant arrowhead: recorded as evidence, adjudicated by the gate.
                    guidedStampObstructions++;
                } else if (atC == Endpoint.CIRCLE) {
                    if (W.getEndpoint(d, c) == Endpoint.CIRCLE) {
                        W.setEndpoint(d, c, Endpoint.TAIL);
                        pins.putIfAbsent(slotKey(d, c), Endpoint.TAIL);
                    }
                }
                // atC == TAIL: invariant tail already satisfies the requirement.
            }
    ...
    /**
     * The construction DFS. At each node: find the first enumerated path NOT definitely
     * blocked by committed marks alone; branch on the blocking commitments that could close
     * it; propagate and consistency-check each; recurse. A state with no open path is
     * completed, confirmed, certified, and gated.
     */
    private boolean guidedDfs(Graph W, Map<String, Endpoint> pins, List<List<Node>> paths, int movesLeft,
                              Graph pag, Graph basePag, Node x, Node y, Set<Node> b, double pValue,
                              boolean excludeSelectionBias, List<Edge> removed, Edge edgeForLog,
                              Set<String> visited, Set<String> leafTried, int[] leafBudget, long deadline)
            throws InterruptedException {
        if (Thread.currentThread().isInterrupted()) {
            throw new InterruptedException();
        }
        if (System.currentTimeMillis() > deadline) return false;
        if (leafBudget[0] <= 0) return false;

        // Restricted states recur across move orders; explore each once.
        if (!visited.add(magKey(W))) return false;

        List<Node> open = firstOpenPath(W, paths, x, y, b);

        while (open == null) {
            // Every enumerated path is DEFINITELY blocked by committed marks alone -- blocked
            // in every completion of W, so the completion below cannot un-block one. But the
            // ENUMERATION is bounded: the completion may still be m-connected over a path the
            // enumeration never saw. When the exact confirm below fails, that path is
            // extracted from the completion and appended to the universe (a def-blocked
            // verdict on the committed marks would contradict its activity in a completion,
            // so it is guaranteed open), and the search resumes here with moves that can
            // finally target it. Progress: each pass through this loop either returns or
            // spends leaf budget on a new completion key.
            leafBudget[0]--;
            if (leafBudget[0] < 0) return false;

            Graph cand = guidedComplete(W, excludeSelectionBias);
            if (!leafTried.add(magKey(cand))) {
                guidedLeafDedups++;
                return false;
            }
            guidedLeavesEmitted++;

            // Exact confirmation (an active path beyond the enumeration bounds would
            // otherwise slip through), then legality, then class certification, then gates.
            Graph probe = new EdgeListGraph(cand);
            probe.removeEdge(x, y);
            if (!new MsepTest(probe).checkIndependence(x, y, b).isIndependent()) {
                guidedLeafMisseps++;
                List<Node> witness = activeSimplePath(probe, x, y, b);
                if (witness == null || containsPath(paths, witness)) {
                    return false;   // extraction cap hit, or no genuinely new path: dead end
                }
                paths.add(witness);
                guidedCounterexamplePaths++;
                open = firstOpenPath(W, paths, x, y, b);
                continue;
            }
            ...
            GateStatus st = closureGateAndCommit(cand, x, y, b, pValue, excludeSelectionBias,
                    removed, edgeForLog, () -> guidedCommits++);
            if (st == GateStatus.COMMITTED) return true;

            guidedLeafGateRefusals++;
            return false;
        }

        if (movesLeft == 0) return false;

        List<CoverMove> moves = guidedMoves(W, pag, pins, open, paths, x, y, b);

        for (CoverMove mv : moves) {
            boolean noop = true;
            for (Assign a : mv.assigns()) {
                if (W.getEndpoint(a.from(), a.at()) != a.end()) {
                    noop = false;
                    break;
                }
            }
            ...
            boolean ok = true;
            for (Assign a : mv.assigns()) {
                if (!guidedSet(W, pins, a.from(), a.at(), a.end(), trail)) {
                    ok = false;
                    break;
                }
            }

            if (ok && !guidedPropagate(W, pins, trail)) {
                guidedPropagationPrunes++;
                ok = false;
            }
            if (ok && !guidedNoNewColliders(W, pag)) {
                guidedClassPrunes++;
                ok = false;
            }
            if (ok && W.paths().existsDirectedCycle()) {
                guidedCyclePrunes++;
                ok = false;
            }

            boolean done = ok && guidedDfs(W, pins, paths, movesLeft - 1, pag, basePag, x, y, b,
                    pValue, excludeSelectionBias, removed, edgeForLog, visited, leafTried,
                    leafBudget, deadline);

            for (int i = trail.size() - 1; i >= 0; i--) {
                Object[] t = trail.get(i);
                W.setEndpoint((Node) t[0], (Node) t[1], (Endpoint) t[2]);
            }
    ...
    /**
     * Commits a single mark on the restricted PAG, recording the prior value on the trail.
     * Only circles may be filled: an attempt to overwrite any committed or invariant mark, or
     * to contradict a stamp pin, reports failure -- during propagation that failure IS the
     * expert's contradiction ("that choice would force an arrowhead where the class has a
     * tail"), and the branch is abandoned.
     */
    private boolean guidedSet(Graph W, Map<String, Endpoint> pins, Node from, Node at, Endpoint want,
                              List<Object[]> trail) {
        Endpoint cur = W.getEndpoint(from, at);
        if (cur == want) return true;
        if (cur != Endpoint.CIRCLE) return false;

        Endpoint pin = pins.get(slotKey(from, at));
        if (pin != null && pin != want) return false;

        trail.add(new Object[]{from, at, cur});
        W.setEndpoint(from, at, want);
        guidedPropagatedMarks++;
        return true;
    }

    /**
     * Sound, PARTIAL propagation of class-membership consequences on the restricted PAG:
     * Zhang's R1 and R2 to fixpoint, every assignment trailed for undo, every conflict
     * reported. R1 is exactly the "no new unshielded collider" constraint made productive
     * (a*-&gt;m o-* c unshielded forces m -&gt; c); R2 is ancestrality (chains force
     * arrowheads). No R3/R4/R8-R10 and no discriminating-path reasoning: missing propagation
     * costs only wasted branches, never a wrong commit -- class membership is certified at
     * the leaf regardless.
     */
    private boolean guidedPropagate(Graph W, Map<String, Endpoint> pins, List<Object[]> trail) {
        boolean changed = true;

        while (changed) {
            changed = false;

            // R1: a*->m o-* c, a and c nonadjacent  ==>  m --> c.
            for (Node m : W.getNodes()) {
                List<Node> adj = W.getAdjacentNodes(m);
                for (Node a : adj) {
                    if (W.getEndpoint(a, m) != Endpoint.ARROW) continue;
                    for (Node c : adj) {
                        if (c == a) continue;
                        if (W.isAdjacentTo(a, c)) continue;
                        if (W.getEndpoint(c, m) != Endpoint.CIRCLE) continue;

                        if (!guidedSet(W, pins, c, m, Endpoint.TAIL, trail)) return false;
                        if (!guidedSet(W, pins, m, c, Endpoint.ARROW, trail)) return false;
                        changed = true;
                    }
                }
            }

            // R2: (a --> m *-> c) or (a *-> m --> c), a *-o c  ==>  a *-> c.
            for (Node m : W.getNodes()) {
                List<Node> adj = W.getAdjacentNodes(m);
                for (Node a : adj) {
                    for (Node c : adj) {
                        if (c == a) continue;
                        if (!W.isAdjacentTo(a, c)) continue;
                        if (W.getEndpoint(a, c) != Endpoint.CIRCLE) continue;

                        boolean form1 = W.getEndpoint(a, m) == Endpoint.ARROW
                                && W.getEndpoint(m, a) == Endpoint.TAIL
                                && W.getEndpoint(m, c) == Endpoint.ARROW;
                        boolean form2 = W.getEndpoint(a, m) == Endpoint.ARROW
                                && W.getEndpoint(m, c) == Endpoint.ARROW
                                && W.getEndpoint(c, m) == Endpoint.TAIL;

                        if (form1 || form2) {
                            if (!guidedSet(W, pins, a, c, Endpoint.ARROW, trail)) return false;
                            changed = true;
                        }
                    }
                }
    ...
    /**
     * No unshielded triple may be a definite collider in the restricted PAG unless it is one
     * in the interim PAG: unshielded colliders are class invariants, so a new one fails in
     * every class member consistent with the branch. Necessary only; sufficiency is the
     * leaf's MagToPag certification.
     */
    private boolean guidedNoNewColliders(Graph W, Graph pag) {
        for (Node m : W.getNodes()) {
            List<Node> adj = W.getAdjacentNodes(m);
            for (int i = 0; i < adj.size(); i++) {
                for (int j = i + 1; j < adj.size(); j++) {
                    ...
                    if (W.isAdjacentTo(a, c)) continue;
                    if (W.isDefCollider(a, m, c) && !pag.isDefCollider(a, m, c)) return false;
                }
            }
        }
        return true;
    ...
    /**
     * First enumerated path NOT definitely blocked by committed marks alone. The 2-node path
     * is the deleted edge itself and never counts.
     */
    private List<Node> firstOpenPath(Graph W, List<List<Node>> paths, Node x, Node y, Set<Node> S) {
        for (List<Node> p : paths) {
            if (p.size() <= 2) continue;
            if (!isDefBlockedGivenS(W, p, x, y, S)) return p;
        }
        return null;
    }

    /**
     * A simple m-connecting x..y path given S in {@code probe} (a MAG with the x--y edge
     * already removed), or null if the expansion cap is hit first. Used only for
     * counterexample extraction after an exact m-separation confirm has FAILED, so a
     * connecting path is known to exist; the cap is a defensive bound, not a completeness
     * bound. Standard per-triple activity: a collider on the path is active iff it has a
     * descendant in S (itself included); a non-collider is active iff it is outside S.
     * If an active walk exists, an active simple path exists, so the simple-path DFS is
     * exhaustive in principle.
     */
    private List<Node> activeSimplePath(Graph probe, Node x, Node y, Set<Node> S) {
        Deque<Node> path = new ArrayDeque<>();
        Set<Node> onPath = new HashSet<>();
        path.addLast(x);
        onPath.add(x);
        int[] steps = {200_000};
        List<Node> out = activeSimplePathDfs(probe, x, y, S, path, onPath, steps);
        return (out != null && out.size() >= 3) ? out : null;
    }

    private List<Node> activeSimplePathDfs(Graph probe, Node cur, Node y, Set<Node> S,
                                           Deque<Node> path, Set<Node> onPath, int[] steps) {
        if (--steps[0] < 0) return null;

        for (Node next : probe.getAdjacentNodes(cur)) {
            if (onPath.contains(next)) continue;

            // The triple (prev, cur, next) must be active before extending through cur.
            if (path.size() >= 2) {
                Iterator<Node> it = path.descendingIterator();
                it.next();                    // cur
                Node prev = it.next();
                boolean collider = probe.getEndpoint(prev, cur) == Endpoint.ARROW
                        && probe.getEndpoint(next, cur) == Endpoint.ARROW;
                boolean active = collider
                        ? hasDescendantIn(probe, cur, S)
                        : !S.contains(cur);
                if (!active) continue;
            }
            ...
            onPath.add(next);
            if (next.equals(y)) {
                List<Node> found = new ArrayList<>(path);
                onPath.remove(next);
                path.removeLast();
                return found;
            }
            List<Node> found = activeSimplePathDfs(probe, next, y, S, path, onPath, steps);
            onPath.remove(next);
            path.removeLast();
            if (found != null) return found;
            if (steps[0] < 0) return null;
    ...
    private boolean hasDescendantIn(Graph g, Node from, Set<Node> S) {
        if (S.contains(from)) return true;
        Deque<Node> queue = new ArrayDeque<>();
        Set<Node> seen = new HashSet<>();
        queue.add(from);
        seen.add(from);
        while (!queue.isEmpty()) {
            ...
            for (Node child : g.getChildren(cur)) {
                ...
                if (S.contains(child)) return true;
                queue.add(child);
            }
        }
        return false;
    ...
    private boolean containsPath(List<List<Node>> paths, List<Node> p) {
        for (List<Node> q : paths) {
            if (q.equals(p)) return true;
            // Skeleton paths are undirected; the reverse is the same path.
            if (q.size() == p.size()) {
                boolean rev = true;
                for (int i = 0; i < q.size(); i++) {
                    if (!q.get(i).equals(p.get(p.size() - 1 - i))) {
                        rev = false;
                        break;
                    }
                }
    ...
    /**
     * DEFINITE blocking on a partially oriented graph -- true only when the path is blocked
     * in EVERY completion: an S-node with a committed tail on a path edge is a non-collider
     * in every completion; a non-S definite collider with no possibly-directed route into S
     * has no descendant in S in any completion. Compare {@link #isActiveGivenS}, which reads
     * a fully-resolved candidate.
     */
    private boolean isDefBlockedGivenS(Graph W, List<Node> path, Node x, Node y, Set<Node> S) {
        for (int i = 1; i < path.size() - 1; i++) {
            Node a = path.get(i - 1), m = path.get(i), c = path.get(i + 1);

            if (S.contains(m)) {
                if (W.getEndpoint(a, m) == Endpoint.TAIL || W.getEndpoint(c, m) == Endpoint.TAIL) {
                    return true;
                }
            } else {
                if (W.isDefCollider(a, m, c) && !possiblyAncestorOfS(W, m, S, x, y)) {
                    return true;
                }
            }
        }
    ...
    /**
     * BFS over potentially directed edges (no arrowhead at the near end, no tail at the far
     * end), skipping the x--y edge itself since m-separation is evaluated in the graph with
     * that edge removed. Conservative in the safe direction: false only when NO completion
     * of the restricted PAG can make m an ancestor of a member of S.
     */
    private boolean possiblyAncestorOfS(Graph W, Node m, Set<Node> S, Node x, Node y) {
        Deque<Node> queue = new ArrayDeque<>();
        Set<Node> seen = new HashSet<>();
        queue.add(m);
        seen.add(m);
        ...
        while (!queue.isEmpty()) {
            ...
            for (Node w : W.getAdjacentNodes(cur)) {
                if (seen.contains(w)) continue;
                if ((cur == x && w == y) || (cur == y && w == x)) continue;
                if (W.getEndpoint(w, cur) != Endpoint.ARROW        // no arrowhead back into cur
                        && W.getEndpoint(cur, w) != Endpoint.TAIL) {  // no tail at w
                    seen.add(w);
                    queue.addLast(w);
                }
            }
    ...
    /**
     * First potentially directed m..s route for any s in S, for route-break move generation.
     * Same edge semantics as {@link #possiblyAncestorOfS}.
     */
    private List<Node> firstPotentiallyDirectedRoute(Graph W, Node m, Set<Node> S, Node x, Node y) {
        Deque<Node> queue = new ArrayDeque<>();
        ...
        while (!queue.isEmpty()) {
            Node cur = queue.removeFirst();
            if (!cur.equals(m) && S.contains(cur)) {
                LinkedList<Node> route = new LinkedList<>();
                for (Node t = cur; ; t = parent.get(t)) {
                    route.addFirst(t);
                    if (t.equals(m)) break;
                }
                return route;
            ...
            for (Node w : W.getAdjacentNodes(cur)) {
                ...
                if ((cur == x && w == y) || (cur == y && w == x)) continue;
                if (W.getEndpoint(w, cur) != Endpoint.ARROW
                        && W.getEndpoint(cur, w) != Endpoint.TAIL) {
                    parent.put(w, cur);
                    queue.addLast(w);
                }
            }
    ...
    /**
     * Move families for the first open path, in restricted-PAG space. The families mirror
     * {@link #coverMoves} with one expert filter the MAG-space search cannot apply:
     * collider-izing an UNSHIELDED triple the PAG does not already show as a collider is
     * never generated, because an unshielded non-collider of the PAG is a non-collider in
     * every class member -- collider status is only ever a choice at shielded triples.
     */
    private List<CoverMove> guidedMoves(Graph W, Graph pag, Map<String, Endpoint> pins,
                                        List<Node> path, List<List<Node>> allPaths,
                                        Node x, Node y, Set<Node> S) {
        List<CoverMove> out = new ArrayList<>();
        ...
        for (int i = 1; i < path.size() - 1; i++) {
            ...
            if (!S.contains(m)) {
                if (!W.isDefCollider(a, m, c)) {
                    // (1) Collider-ize -- only where a class member could.
                    boolean classViable = W.isAdjacentTo(a, c) || pag.isDefCollider(a, m, c);
                    if (classViable
                            && guidedSlotFree(W, pins, a, m, Endpoint.ARROW)
                            && guidedSlotFree(W, pins, c, m, Endpoint.ARROW)
                            && !invariantAncestorOfS(pag, m, S)) {
                        out.add(new CoverMove(List.of(
                                new Assign(a, m, Endpoint.ARROW),
                                new Assign(c, m, Endpoint.ARROW))));
                    }
                } else if (!invariantAncestorOfS(pag, m, S)) {
                    // (2) Route breaks: m is already a definite collider but can still reach
                    // S; sever the first potentially directed route, one edge per move (an
                    // arrowhead at the route's earlier node kills its potential direction).
                    List<Node> route = firstPotentiallyDirectedRoute(W, m, S, x, y);
                    if (route != null) {
                        for (int k = 0; k < route.size() - 1; k++) {
                            Node u = route.get(k), w = route.get(k + 1);
                            if (guidedSlotFree(W, pins, w, u, Endpoint.ARROW)) {
                                out.add(new CoverMove(List.of(new Assign(w, u, Endpoint.ARROW))));
                            }
                        }
                    }
                }
            } else {
                // (3) De-collider-ize: a committed tail at the S-node blocks in every
                // completion; the companion arrowhead keeps the edge's legal shape.
                addGuidedTailMove(W, pins, a, m, out);
                addGuidedTailMove(W, pins, c, m, out);
            }
        }

        // (4) Aggregate fork-flip across all open paths -- the Stage-2b composite, filtered
        // by the same class-viability rule per unshielded neighbor pair; the consistency
        // prune adjudicates residual cases.
        Map<Node, Set<Node>> forkNbrs = new LinkedHashMap<>();
        for (List<Node> p : allPaths) {
            if (p.size() <= 2) continue;
            if (isDefBlockedGivenS(W, p, x, y, S)) continue;
            for (int i = 1; i < p.size() - 1; i++) {
                Node a = p.get(i - 1), m = p.get(i), c = p.get(i + 1);
                if (S.contains(m)) continue;
                if (W.isDefCollider(a, m, c)) continue;
                forkNbrs.computeIfAbsent(m, k -> new LinkedHashSet<>()).add(a);
                forkNbrs.get(m).add(c);
            }
        }
        ...
        for (Map.Entry<Node, Set<Node>> en : forkNbrs.entrySet()) {
            ...
            boolean feasible = true;
            for (Node w : en.getValue()) {
                if (!guidedSlotFree(W, pins, w, m, Endpoint.ARROW)) {
                    feasible = false;
                    break;
                }
                assigns.add(new Assign(w, m, Endpoint.ARROW));
            }
            if (feasible && assigns.size() > 1) out.add(new CoverMove(assigns));
        }
    ...
    /**
     * True iff the slot is a circle (or already the wanted mark) and no pin says otherwise --
     * the restricted-space analog of {@link #slotCanBe}, reading the LIVE restricted PAG
     * rather than the frozen interim PAG so earlier commitments constrain later moves.
     */
    private boolean guidedSlotFree(Graph W, Map<String, Endpoint> pins, Node from, Node at, Endpoint want) {
        Endpoint cur = W.getEndpoint(from, at);
        if (cur != Endpoint.CIRCLE && cur != want) return false;
        Endpoint pin = pins.get(slotKey(from, at));
        return pin == null || pin == want;
    }

    /**
     * Restricted-space analog of {@link #addTailMove}: tail at m on far--m, with the
     * companion arrowhead at far when far's mark is still a circle (a tail against a tail
     * would be a selection edge). Adds nothing when infeasible.
     */
    private void addGuidedTailMove(Graph W, Map<String, Endpoint> pins, Node far, Node m,
                                   List<CoverMove> out) {
        if (!guidedSlotFree(W, pins, far, m, Endpoint.TAIL)) return;

        List<Assign> assigns = new ArrayList<>();
        assigns.add(new Assign(far, m, Endpoint.TAIL));

        Endpoint atFar = W.getEndpoint(m, far);
        if (atFar == Endpoint.TAIL && W.getEndpoint(far, m) != Endpoint.TAIL) {
            return;   // committing the tail would mint an undirected (selection) edge
        }
        if (atFar == Endpoint.CIRCLE) {
            if (!guidedSlotFree(W, pins, m, far, Endpoint.ARROW)) return;
            assigns.add(new Assign(m, far, Endpoint.ARROW));
        }
    ...
    /**
     * Completes the restricted PAG to a MAG. Tail-against-circle edges are pre-resolved
     * first: {@link GraphTransforms#zhangMagFromPag} renders them UNDIRECTED (the
     * selection-allowed reading), which is illegal here when selection is excluded, and the
     * only selection-free MAG completion of such an edge is the arrowhead at the circle end.
     * Every other circle is resolved by the Zhang completion itself, coherently with the
     * committed marks.
     */
    private Graph guidedComplete(Graph W, boolean excludeSelectionBias) {
        Graph V = new EdgeListGraph(W);

        if (excludeSelectionBias) {
            for (Edge e : new ArrayList<>(V.getEdges())) {
                Node u = e.getNode1(), v = e.getNode2();
                if (V.getEndpoint(u, v) == Endpoint.TAIL && V.getEndpoint(v, u) == Endpoint.CIRCLE) {
                    V.setEndpoint(v, u, Endpoint.ARROW);       // tail at v, circle at u: v --> u
                } else if (V.getEndpoint(v, u) == Endpoint.TAIL && V.getEndpoint(u, v) == Endpoint.CIRCLE) {
                    V.setEndpoint(u, v, Endpoint.ARROW);       // tail at u, circle at v: u --> v
                }
            }
        }

        return GraphTransforms.zhangMagFromPag(V);
    }

    // ================== END GUIDED WITNESS CONSTRUCTION ==================
    ...
    /**
     * Sets whether the guided witness construction replaces the configured generator for the
     * within-class passes. See {@link #useGuidedConstruction}.
     *
     * @param useGuidedConstruction true to use the guided construction.
     */
    public void setUseGuidedConstruction(boolean useGuidedConstruction) {
        this.useGuidedConstruction = useGuidedConstruction;
    }

    /**
     * Sets whether a pair the guided construction cannot host is handed to the configured
     * legacy generator. See {@link #guidedFallbackToLegacy}.
     *
     * @param guidedFallbackToLegacy true to fall back.
     */
    public void setGuidedFallbackToLegacy(boolean guidedFallbackToLegacy) {
        this.guidedFallbackToLegacy = guidedFallbackToLegacy;
    }

    /**
     * Sets the per-deletion cap on completed leaf candidates for the guided construction;
     * negative means uncapped. See {@link #guidedMaxLeaves}.
     *
     * @param guidedMaxLeaves the cap.
     */
    public void setGuidedMaxLeaves(int guidedMaxLeaves) {
        this.guidedMaxLeaves = guidedMaxLeaves;
    }
