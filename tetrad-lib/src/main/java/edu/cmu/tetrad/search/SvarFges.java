...
public final class SvarFges implements IGraphSearch {
    private final Fges fges;
    private final EdgeReplicationPolicy policy = new LagReplicationPolicy();
    ...
    @Override
    public Graph search() throws InterruptedException {
        List<Node> nodes = fges.getTest().getCov().getVariables(); // or from test/score
        Graph working = new ReplicatingGraph(nodes, policy);
        // Use whatever FGES API you have to start from a provided graph:
        // prefer a method that lets FGES mutate *this* instance.
        fges.setInitialGraph(working);      // if available
        Graph out = fges.search();          // FGES will mutate 'working'
        // If FGES returns a new graph instead of mutating, wrap the result:
        // out = new RepeatingGraph(out, policy);
        return out;
    }
...
//    public void setNumCPDAGsToStore(int i) {
//        fges.setNumCPDAGsToStore(i);
//    }
