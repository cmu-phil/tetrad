...
public final class Rcit implements IndependenceTest, RowsSettable {
    ...
    private static SimpleMatrix ridgeResidual(SimpleMatrix X, SimpleMatrix Z, double lambda) {
        ...
        SimpleMatrix Czz = Zt.mult(Z).scale(1.0 / denom);
        SimpleMatrix Czx = Zt.mult(X).scale(1.0 / denom);
    ...
    /** Covariance of elementwise residual products (Khatri-Rao rows). */
    private static SimpleMatrix kronResCov(SimpleMatrix RX, SimpleMatrix RY) {
        int n = RX.getNumRows();
        int fx = RX.getNumCols();
        int fy = RY.getNumCols();
        int fz = fx * fy;
        SimpleMatrix Z = new SimpleMatrix(n, fz);

        double[] xd = RX.getDDRM().data;  // n×fx row-major
        double[] yd = RY.getDDRM().data;  // n×fy row-major
        double[] zd = Z.getDDRM().data;   // n×(fx*fy) row-major
    ...
    /** cov(A, permuted(B)) given AT = A^T (p×n); assumes centered columns. */
    private static SimpleMatrix covWithPermutedB(SimpleMatrix AT, SimpleMatrix B, int[] perm) {
        int n = B.getNumRows();
        int q = B.getNumCols();

        SimpleMatrix Bp = new SimpleMatrix(n, q);
        double[] src = B.getDDRM().data;
        double[] dst = Bp.getDDRM().data;
        for (int i = 0; i < n; i++) {
            System.arraycopy(src, perm[i] * q, dst, i * q, q);
        }
        return AT.mult(Bp).scale(1.0 / (n - 1));
    }
    ...
    private static double[][] sampleOrthogonalW(int mFeatures, int d, double wStd, java.util.Random rng) {
        ...
        while (filled < mFeatures) {
            ...
            for (int i = 0; i < block; i++) {
                for (int j = 0; j < d; j++) Q[i][j] = rng.nextGaussian();
            }
    ...
    public IndependenceResult checkIndependence(Node x, Node y, Set<Node> z)
        ...
        // Features are z-scored (hence centered) inside rffOrOrfFeaturesFor,
        // so cache hits require no per-call reprocessing.
        ...
        if (FZ == null || FZ.getNumCols() == 0) {
            ...
            SimpleMatrix XY = FX.concatColumns(FY);
            SimpleMatrix RXY = ridgeResidual(XY, FZ, lambda);
            int fx = FX.getNumCols();
            RX = RXY.extractMatrix(0, n, 0, fx);
            RY = RXY.extractMatrix(0, n, fx, RXY.getNumCols());
        }
        ...
        if (approx == Approx.PERMUTATION && permutations > 0) {
            ...
            java.util.Random permRng = new java.util.Random(permSeed);
            ...
            for (int b = 0; b < permutations; b++) {
                ...
                SimpleMatrix Cb = covWithPermutedB(RXt, RY, perm);
...
//        }
    ...
    private SimpleMatrix rffOrOrfFeaturesFor(List<Node> vs, int mFeatures, String tag) {
        ...
        // z-score feature columns (implies centering; cov() assumes centered)
        zscoreInPlace(M);
    ...
    private double[][] rffFeatures(double[][] Z, int mFeatures, double bw2, long seed) {
        ...
        // Local RNG: makes feature generation deterministic per (block, seed)
        // and independent of the global RandomUtil singleton (thread-safe here).
        ...
        if (d == 0) {
            ...
            for (int j = 0; j < mFeatures; j++) b0[j] = 2.0 * TMath.PI * rng.nextDouble();
            for (int i = 0; i < n; i++) {
        ...
        if (featureType == FeatureType.RFF) {
            ...
            for (int j = 0; j < mFeatures; j++) {
                for (int k = 0; k < d; k++) W[j][k] = wStd * rng.nextGaussian();
    ...
    private static Moments gammaMomentsNoEig(SimpleMatrix RX, SimpleMatrix RY) {
        ...
        // ---- tr(Cov^2) = (1/(n-1)^2) * ||Z Z^T||_F^2 with
        // (Z Z^T)[r,s] = <x_r,x_s> * <y_r,y_s>.
        // Exploit symmetry (sum diagonal once, off-diagonal twice) and never
        // materialize the n×n Gram matrices — O(n) memory instead of O(n^2).
