...
public final class LatentNameAssigner {
    ...
    public static BlockSpec giveGoodLatentNames(
        ...
        for (int i = 0; i < blocks.size(); i++) {
            if (i < origLatents.size() && origLatents.get(i) != null) {
                origNames.add(sanitizeName(origLatents.get(i).getName()));
            } else {
                origNames.add("L" + (i + 1));
            }
        }
        ...
        if (mode == NamingMode.LEARNED_SINGLE) {
            ...
            for (int bi = 0; bi < blocks.size(); bi++) {
                final String preferredOrig = maybeCollapse(cleanDecorations(origNames.get(bi)), config);
                final String baseDisplay  = maybeCollapse(cleanDecorations(baseNames.get(bi)), config);
            ...
            for (int bi = 0; bi < blocks.size(); bi++) {
                ...
                final String baseCandidate = maybeCollapse(cleanDecorations(baseNames.get(bi)), config);
                final String origCandidate = maybeCollapse(cleanDecorations(origNames.get(bi)), config);
                ...
                for (int j = 0; j < r; j++) {
                    final String candidate = (j == 0) ? base : nextLetteredBase(base, j, config);
                    final String name = ensureUnique(candidate, used, config);
                    used.add(name);
    ...
    public static final class Config {
        ...
        public static Config defaults() {
            return new Config(3, true, "-", "+", "Mixed", true, "-");
        }
    ...
    // If you already have this elsewhere, use your enum and remove this copy.
