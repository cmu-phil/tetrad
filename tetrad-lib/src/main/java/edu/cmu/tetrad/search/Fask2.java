...
//        D = dataSet.getDoubleData().transpose().toArray();
        ...
        Grasp grasp = new Grasp(this.score);
        ...
        for (int i = 0; i < V; i++) {
            for (int j = i + 1; j < V; j++) {
                ...
                double cx = Fask2.correxp(x, y, x);
                double cy = Fask2.correxp(x, y, y);
...
//        lr *= signum(r);
    ...
    private double faskLeftRightV1(double[] x, double[] y) {
        double left = Fask2.cu(x, y, x) / (sqrt(Fask2.cu(x, x, x) * Fask2.cu(y, y, x)));
        double right = Fask2.cu(x, y, y) / (sqrt(Fask2.cu(x, x, y) * Fask2.cu(y, y, y)));
