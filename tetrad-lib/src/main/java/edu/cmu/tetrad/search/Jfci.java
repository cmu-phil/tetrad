...
//        System.out.println("Degree of graph = " + DataGraphUtils.getDegree(graph));
        ...
        List<Edge> edges = new ArrayList<Edge>(fullGraph.getEdges());
...
//            System.out.println("Num removed = " + numRemoved);
    ...
    private Graph orient(Graph graph, SepsetMap sepsets) {
        return orientPc(graph, knowledge, sepsets, verbose);
    }
...
//                if (node1 == x || node1 == y) continue;
    ...
    private Graph orientPc(Graph graph, IKnowledge knowledge, SepsetMap sepsets, boolean verbose) {
        graph = GraphUtils.undirectedGraph(graph);
        SearchGraphUtils.pcOrientbk(knowledge, graph, graph.getNodes());
