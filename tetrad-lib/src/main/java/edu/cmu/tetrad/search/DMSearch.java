...
public class DMSearch {
    ...
    public Graph search() {
        ...
        if (useFges) {
            ...
            CPDAG = this.recursiveFges(CPDAG, knowledge, gesDiscount, this.getMinDepth(), data, inputString);
        } else {
            cov = new CovarianceMatrix(data);
...
//            ExecutorService executorService = Executors.newFixedThreadPool(4); // number of threads
            ...
            IndTestFisherZ ind = new IndTestFisherZ(cov, alphaPC);
...
//                    System.out.println(j);
            ...
            this.applyDmSearch(CPDAG, inputString, penalty);
        }
...
//        System.out.println(outputNodes);
        ...
        for (Node node : outputNodes) {
            outputParentsList.add(new TreeSet<>(this.getInputParents(node, inputString, CPDAG)));
        }
...
//                    }
            ...
            if (sameSetParents.size() > 0) {
                ...
                if (!this.setContained(structure, structure.inputs.keySet(), sameSetParents) || structure.inputs.isEmpty()) {
                ...
                for (Node node : outputNodes) {
                    ...
                    if (new TreeSet<>(this.getInputParents(node, inputString, CPDAG)).equals(sameSetParents)) {
        ...
        TreeMap<TreeSet<Node>, Node> latentsSortedByInputSetSize = this.sortMapByValue(structure.inputs, structure.latents, structure);
...
//          2DO: Need to only perform this test if haven't already looked at latent. (for latent 1).
            ...
            inputs1 = this.findFirstUnseenElement(sortedInputs, alreadyLookedAt, latentsSortedByInputSetSize);
            ...
            for (int j = 0; j <= latentsSortedByInputSetSize.keySet().size(); j++) {
                ...
                inputs2 = this.findFirstUnseenElement(sortedInputs, alreadyLookedAtInnerLoop, latentsSortedByInputSetSize);
                ...
                if (structure.getInputs(latent2).containsAll(structure.getInputs(latent1))) {
                    ...
                    latentsSortedByInputSetSize = this.removeSetInputs(structure, structure.getInputs(latent1),
        ...
        for (Node latent : structure.getLatents()) {
            if (structure.latentEffects.containsKey(latent)) {
                for (Node latentEffect : structure.getLatentEffects(latent)) {
                    this.applySobersStep(structure.getInputs(latent), structure.getInputs(latentEffect),
...
//        fges.setIgnoreLinearDependent(true);
        ...
        if (penalty > minPenalty) {
            this.applyDmSearch(CPDAG, inputString, penalty);
            return (this.recursiveFges(CPDAG, knowledge, penalty - 1, minPenalty, data, inputString));
        } else {
            this.applyDmSearch(CPDAG, inputString, penalty);
            return (CPDAG);
        }
    ...
    private void applySobersStep(SortedSet<Node> inputsLatent, SortedSet<Node> inputsLatentEffect,
        ...
        IndependenceTest test = new IndTestFisherZ(cov, alphaSober);
    ...
    public class LatentStructure {
        ...
        public void removeLatent(Node latent) {
            latents.remove(latent);
            inputs.remove(latent);
            outputs.remove(latent);
            latentEffects.remove(latent);
        }
