...
//            long time1 = System.currentTimeMillis();
            SvarFciOrient svarFciOrient = new SvarFciOrient(new SepsetsSet(sepsets, independenceTest), independenceTest);
...
//            new SvarFciOrient(new SepsetsSet(this.sepsets, independenceTest), independenceTest).ruleR0(graph);
            ...
            for (Edge edge : new ArrayList<>(graph.getEdges())) {
                ...
                if (sepset != null) {
                    ...
                    this.removeSimilarPairs(this.getIndependenceTest(), x, y, sepset); // added 4.27.2016
                }
            }
...
//        new FciOrient(graph, new Sepsets(this.sepsets)).ruleR0(new Sepsets(this.sepsets));
        ...
        SvarFciOrient fciOrient = new SvarFciOrient(new SepsetsSet(sepsets, independenceTest), independenceTest);
...
//                       Collections.sort(temptier);
                    ...
                    for (int j = 0; j < temptier.size(); ++j) {
                        ...
                        if (this.getNameNoLag(tempNode.getName()).equals(this.getNameNoLag(temptier.get(j)))) {
                            ind_temp = j;
                            break;
                        }
                    }
