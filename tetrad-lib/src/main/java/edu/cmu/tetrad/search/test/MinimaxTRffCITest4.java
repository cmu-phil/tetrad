...
/**
 * <p><b>Minimax-t RFF Conditional Independence Test (mixed), v4</b></p>
 *
 * <p>
 ...
 * </p>
 *
 * <p><b>Calibration (v4): Freedman–Lane residual permutation for continuous Y.</b>
 * <ul>
 *   <li>Fit null model to get yhat0 and residuals e.</li>
 *   <li>Within strata of Z (quantile bins for continuous Z, exact match for discrete Z), shuffle residuals e.</li>
 *   <li>Construct Y* = yhat0 + e* and recompute LRT statistic by refitting null and alt to Y*.</li>
 * </ul>
 * This is typically far better calibrated than permuting X-within-strata or permuting Y directly when Z is continuous.
 * </p>
 *
 * <p>
 * Discrete Y branch keeps stratified permutation of Y within strata(Z) (Freedman–Lane is less straightforward there).
 * </p>
 *
 * <p>
 ...
 * </p>
 *
 * <p><b>Defaults (hard-coded, recommended starting point):</b></p>
 * <ul>
 *   <li>rffFeatures=256, rffSigma=1.0, rffSeed=1</li>
 *   <li>ridge=1e-2</li>
 *   <li>nu=5.0, scale=1.0</li>
 *   <li>binsPerContZ=10, minStratumSize=8</li>
 *   <li>permutations=1000, permSeed=1</li>
 *   <li>irlsIters=8, irlsTol=1e-6</li>
 * </ul>
 */
public final class MinimaxTRffCITest4 implements IndependenceTest, RowsSettable {
    ...
    // ---------------- knobs (defaults hard-coded) ----------------
    private double alpha;

    // permutation
    private int permutations = 1000;
    private long permSeed = 1L;

    // stratification on Z
    private int binsPerContZ = 10;
    private int minStratumSize = 8;

    // model knobs
    private double ridge = 1e-2;
    ...
    public MinimaxTRffCITest4(DataSet data, double alpha) {
    ...
    /**
     * Core test.
     * - Discrete Y: stratified permutation of Y within strata(Z) (as in v3)
     * - Continuous Y: Freedman–Lane residual permutation within strata(Z) (new in v4)
     */
    public double getPValue(Node x, Node y, Set<Node> z) {
        ...
        int B = Math.max(200, permutations);

        // ---------------- DISCRETE Y: stratified permutation of Y ----------------
        if (isDiscrete(iy)) {
        ...
        // ---------------- CONTINUOUS Y: Freedman–Lane residual permutation ----------------
        ...
        FitT fit0 = fitStudentTRidge(yObs, dmNull.Phi);
        FitT fit1 = fitStudentTRidge(yObs, dmAlt.Phi);
        ...
        for (int i = 0; i < n; i++) resid[i] = yObs[i] - fit0.yhat[i];

        int ge = 0;
        int valid = 0;

        // scratch
        double[] residPerm = new double[n];
        double[] yPerm = new double[n];

        for (int b = 0; b < B; b++) {
            System.arraycopy(resid, 0, residPerm, 0, n);
            shuffleWithinStrataDouble(residPerm, strata, rng);

            // y* = yhat0 + e*
            for (int i = 0; i < n; i++) yPerm[i] = fit0.yhat[i] + residPerm[i];

            FitT pfit0 = fitStudentTRidge(yPerm, dmNull.Phi);
            FitT pfit1 = fitStudentTRidge(yPerm, dmAlt.Phi);
            if (!Double.isFinite(pfit0.ll) || !Double.isFinite(pfit1.ll)) continue;

            double t = 2.0 * (pfit1.ll - pfit0.ll);
            if (!Double.isFinite(t)) continue;
    ...
    private static final class FitT {
        final double ll;
        final double[] yhat;
        FitT(double ll, double[] yhat) {
            this.ll = ll;
            this.yhat = yhat;
        }
    }

    private FitT fitStudentTRidge(double[] yCentered, double[][] Phi) {
        if (!(ridge > 0) || !Double.isFinite(ridge)) return new FitT(Double.NaN, new double[yCentered.length]);
        ...
        for (int iter = 0; iter < irlsIters; iter++) {
            ...
            if (!chol.decompose(G)) return new FitT(Double.NaN, new double[n]);
    ...
    // =========================================================
    // Permutation helpers: shuffle within each stratum
    // =========================================================

    private static void shuffleWithinStrataDouble(double[] a, int[][] strata, SplittableRandom rng) {
        for (int[] g : strata) {
    ...
    private static void shuffleWithinStrataInt(int[] a, int[][] strata, SplittableRandom rng) {
        for (int[] g : strata) {
            for (int i = g.length - 1; i > 0; i--) {
                int j = rng.nextInt(i + 1);
                int ii = g[i], jj = g[j];
    ...
    public String toString() {
        return "Minimax-t RFF CI test (mixed) v4 (Freedman–Lane for continuous)";
    }
