    ...
    //    private final Graph cpdag;          // may actually be a PAG too
    //    private final boolean dag;
        private final boolean pag;          // <--- new

        public PagIda(DataSet dataSet, Graph graph, List<Node> possibleCauses) {
            if (dataSet == null) {
        ...
        public static boolean isRefinementOf(Graph coarse, Graph fine) {
            ...
            // Optional extra: insist that fine is itself a legal PAG.
            // if (!fine.paths().isLegalPag()) return false;
        ...
        /**
         * Original IDA / Optimal IDA for DAG/CPDAG (unchanged).
         */
        ...
        /**
         * PAG-based IDA:
         * For each local refinement g' of the PAG around X that:
         *   (i)  refines the original PAG,
         *   (ii) is a legal PAG,
         *   (iii) makes (X,Y) amenable in the Perković sense,
         * we compute an adjustment set via RecursiveAdjustment / GAC on g'
         * and regress Y on X ∪ Z to get one total effect.
         *
        ...
        private LinkedList<Double> getTotalEffectsPagInternal(Node x, Node y) {
            ...
            if (orientable.isEmpty()) {
                dfsOrientAndCollect(base, x, y, orientable, 0, totalEffects);
                Collections.sort(totalEffects);
                return totalEffects;
            }
        ...
        /**
         * Depth-first search over all local orientation choices of edges X o-* W in "orientable".
         * At the leaf, applies FCI final rules and, if the resulting graph passes consistency tests,
         * runs RecursiveAdjustment and collects one beta.
         */
        private void dfsOrientAndCollect(Graph current,
                                         Node x,
                                         Node y,
...
//                }
                ...
                //   - otherwise use PAG semantics.
                String graphTypeForAmenability;
                if (gPrime.paths().isLegalDag() || gPrime.paths().isLegalPdag()) {
                    graphTypeForAmenability = "PDAG";
                } else {
                    graphTypeForAmenability = "PAG";
                }
            ...
            tryOrientation(current, x, other,
                    oldAtX, oldAtOther,
        ...
        /**
         * Try one concrete orientation of the edge between X and "other", if it refines the original endpoints.
         * If acceptable, recurse to the next edge.
         */
        private void tryOrientation(Graph current,
                                    Node x,
                                    Node other,
                                    Endpoint oldAtX,
                                    Endpoint oldAtOther,
            ...
            dfsOrientAndCollect(gNext, x, y, orientable, index + 1, totalEffects);
        }

        /**
         * Given a refinement gPrime that:
         *   - refines the original graph,
         *   - is legal (PAG or PDAG/DAG),
         *   - and makes (X,Y) amenable under the chosen semantics,
         * run RA to get an adjustment set Z, regress Y on X ∪ Z, and append the resulting beta.
         */
        private void collectEffectsFromRefinement(Graph gPrime,
                                                  Node x,
                                                  Node y,
...
//
//                LinkedList<Double> effects = getTotalEffects(x, y);
//                double minEffect = effects.isEmpty() ? 0.0 : effects.getFirst();
//                minEffects.put(x, minEffect);
//            }
//
//            return minEffects;
//        }
