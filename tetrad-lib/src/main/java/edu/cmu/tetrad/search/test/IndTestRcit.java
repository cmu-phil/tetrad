 ...
 *   <li>Bandwidths (σ) via median pairwise Euclidean distance using the first min(n,500) rows</li>
 *   <li>Random Fourier Features (RFF): feat = sqrt(2) * cos(W X^T + b), W ~ N(0, 1/σ), b ~ U[0, 2π]</li>
 *   <li>Stat: n * || Cxy - Cxz Czz^{-1} Czy ||_F^2 (RIT if Z empty)</li>
 *   <li>Null: Gamma (Satterthwaite–Welch) with moments from Cov(res_x ⊙ res_y),
 *       or permutation if approx=PERMUTATION</li>
 * </ul>
 *
 * Parameters supported via setters (see below) or via constructor(…, Parameters) legacy keys:
 * <ul>
 *   <li>num features: X/Y (numFeatXY), Z (numFeatZ)</li>
 *   <li>approx: 1=LPB4, 2=HBE, 3=GAMMA, 4=CHI2, 5=PERMUTATION (internally only GAMMA or PERMUTATION are used)</li>
...
public final class IndTestRcit implements IndependenceTest {
     ...
     * Typical Tetrad flow will configure via setters from the wrapper.
     */
    public IndTestRcit(DataSet dataSet, Parameters params) {
        ...
        // If someone passed a ridge as a legacy tweak:
    ...
    /** 1=LPB4, 2=HBE, 3=GAMMA, 4=CHI2, 5=PERMUTATION (internally: PERMUTATION vs GAMMA fallback) */
    ...
    /** Whether to standardize (z-score) the RFF features prior to covariance. */
    ...
    public IndependenceResult checkIndependence(Node x, Node y, Set<Node> z) throws InterruptedException {
        ...
        if (fZ == null || fZ.getNumCols() == 0) {
            ...
            if (approx == Approx.PERMUTATION && permutations > 0) {
                ...
                // Gamma approx from eigenvalues of Cov(resX ⊙ resY)
                SimpleMatrix resX = fX.minus(colMeanRow(fX));
                SimpleMatrix resY = fY.minus(colMeanRow(fY));
                p = gammaApproxP(stat, kronResCov(resX, resY));
            }
        } else {
            // RCIT proper (with Z)
    ...
    private static SimpleMatrix rff(SimpleMatrix X, int numF, double sigma, Random rng) {
        ...
        // W: numF x d with std = 1/sigma
        SimpleMatrix W = new SimpleMatrix(numF, d);
        double sd = 1.0 / sigma;
        for (int i = 0; i < numF; i++) for (int j = 0; j < d; j++) W.set(i, j, rng.nextGaussian() * sd);

        // b: numF x 1 uniform [0, 2π], then add per column later
        double twoPi = 2.0 * Math.PI;
        double[] b = new double[numF];
        for (int i = 0; i < numF; i++) b[i] = rng.nextDouble() * twoPi;

        // feat = sqrt(2) * cos(W X^T + b)
        SimpleMatrix Xt = X.transpose();           // d x n
        SimpleMatrix WX = W.mult(Xt);              // numF x n
        for (int i = 0; i < numF; i++) {
            for (int j = 0; j < n; j++) WX.set(i, j, WX.get(i, j) + b[i]);
        }
        SimpleMatrix feat = new SimpleMatrix(n, numF);
        double scale = Math.sqrt(2.0);
        for (int i = 0; i < numF; i++) {
            for (int j = 0; j < n; j++) feat.set(j, i, scale * Math.cos(WX.get(i, j)));
        }
        return feat;
    }
    ...
    /** Gamma (Satterthwaite–Welch) p-value from eigenvalues of Cov. */
    private static double gammaApproxP(double stat, SimpleMatrix Cov) {
        double sum = 0.0, sum2 = 0.0;
        var evd = Cov.eig();
        int m = evd.getNumberOfEigenvalues();
        for (int i = 0; i < m; i++) {
            double lam = evd.getEigenvalue(i).getReal();
            if (lam > 0 && Double.isFinite(lam)) { sum += lam; sum2 += lam * lam; }
        }
        if (sum <= 0 || sum2 <= 0) return stat <= 1e-12 ? 1.0 : 0.0;
        double mu = sum, var = 2.0 * sum2;
        double k = (mu * mu) / var;      // shape
        double theta = var / mu;         // scale
        if (!Double.isFinite(k) || !Double.isFinite(theta) || k <= 0 || theta <= 0) {
            return stat <= 1e-12 ? 1.0 : 0.0;
        }
