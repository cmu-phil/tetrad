 ...
 */
public class TrekSeparationClustersOld {
    /**
     ...
     */
    public TrekSeparationClustersOld(List<Node> variables, CovarianceMatrix cov, int sampleSize) {
    ...
    private Set<Set<Integer>> findClustersAtRank(List<Integer> vars, int size, int rank) {
        ...
        return LongStream.range(0, total).parallel().mapToObj(m -> {
            ...
            return cluster;
        }).filter(Objects::nonNull).collect(Collectors.toCollection(ConcurrentHashMap::newKeySet));
    }
