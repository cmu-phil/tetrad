...
/**
 * <b>KEEP-KNOWLEDGE VARIANT.</b> This class is the knowledge-orientation-keeping variant of FcitSl,
 * parallel to StarFciKeepKnowledgeOrientations and FcitKeepKnowledgeOrientations. Unlike Fcit, FcitSl
 * already KEEPS background-knowledge orientations by construction: the final orientation stage's ruleR0
 * applies fciOrientbk internally (the FciOrient pulls knowledge from the strategy delegate) and nothing
 * after it re-canonicalizes the graph. Changes from FcitSl (2026-8) are therefore certification only:
 * with nonempty knowledge, the final graph is checked against the modulo-knowledge legality certificate
 * (equality with the knowledge-refined canonical PAG of its implied Zhang MAG); on failure the final
 * stage is recomputed without knowledge, and if that knowledge-free rendering is a strictly legal PAG it
 * is returned instead with a loud log (knowledge never degrades legality); if neither certifies, the
 * knowledge-refined graph is kept, matching FcitSl's current uncertified behavior. With empty knowledge
 * this class behaves exactly like FcitSl. The interim MAG-level class machinery deliberately remains
 * knowledge-neutral (class identity is canonical); knowledge enters only through the seed dagToPag and
 * the final stage. That scoping is a deliberate, contestable modeling choice.
 * <p>
 * FCIT-SL ("Step Lemma"): FCIT-ZM extended with a Step-Lemma-guided representative search.
 *
 * @author josephramsey
 */
public final class FcitSlKeepKnowledgeOrientations implements IGraphSearch {
    /**
     ...
     */
    public FcitSlKeepKnowledgeOrientations(IndependenceTest test, Score score) {
        if (test == null) {
    ...
    public Graph search() throws InterruptedException {
        ...
        // KEEP-KNOWLEDGE VARIANT: ruleR0 above applied fciOrientbk internally (the FciOrient
        // pulled the knowledge from the strategy delegate), so finalPag already carries the
        // background-knowledge orientations -- FcitSl keeps them by construction. What this
        // variant ADDS is the certificate and the "knowledge never degrades legality"
        // fallback: with nonempty knowledge, finalPag is checked against the modulo-knowledge
        // certificate (equal to the canonical PAG of its implied Zhang MAG after that
        // canonical PAG is knowledge-refined and re-closed). If it fails, the same final
        // stage is recomputed with EMPTY knowledge; if that canonical rendering passes the
        // STRICT certificate, it is returned instead, with a loud log -- the knowledge/data
        // conflict is resolved in favor of legality, exactly as in
        // StarFciKeepKnowledgeOrientations. If neither certifies, finalPag is kept, which is
        // FcitSl's current (uncertified) behavior; with empty knowledge nothing here runs and
        // this class behaves exactly like FcitSl.
        if (knowledge != null && !knowledge.isEmpty()) {
            // FcitSl carries no selection-node field; collect them from the graph as Fcit does.
            Set<Node> selectionNodes = new LinkedHashSet<>();
            for (Node node : finalPag.getNodes()) {
                if (node.getNodeType() == NodeType.SELECTION) {
                    selectionNodes.add(node);
                }
            }

            PagLegalityCheck.LegalPagRet moduloRet = legalPagModuloKnowledge(finalPag,
                    new HashSet<>(selectionNodes), fciOrient);

            if (!moduloRet.isLegalPag()) {
                TetradLogger.getInstance().log("Keep-knowledge certificate FAILED on the final graph ("
                        + moduloRet.getReason() + "); recomputing the final orientation without "
                        + "knowledge to check for a knowledge/data conflict.");

                Graph canonicalPag = interimPags.getLast().copy();
                ...
                if (PagLegalityCheck.isLegalPag(canonicalPag, new HashSet<>(selectionNodes)).isLegalPag()) {
                    ...
                    TetradLogger.getInstance().log("Neither the knowledge-refined nor the knowledge-free "
                            + "final graph certifies; keeping the knowledge-refined graph (FcitSl's "
                            + "uncertified behavior).");
                }
            } else {
                TetradLogger.getInstance().log("Keep-knowledge certificate PASSED: the final graph is a "
                        + "legal PAG refined by background knowledge.");
            }
        }
...
//        System.out.println("MAGs to return = " + out);
     ...
     */
    /**
     * The legality certificate of this variant: legal PAG *modulo background knowledge*, identical in design to
     * StarFciKeepKnowledgeOrientations#legalPagModuloKnowledge and FcitKeepKnowledgeOrientations. When knowledge is
     * empty it delegates to {@link PagLegalityCheck#isLegalPag(Graph, Set)}. Otherwise: all nodes must be measured;
     * the implied Zhang MAG must exist and be a legal MAG; and the candidate must equal the canonical PAG of that
     * MAG after the canonical PAG is re-refined with fciOrientbk and re-closed under the final rules by
     * {@link #refineWithKnowledge} using the supplied orientation engine (the final-stage engine, so R0/R4
     * adjudication is reproduced from the same recorded evidence). Every mark in an accepted graph is thereby
     * either invariant in the equivalence class or forced by knowledge; graphs that are "between a MAG and a PAG"
     * for any other reason (e.g., non-invariant evidence-backed colliders) fail, which is why the caller treats a
     * failure as a signal to compare against the knowledge-free rendering rather than as an error.
     */
    ...
    /**
     * Refines a graph with background knowledge and closes under the final rules using the supplied engine:
     * fciOrientbk for the required/forbidden-edge marks, then the one-argument finalOrientation (matching the
     * final-stage call above). Works on a copy; identity when knowledge is empty.
     */
    private Graph refineWithKnowledge(Graph graph, FciOrient orient) {
        if (knowledge == null || knowledge.isEmpty()) {
            return graph;
        }
