...
public final class GesMulti implements GraphSearch, GraphScorer {
    ...
    private double medSize;

    //===========================CONSTRUCTORS=============================//

    public GesMulti(Map<Node, List<Node>> nodeMap, DataSet dataSet) {
        this.nodeMap = nodeMap;

        this.variables = new ArrayList<Node>(nodeMap.keySet());

        this.varNames = new String[variables.size()];
        for (int i = 0; i < variables.size(); i++) {
            this.varNames[i] = variables.get(i).getName();
        }

        this.dataVars = dataSet.getVariables();

        setDataSet(dataSet);

        int sum = 0;
        int count = 0;
        double[] sizes = new double[nodeMap.keySet().size()];

        for (Node node : nodeMap.keySet()) {
            sum += nodeMap.get(node).size();
            sizes[count] = nodeMap.get(node).size();
            count++;
        }

        medSize = sum / (double) count;
        medSize = StatUtils.median(sizes);
    }

    public GesMulti(Map<Node, List<Node>> nodeMap, ICovarianceMatrix covMatrix) {
        this.nodeMap = nodeMap;
        ...
        this.dataVars = covMatrix.getVariables();
...
//        this.variables = covarianceMatrix.getVariables();
    ...
    private double localSemScore(int i, int[] parents) {
        ...
        List<Node> y = nodeMap.get(variables.get(i));

        List<Node> x = new ArrayList<Node>();

        for (int p2 : parents) {
            x.addAll(nodeMap.get(variables.get(p2)));
        }
        ...
        try {
            TetradMatrix Cxx = subMatrix(dataVars, cov, x, x);
            TetradMatrix Cxy = subMatrix(dataVars, cov, x, y);

            TetradMatrix V = Cxx.inverse().times(Cxy);

            TetradVector W = new TetradVector(y.size());

            for (int v = 0; v < y.size(); v++) {
                W.set(v, V.getColumn(v).dotProduct(Cxy.getColumn(v)));
            }
...
//        int k = ((p + 1) * (p + 2)) / 2;
        ...
        try {
            List<Node> allVars = new ArrayList<Node>();
            allVars.addAll(x);
            allVars.addAll(y);

            TetradMatrix Caa = subMatrix(dataVars, cov, allVars, allVars);
            TetradMatrix Cay = subMatrix(dataVars, cov, allVars, y);

            TetradMatrix V = Caa.inverse().times(Cay);

            TetradVector W = new TetradVector(y.size());

            for (int v = 0; v < y.size(); v++) {
                W.set(v, V.getColumn(v).dotProduct(Cay.getColumn(v)));
            }
