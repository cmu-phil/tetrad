 ...
 * </ul>
 */
public final class MinimaxCITest1 implements IndependenceTest, RowsSettable {
    ...
    public MinimaxCITest1(DataSet data, double alpha) {
    ...
    // =========================================================
// Continuous-continuous case: HSIC (RBF) within strata
// =========================================================
    ...
    private double pValueContContOrig(int ix, int iy,
        ...
        // Prebuild per-stratum HSIC plans: precompute centered kernel for X,
        // and fix sigmaX/sigmaY using median heuristic on observed data.
        HsicPlan[] plans = buildHsicPlans(xArr, yArr, strata);
        if (plans.length == 0) return 0.0;

        // observed: quantile aggregation across strata
        double obs = hsicPlansQuantile(plans, yArr, qMinimax);
        if (!Double.isFinite(obs)) return 1.0;
        ...
        for (int b = 0; b < BB; b++) {
            ...
            // shuffle Y within each stratum (same as before)
            for (int[] g : strata) {
            ...
            double t = hsicPlansQuantile(plans, yPerm, qMinimax);
...
//    }
    ...
    // -------------------- HSIC helpers (RBF kernel, centered) --------------------

    /**
     * Per-stratum HSIC plan: g is indices in "useRows-space".
     * KxC is the *centered* RBF kernel matrix for X within this stratum (fixed across perms).
     * sigmaY is fixed (median heuristic) so the permutation null keeps the same scaling.
     */
    private static final class HsicPlan {
        final int[] g;
        final double[][] KxC;
        final double sigmaY;

        HsicPlan(int[] g, double[][] KxC, double sigmaY) {
            this.g = g;
            this.KxC = KxC;
            this.sigmaY = sigmaY;
        }
    ...
    private HsicPlan[] buildHsicPlans(double[] xArr, double[] yArr, int[][] strata) {
        ArrayList<HsicPlan> out = new ArrayList<>(strata.length);

        for (int[] g : strata) {
            int m = g.length;
            if (m < minStratumSize) continue;
            if (m < 4) continue; // HSIC is noisy for tiny groups

            double sigmaX = medianHeuristicSigma(xArr, g);
            double sigmaY = medianHeuristicSigma(yArr, g);

            // If sigma degenerates (all equal), skip stratum (it can't show dependence anyway)
            if (!(sigmaX > 0) || !Double.isFinite(sigmaX)) continue;
            if (!(sigmaY > 0) || !Double.isFinite(sigmaY)) continue;

            double[][] Kx = rbfKernel1D(xArr, g, sigmaX);
            double[][] KxC = centerKernelInPlace(Kx);

            out.add(new HsicPlan(g, KxC, sigmaY));
        }

        return out.toArray(new HsicPlan[0]);
    }

    /**
     * Quantile aggregation of within-stratum HSIC stats. Uses your QuickSelect-based quantile().
     */
    private double hsicPlansQuantile(HsicPlan[] plans, double[] y, double q) {
        double[] stats = new double[plans.length];
        int k = 0;

        for (HsicPlan p : plans) {
            double t = hsicStatForPlan(p, y);
            if (Double.isFinite(t)) stats[k++] = t;
        }

        if (k == 0) return Double.NaN;
        if (k < stats.length) stats = Arrays.copyOf(stats, k);

        return quantile(stats, q);
    }

    /**
     * HSIC statistic for one plan:
     *   HSIC = (1/(m-1)^2) * sum_{ij} KxC_ij * KyC_ij
     * where KyC is centered RBF kernel on Y within the same group.
     */
    private double hsicStatForPlan(HsicPlan plan, double[] y) {
        int[] g = plan.g;
        int m = g.length;
        if (m < 4) return Double.NaN;

        double[][] Ky = rbfKernel1D(y, g, plan.sigmaY);
        double[][] KyC = centerKernelInPlace(Ky);

        double s = 0.0;
        for (int i = 0; i < m; i++) {
            double[] kxi = plan.KxC[i];
            double[] kyi = KyC[i];
            for (int j = 0; j < m; j++) {
                s += kxi[j] * kyi[j];
            }
    ...
    /**
     * RBF kernel matrix on 1D array values restricted to indices g (indices refer to useRows-space).
     */
    private static double[][] rbfKernel1D(double[] v, int[] g, double sigma) {
        int m = g.length;
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0; j < i; j++) {
                double d = vi - v[g[j]];
                double kij = Math.exp(-(d * d) * inv2s2);
                K[i][j] = kij;
                K[j][i] = kij;
            }
        }
    ...
    /**
     * Center a symmetric kernel matrix in place:
     *   Kc = H K H, H = I - (1/m)11^T
     * Returns the same matrix reference for convenience.
     */
    private static double[][] centerKernelInPlace(double[][] K) {
        int m = K.length;
        if (m == 0) return K;

        double[] rowMean = new double[m];
        double grand = 0.0;

        for (int i = 0; i < m; i++) {
            double s = 0.0;
            double[] Ki = K[i];
            for (int j = 0; j < m; j++) s += Ki[j];
            rowMean[i] = s / m;
            grand += rowMean[i];
        }
        grand /= m;
    ...
    /**
     * Median heuristic for 1D RBF sigma on values in group g:
     * sigma := sqrt( median_{i<j} ( (v_i - v_j)^2 ) / 2 )
     *
     * Uses exact O(m^2) distances for m<=300; otherwise samples up to ~4000 pairs.
     */
    private static double medianHeuristicSigma(double[] v, int[] g) {
        int m = g.length;
        if (m < 2) return Double.NaN;

        // Exact for small strata
        if (m <= 300) {
            int M = m * (m - 1) / 2;
            ...
            double med = quickSelect(d2, 0, d2.length - 1, d2.length / 2);
            if (!(med > 0) || !Double.isFinite(med)) return Double.NaN;
            return Math.sqrt(med / 2.0);
        }

        // Sample pairs for larger strata
        int maxPairs = 4000;
        int pairs = Math.min(maxPairs, m * 20); // ~O(m) samples
        double[] d2 = new double[pairs];

        SplittableRandom rng = new SplittableRandom(1234567L ^ m);
        for (int t = 0; t < pairs; t++) {
            int i = rng.nextInt(m);
            int j = rng.nextInt(m - 1);
            if (j >= i) j++;
            double di = v[g[i]] - v[g[j]];
            d2[t] = di * di;
        }
