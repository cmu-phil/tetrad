...
public class MeekRules implements ImpliedOrientation {
    ...
    private boolean meekR2(Node a, Node c, Graph graph, Set<Node> visited) {
        ...
        for (Node b : common) {
            ...
            if (graph.getPaths().isDirectedFromTo(c, b) && graph.getPaths().isDirectedFromTo(b, a)) {
    ...
    private boolean r3Helper(Node a, Node d, Node b, Node c, Graph graph, Set<Node> visited) {
        ...
        boolean b4 = graph.getPaths().isUndirectedFromTo(d, a);
        boolean b5 = graph.getPaths().isUndirectedFromTo(d, b);
        boolean b6 = graph.getPaths().isUndirectedFromTo(d, c);
        boolean b7 = graph.getPaths().isDirectedFromTo(b, a);
        boolean b8 = graph.getPaths().isDirectedFromTo(c, a);
...
//        }
        ...
        if (aggressivelyPreventCycles && graph.getPaths().existsDirectedPathFromTo(c, a)) {
            graph.addEdge(before);
            return false;
        }
