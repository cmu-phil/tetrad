...
public class ConditionalGaussianOtherLikelihood {
    ...
    public ConditionalGaussianOtherLikelihood(DataSet dataSet) {
        ...
        mixedVariables = dataSet.getVariables();
        ...
        adTree = AdTrees.getAdLeafTree(this.dataSet);
    ...
    private DataSet useErsatzVariables() {
        ...
        for (int j = 0; j < mixedVariables.size(); j++) {
            if (mixedVariables.get(j) instanceof DiscreteVariable) {
                ...
                Discretizer.Discretization d = discretize(column, breakpoints, mixedVariables.get(j).getName(), categoryNames);
    ...
    private Ret likelihoodJoint(List<ContinuousVariable> X, List<DiscreteVariable> A, Node target) {
        ...
        if (target instanceof DiscreteVariable) {
            for (ContinuousVariable x : new ArrayList<>(X)) {
                Node variable = dataSet.getVariable(x.getName());
        ...
        for (List<Integer> cell : cells) {
            ...
            if (A.size() > 0) {
                c1 += a * this.multinomialLikelihood(a, N);
            }
            ...
            if (X.size() > 0) {
                try {
                    ...
                    if (a > continuousCols.length + 10) {
                        Matrix cov = this.cov(this.getSubsample(continuousCols, cell));
                        c2 += a * this.gaussianLikelihood(k, cov);
                    } else {
                        Matrix cov = this.cov(this.getSubsample(continuousCols, all));
                        c2 += a * this.gaussianLikelihood(k, cov);
                    }
                } catch (Exception e) {
                    // No contribution.
    ...
    private double gaussianLikelihood(int k, Matrix sigma) {
        return -0.5 * this.logdet(sigma) - 0.5 * k - 0.5 * k * LOGMATH2PI;
    }
            ...
            for (int u = 0; u < x.size(); u++) {
                for (int i = 0; i < x.get(u).rows(); i++) {
                    for (int v = 0; v < x.size(); v++) {
                        ...
                        a[v] = this.prob(factors[v], inv.get(v), xm);
                    }
    ...
    private Ret likelihoodJointMultinomial(List<ContinuousVariable> X, List<DiscreteVariable> A) {
        ...
        for (List<Integer> cell : cells) {
            ...
            if (W.size() > 0) {
                lnL += a * this.multinomialLikelihood(a, N);
            }
        }
