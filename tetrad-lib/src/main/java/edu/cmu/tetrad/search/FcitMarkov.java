...
/**
 * A Markov-preserving variant of {@link Fcit}. It is the same search as Fcit, except that every committed edge removal
 * (both the single-edge and the multi-edge path) is passed through an additional <em>Markov-preservation gate</em> after
 * the legal-PAG check. A move is committed only if it introduces no spurious m-separation; otherwise it is reverted
 * exactly as an illegal-PAG move would be.
 * <p>
 * Unlike {@link FcitMagMarkov}, which is MAG-based and already has a MAG in hand at the commit step, Fcit carries the PAG
 * directly. The gate therefore judges each accepted move on the Zhang MAGs of the before/after PAGs
 * ({@code zhangMagFromPag(_pag)} and {@code zhangMagFromPag(this.pag)}). A PAG represents a Markov-equivalence class of
 * MAGs that all share the same m-separation relation, so the Zhang MAG's m-separations are exactly those entailed by the
 * PAG; checking Markovness on it is equivalent to checking it on the PAG. The gate code itself
 * ({@link #markovPreserved}, {@link #markovPillow}, {@link #districtWithin}, {@link #directedTopologicalOrder}) is
 * identical to FcitMagMarkov's.
 * <p>
 * <b>What it guarantees.</b> Run the chain of PAGs as an induction. If the base PAG ({@code dagToPag} of the BOSS DAG)
 * is Markov to the data, then each committed move preserves Markovness, so every PAG in the chain is Markov. The gate
 * enforces the exact condition: a move from a Markov MAG {@code M} to {@code M'} keeps Markovness iff every m-separation
 * entailed by {@code M'} but not by {@code M} is a real conditional independence in the data. The legal-PAG check alone
 * does not give this (legality is not Markovness), and neither does the single tested fact x &perp; y | S, because
 * removing x&ndash;y and re-orienting can entail further separations for other pairs.
 * <p>
 ...
 * induction hypothesis). Only the genuinely new constraints cost a data test.
 * <p>
 * <b>Assumptions / caveats.</b> (i) The base PAG is Markov &mdash; true at the population level if BOSS lands a Markov
 * DAG, only approximately at finite n. (ii) A consistent CI test; with finite data the &ldquo;guarantee&rdquo; is really
 * &ldquo;no move introduces a test-detected spurious separation.&rdquo; (iii) The distribution is a compositional
 * graphoid (licenses ordered-local &hArr; global). (iv) The Markov-pillow construction is written for MAGs over directed
 * (&rarr;) and bidirected (&harr;) edges, i.e. the {@code excludeSelectionBias} regime; selection-bias MAGs with
 * undirected edges would need the extended pillow.
 *
 * @author josephramsey
 */
public final class FcitMarkov implements IGraphSearch {
    /**
     ...
     */
    public FcitMarkov(IndependenceTest test, Score score) {
        if (test == null) {
...
// Sound positive verdicts only. A committed separator means the pair was
    ...
    private boolean tryToModifyGraph(Node x, Node y, Set<Node> b, String type, boolean excludeSelectionBias, Set<Triple> initialColliders) {
        ...
        // Markov gate: the move keeps Markovness iff it introduces no spurious
        // m-separation. Judge it on the Zhang MAGs of the before/after PAGs — both are
        // legal PAGs here, so zhangMagFromPag is well defined, and a MAG carries exactly
        // the PAG's m-separations. Same revert path as an illegal PAG.
        Graph magPrev = GraphTransforms.zhangMagFromPag(_pag);
        Graph magNew = GraphTransforms.zhangMagFromPag(this.pag);

        boolean markovOk;
        try {
            markovOk = markovPreserved(magPrev, magNew);
        } catch (InterruptedException ie) {
            Thread.currentThread().interrupt();
            throw new RuntimeException(ie);
        }
    ...
    // ===================================================================================
    // Markov-preservation gate. Identical to FcitMagMarkov's; see this class's Javadoc
    // for how the before/after MAGs are sourced from the PAG flow.
    //
