...
public class PurifyTetradBased implements IPurify {
    ...
    public List<List<Node>> purify(List<List<Node>> clustering) {
        ...
        List<int[]> _clustering2 = this.purify2(_clustering);
    ...
    private List tetradBasedPurify(List clustering) {
        ...
        for (Iterator it = clustering.iterator(); it.hasNext(); ) {
            this.intraConstructPhase2((int[]) it.next(), eliminated);
        }
        this.printlnMessage();
    ...
    private void intraConstructPhase2(int[] _cluster, boolean[] eliminated) {
        ...
        if (doFdr) {
            List<Double> allPValues = new ArrayList<>(this.listPValues(cluster, eliminated, Double.MAX_VALUE));
        ...
        List<Double> pValues2 = this.listPValues(cluster, eliminated, cutoff);
...
//                System.out.println("Dropped " + tetradTest.getVarNames()[index]);
    ...
    private void crossConstructPhase2(List<int[]> clustering, boolean[] eliminated) {
        ...
        if (doFdr) {
            List<Double> allPValues = new ArrayList<>(this.listCrossConstructPValues(clustering, eliminated, Double.MAX_VALUE));
        ...
        List<Double> pValues = this.listCrossConstructPValues(clustering, eliminated, cutoff);
                ...
                if (cluster1.length >= 3 && cluster2.length >= 1) {
                    ...
                    while ((choice1 = gen1.next()) != null) {
                        ...
                        while ((choice2 = gen2.next()) != null) {
                            ...
                            List<Double> pValues = this.listPValues(crossCluster, eliminated, cutoff);
                ...
                if (cluster1.length >= 2 && cluster2.length >= 2) {
                    ...
                    while ((choice1 = gen1.next()) != null) {
                        ...
                        while ((choice2 = gen2.next()) != null) {
                            ...
                            List<Double> pValues = this.listPValues2by2(crossCluster, eliminated, cutoff);
