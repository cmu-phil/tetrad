 ...
 * @author josephramsey
 */
public final class FcitMag2 implements IGraphSearch {
    /**
     ...
     */
    public FcitMag2(IndependenceTest test, Score score) {
        if (test == null) {
...
//                }
    ...
    private boolean tryToModifyGraph(Node x, Node y, Set<Node> b, String type,
        ...
        orientSepsetCollidersInMagRobust(_mag, sepsets);
...
//        // Commit-step Markov guard: legality does not test the I-map property, so a
//        // legal MAG can still carry an unsound collider whose apex sits on a confirmed
//        // separator. Reject (revert) on the first such collider.
//        Triple unsound = firstUnsoundColliderInMag(_mag);
//        if (unsound != null) {
//            if (verbose) {
//                TetradLogger.getInstance().log("\tTried removing " + _edge
//                        + ", legal MAG but unsound collider " + unsound
//                        + " (apex on a confirmed separator); reverted.");
//            }
//            this.pag = _pag;
//            sepsets.set(x, y, prevSepset);
//            return false;
...
//        return true;
        ...
        // Commit-step Markov guard: legality does not test the I-map property, so a legal
        // MAG can still carry an unsound collider whose apex sits on a confirmed separator.
        // Scan the COMMITTED graph (round-tripped through MagToPag), not the intermediate
        // stamped _mag: the offending R4/completion collider may exist only after the round
        // trip, while _mag carries only the recorded-sepset (R0) stamps. Build the committed
        // PAG, take its MAG, and guard that. Revert on the first unsound apex.
        Graph committedPag = new MagToPag(_mag).convert(false, excludeSelectionBias);
        Graph committedMag = GraphTransforms.zhangMagFromPag(committedPag);

        Triple unsound = firstUnsoundColliderInMag(committedMag);
        if (unsound != null) {
            if (verbose) {
                TetradLogger.getInstance().log("\tTried removing " + _edge
                        + ", legal MAG but unsound collider " + unsound
        ...
        this.pag = committedPag;
        return true;
    }

    private void orientSepsetCollidersInMagRobust(Graph mag, SepsetMap sepsets)
            throws InterruptedException {
        ...
        for (Set<Node> pair : sepsets.keySet()) {
            ...
            for (Node c : common) {
                ...
                if (hasConfirmedSeparatorContaining(mag, x, y, c)) {
                    continue; // ambiguous/noncollider-compatible; abstain
                }
...
//    private boolean hasConfirmedSeparatorContaining(Graph graph, Node x, Node y, Node c)
//            throws InterruptedException {
//
...
//        return test.checkIndependence(x, y, z).isIndependent();
//    }

    // Is there a confirmed separator of (x,y) that CONTAINS c? Equivalently: can c be a
    // non-collider on a real separator, making the stamped x*->c<-*y arrowheads unsound?
    //
    // RB is asked the UNFORCED question only, to harvest the path-relevant neighborhood;
    // forcing c in here would mislead RB because c is a collider in `graph` (forcing a
    // collider in makes the path look open, so RB returns no block -- the under-firing
    // failure mode). The apex is forced in only at the test below. The pool is RB's
    // blocking set UNION adj(x) UNION adj(y): local (degree-bounded), not n-bounded.
    //
    // NOTE ON SCALE: per accepted commit this runs once per unshielded collider, each at
    // cost C(|pool|, <=cap). |pool| is local degree, but a dense intermediate MAG can make
    // this expensive; see the depth cap below. cap follows `depth` (-1 => |pool|).
    private boolean hasConfirmedSeparatorContaining(Graph graph, Node x, Node y, Node c)
            throws InterruptedException {
        ...
        RecursiveBlocking.BlockingResult result =
                RecursiveBlocking.blockPathsRecursively(
                        graph, x, y,
                        Set.of(),       // UNFORCED: harvest the pool, do not mislead RB
                        Set.of(),
                        recursiveDepth, depth, rbRadius, 1, true,
                        timeout < 0 ? Long.MAX_VALUE : System.currentTimeMillis() + timeout);

        Set<Node> pool = new LinkedHashSet<>();
        if (result != null && !result.indeterminate() && result.blockingSet() != null) {
            pool.addAll(result.blockingSet());
        }
        pool.addAll(graph.getAdjacentNodes(x));
        pool.addAll(graph.getAdjacentNodes(y));
        pool.remove(x);
        pool.remove(y);
        pool.remove(c);                 // c is added unconditionally below

        List<Node> poolList = new ArrayList<>(pool);
        int cap = (depth < 0) ? poolList.size() : Math.min(depth, poolList.size());
        ...
        for (int k = 0; k <= cap; k++) {
            ...
            while ((choice = gen.next()) != null) {
                ...
                if (System.currentTimeMillis() > deadline) return false;  // budget: treat as "not confirmed"
                Set<Node> z = new HashSet<>(GraphUtils.asSet(choice, poolList));
                z.add(c);
                if (test.checkIndependence(x, y, z).isIndependent()) return true;
            }
        }
        return false;
    }
    ...
    /**
     * Commit-step Markov guard. Scans every unshielded definite collider x*->c<-*y
     * in the candidate MAG (x, y non-adjacent) and asks whether (x,y) has a
     * test-confirmed separator that CONTAINS the apex c. If one does, c is a valid
     * non-collider on a real separator, so the stamped arrowheads are unsound and
     * the collider blocks a path that should stay active -- committing the removal
     * would drop a true dependency (leave the I-map class) on legality alone.
     *
     * Generalizes findR0CollidersWithSeparableLeg / the robust sepset abstention
     * from recorded-sepset (R0) colliders to ALL output colliders, so it also
     * covers the R4/completion channel that owns the commit-step breaks. The
     * criterion is apex-containment, NOT leg-separability: an R4 collider with both
     * legs real in G* is invisible to the leg test but flagged here whenever a
     * separator through its apex exists.
     *
     * Reverting on a hit keeps x-y (a denser graph), which can only add
     * dependencies, never manufacture a false independence -- so the guard is
     * I-map-conservative: it converts a soundness failure into a (recoverable)
     * stall, feeding the same deadlock/MAG-sweep rescue path as the other stalls.
     *
     * @return the first unsound collider, or null if every unshielded collider's
     * apex is excluded by every confirmed separator.
     */
    private Triple firstUnsoundColliderInMag(Graph mag) throws InterruptedException {
        Set<Triple> seen = new LinkedHashSet<>();
        for (Node c : mag.getNodes()) {
            ...
            for (int i = 0; i < adj.size(); i++) {
                for (int j = i + 1; j < adj.size(); j++) {
                    Node x = adj.get(i);
                    Node y = adj.get(j);
                    if (mag.isAdjacentTo(x, y)) continue;         // unshielded only
                    if (!mag.isDefCollider(x, c, y)) continue;    // collider x*->c<-*y
                    Triple t = new Triple(x, c, y);
                    if (!seen.add(t)) continue;
                    if (hasConfirmedSeparatorContaining(mag, x, y, c)) {
                        return t;                                 // unsound apex
                    }
                }
            }
        }
    ...
    private boolean tryToModifyGraph(List<Edge> edges, String type,
        ...
        // Commit-step Markov guard (see single-edge tryToModifyGraph).
        Triple unsound = firstUnsoundColliderInMag(_mag);
        if (unsound != null) {
            if (verbose) {
                TetradLogger.getInstance().log("\tTried removing " + edges
                        + " (multi-edge), legal MAG but unsound collider " + unsound
                        + " (apex on a confirmed separator); reverted.");
            }
            this.pag = _pag;
            prev.forEach((e, s) -> sepsets.set(e.getNode1(), e.getNode2(), s));
