 ...
 * A linear non-Gaussian acyclic model for causal discovery. Journal of Machine Learning Research, 7(10).</p>
 *
 * <p>The focus for this implementation was to make a version of ICA-LiNGAM that
 * would be compatible with LiNG-D (see). There are two parameters, one to choose whether an acyclic result will be
 * guaranteed, and another to set a threshold on the lower bound of the absolute value of the coefficients in the B Hat
 * matrix. The latter is used to find edges in the final graph.</p>
 *
 * <p>ICA-LiNGAM is a method for estimating a causal graph from a dataset. It is based on
 * the assumption that the data are generated by a linear model with non-Gaussian noise. The method is based on the
 ...
 * zero, which allows for certain cyclic structures to be recovered.</p>
 *
