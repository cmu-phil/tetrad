...
public class PcStableMaxLocal implements GraphSearch {
    ...
    public Graph search() {
        ...
        this.addColliders(graph, sepsetProducer, knowledge);
    ...
    private void tryRemovingEdge(Node x, Node y) {
        ...
        if (this.sepset(x, y) != null) {
            ...
            for (Node w : graph.getAdjacentNodes(x)) {
                this.tryAddingEdge(w, x);
            }

            for (Node w : graph.getAdjacentNodes(y)) {
                this.tryAddingEdge(w, y);
            }
        }
    }
...
//                if (z == w) continue;
    ...
    private void addColliders(Graph graph, SepsetProducer sepsetProducer, IKnowledge knowledge) {
        Map<Triple, Double> collidersPs = this.findCollidersUsingSepsets(sepsetProducer, graph, false);
...
//            if (!graph.getEdge(a, b).pointsTowards(a) && !graph.getEdge(b, c).pointsTowards(c)) {
    ...
    public Map<Triple, Double> findCollidersUsingSepsets(SepsetProducer sepsetProducer, Graph graph, boolean verbose) {
        ...
        for (Node b : nodes) {
            this.findColliders(sepsetProducer, graph, verbose, colliders, b);
        }
