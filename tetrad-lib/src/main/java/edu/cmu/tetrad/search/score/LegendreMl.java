 ...
 * </ul>
 */
public final class LegendreMl implements Score, EffectiveSampleSizeSettable {
     ...
     */
    public LegendreMl(DataSet dataSet) {
...
//                long seed = mix64(((long) i) ^ 0x9E3779B97F4A7C15L);
    ...
    private static double toMinusOneOne(double z) {
        // z is typically z-scored. Map to (-1, 1) smoothly.
        // The 1.5 scaling controls how quickly it saturates.
        if (!Double.isFinite(z)) return 0.0;
        double x = Math.tanh(z / 1.5);
        // keep away from endpoints for numerical stability
