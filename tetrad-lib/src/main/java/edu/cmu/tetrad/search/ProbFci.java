...
public final class ProbFci implements GraphSearch {
    ...
    public ProbFci(IndependenceTest independenceTest) {
        ...
        corr = new CorrelationMatrix(independenceTest.getCov());
    }
...
//        adj.setFci(true);
        ...
        if (!RFCI_Used) {
            ...
            if (this.isPossibleDsepSearchDone()) {
                ...
                possibleDSep.setMaxPathLength(this.getMaxReachablePathLength());
...
//    }
    ...
    private void ruleR0() {
        ...
        for (Node b : nodes) {
            ...
            while ((combination = cg.next()) != null) {
                ...
                List<Node> sepset2 = sepsets.get(a, c);
    ...
    private void doFinalOrientation_RFCI() {
        ...
        while (changeFlag) {
            ...
            this.rulesR1R2cycle();
            this.ruleR3();
            this.ruleR4();   // some changes to the original R4 inline
        }
    ...
    private void doFinalOrientation() {
        ...
        while (changeFlag) {
            ...
            this.rulesR1R2cycle();
            this.ruleR3();
            this.ruleR4();   //slowest, so do last
        }
...
//    }
        ...
        for (Node B : nodes) {
            ...
            for (Node D : intoBCircles) {
                ...
                while ((choice = gen.next()) != null) {
                    ...
                    if (!this.isArrowpointAllowed(D, B)) {
                        continue;
                    }
        ...
        while (reachable.size() > 0) {
            ...
            for (Node d : pathExtensions) {
                ...
                if (!graph.isAdjacentTo(d, c)) {
                    ...
                    {
                        next.put(d, x);
                        this.doDdpOrientation_RFCI(d, a, b, c, next);
                    } else  // non-RFCI
                    {
    ...
    private void doDdpOrientation(Node d, Node a, Node b, Node c) {
        List<Node> sepset = sepsets.get(d, c);
...
//        }
        ...
        while (r != k) {
            ...
            {
                List<Node> sepset1 = sepsets.get(i, k);
                ...
                for (int setSize = 0; setSize <= sepSet2.size(); setSize++) {
                    ...
                    while ((combination = cg.next()) != null) {
                        ...
                        if (indep) {
                            this.getSepsets().set(r, q, condSet);
                            ...
                            this.ruleR0_RFCI(rTuples);   // Algorithm 4.4 (Colombo et al, 2012)
        ...
        if (sepset.contains(j)) {
            ...
            if (!this.isArrowpointAllowed(j, k)) {
                return;
            }
            ...
            if (!this.isArrowpointAllowed(l, j) || !this.isArrowpointAllowed(j, l)
                    || !this.isArrowpointAllowed(j, k) || !this.isArrowpointAllowed(k, j)) {
                return;
            }
