            ...
            for (Graph inGraph : this.input) {
                for (Edge edge : inGraph.getEdges()) {
                    ...
                    if (Edges.isDirectedEdge(edge)) {
                        if (!pag.paths().existsPotentiallyDirectedPath(node1, Collections.singleton(node2))) {
                            allAccountFor = false;
                            break GRAPH;
                        }
                    }
                    if (/*!pag.existsTrek(node1, node2) ||*/ Edges.isPartiallyOrientedEdge(edge)) {
                        if (pag.paths().existsPotentiallyDirectedPath(node2, Collections.singleton(node1))) {
                            allAccountFor = false;
                            break GRAPH;
                        }
                    }
