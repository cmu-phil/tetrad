/// ////////////////////////////////////////////////////////////////////////////
// PhantomKernelEnumerator14.java  (parallel, PAG-space, end-to-end FcitZm)  //
//                                                                             //
// pke14 = pke12's QUESTION and POPULATION, with FcitZm under test.                            //
//                                                                             //
// PKE8 asked the single thing that matters for correctness -- does the actual //
// shipping FcitZm, from its own GRaSP start, driven by the m-separation       //
...
// PKE8's question: FcitZm sees only the oracle, and the oracle is a function  //
// of the true PAG's Markov class.  So pke14 enumerates distinct true PAGs a   //
// la PKE10 and runs PKE8's end-to-end check once per PAG:                     //
//                                                                             //
//     terminal PAG returned by FcitZm.search()  ==  G* (the true PAG)?        //
//                                                                             //
...
// is exactly PKE10's analyze() convention).  FcitZm is constructed with the   //
...
// to FcitZm, since G* and the search must operate under the same assumption.  //
...
//                         over-deletion.                                      //
//   ERROR              : FcitZm threw or timed out.                           //
...
// string over all OBS! relabellings -- so FcitZm runs once per isomorphism      //
// class (PAG up to relabelling), not once per labelling.  This is the lever     //
// that cuts the FcitZm call count (up to OBS!-fold in the limit, less in        //
...
// specific representative that was analyzed.  FcitZm's verdict is relabelling-   //
...
// analyze, pke14 runs FcitZm on each distinct PAG the instant it is first      //
...
//       [4]=violation log path (default pke14_violations.log)                 //
//       [5]=magspace checkpoint path (default pke14_magspace_obs<OBS>_        //
//           checkpoint.txt) -- delete to restart enumeration from scratch     //
//       [6]=magspace distinct-class keys path (default pke14_magspace_obs<OBS>_//
...
//       [7]=FcitZm commit route (MAG|PAG, default MAG = FCIT-ZM proper).        //
//           Checkpoint/keys defaults do NOT include the route, so give distinct //
//           paths for [5]/[6] when running both routes, or the second run will  //
//           resume from the first one's checkpoint and analyze nothing.         //
//                                                                             //
...
/**
 * pke14: enumerate DISTINCT TRUE PAGs (magspace or dagsweep, as in PKE10) and, once per
 * PAG, run the real FcitZm from a GRaSP start, oracle-driven, reporting every PAG whose
 * terminal graph is not exactly G* (PKE8's end-to-end check over PKE10's population).
 */
public final class PhantomKernelEnumerator14 {

    /**
     * Default constructor.
     */
    public PhantomKernelEnumerator14() {
    }

    // ────────────────────────────────────────────────────────────────────────
    // CONFIGURATION (enumeration knobs via args, FcitZm knobs hard-coded)
    // ────────────────────────────────────────────────────────────────────────
    ...
    // ── FcitZm configuration (verbatim PKE8; match the shipping defaults to certify) ──
    /** Per-run FcitZm timeout, ms; -1 = unlimited.  A positive value turns a
     *  hang into an ERROR verdict instead of stalling the whole run. */
    private static final long FCITZM_TIMEOUT_MS = -1L;
    /** Depth / recursive-depth passed to FcitZm (-1 = unlimited). */
    private static final int DEPTH = -1;
    private static final int RECURSIVE_DEPTH = -1;
    /**
     * Which FcitZm commit route to certify. MAG is FCIT-ZM proper (project back through the
     * canonical Zhang MAG, the route the reachability paper describes); PAG reorients the PAG
     * directly. The legality gate is identical either way, so a route difference in the
     * verdict is a fact about the projection, not the gate -- run both.
     */
    private static FcitZm.COMMIT_ROUTE COMMIT_ROUTE = FcitZm.COMMIT_ROUTE.MAG;

    /** Applied consistently to the enumeration projections AND FcitZm (see header). */
    ...
    public static void main(String[] args) throws IOException {
        ...
        String violationLogPath = (args.length > 4) ? args[4] : "pke14_violations.log";
        if (args.length > 7) COMMIT_ROUTE = FcitZm.COMMIT_ROUTE.valueOf(args[7].toUpperCase());
        ...
        String checkpointPath = (args.length > 5) ? args[5]
                : "pke14_magspace_obs" + OBS + "_checkpoint.txt";
        String keysPath = (args.length > 6) ? args[6]
                : "pke14_magspace_obs" + OBS + "_keys.txt";

        CANON = new ArrayList<>();
        for (int i = 0; i < OBS; i++) CANON.add(new GraphNode("V" + (i + 1)));

        CONFIG_LINE = String.format(
                "# pke14 config: mode=%s N=%d latent=%d observed=%d "
        ...
        String header = CONFIG_LINE + "\n# run started " + new Date()
                + "\n# VIOLATIONS: every distinct true PAG whose terminal FcitZm PAG (GRaSP start,"
                + "\n# oracle-driven) is NOT exactly G*.  Analysis is INLINE: each new distinct"
                + "\n# PAG is run through FcitZm the moment it is found, and any violation below"
        ...
        // FcitZm runs on each distinct PAG as it is discovered; `total` accumulates the
    ...
    // ────────────────────────────────────────────────────────────────────────
    // ENUMERATION + INLINE ANALYSIS: FcitZm runs on each distinct PAG as found
    // ────────────────────────────────────────────────────────────────────────
     ...
     * time it is seen is immediately run through FcitZm (inline analysis).
     *
        ...
        LongStream.range(0, totalMasks).parallel().forEach(mask -> {
            ...
            while ((latChoice = latGen.next()) != null) {
                ...
                try {
                    ...
                    // Dedup on the canonical (isomorph-invariant) key: one FcitZm run per
     ...
     * PAG (won the {@code seen.putIfAbsent} race), FcitZm runs on it and any violation
    ...
    private static long enumerateAndAnalyzeMagSpace(Result total, String checkpointPath,
        ...
        // The commit route is part of the config: a checkpoint/keys pair written under one
        // route must not silently resume under the other, since the verdicts differ.
        String enumConfig = String.format(
                "# pke14 magspace-enum config: OBS=%d blockSize=%d route=%s",
                OBS, MAGSPACE_BLOCK_SIZE, COMMIT_ROUTE);

        Set<Long> doneBlocks = loadCheckpoint(checkpointPath, enumConfig);
        // Dedup set keyed by CANONICAL (isomorph-invariant) key: FcitZm runs once per
        ...
        for (long oi = 0; oi < numBlocks; oi++) {
            ...
            LongStream.range(lo, hi).parallel().forEach(code -> {
                try {
                    ...
                    // relabellings), so isomorphic PAGs collapse to one FcitZm run.
     ...
     *  the dedup key, so FcitZm runs once per class up to relabelling. Cost is one OBS!
     *  minimization per surviving candidate -- cheaper than the FcitZm run it saves. */
    ...
    // ────────────────────────────────────────────────────────────────────────
    // INLINE ANALYSIS: PKE8's end-to-end FcitZm check, run the instant a PAG is found
    // ────────────────────────────────────────────────────────────────────────

    /** Run FcitZm on one just-discovered distinct true PAG and record the verdict.
    ...
    private static void analyzeInline(Result r, Graph trueMag, Graph truePag) {
        ...
        try {
            ...
            // colliders, handing FcitZm an oracle from a DIFFERENT class and
            ...
            runFcitZm(r, modelId, trueMag, truePag, CANON);
        } catch (Exception ex) {
            r.skipped.increment();
    ...
    private static void runFcitZm(Result r, String modelId, Graph trueMag, Graph truePag,
                                List<Node> obs) {
        // Oracle drive: MsepTest on the true MAG as both test and score basis.  FcitZm's
        // constructor flips startWith to GRASP when the test is an MsepTest, so this is the
        // Oracle GRaSP path.  A fresh graph per FcitZm (it mutates node types internally).
        ...
        Graph terminal;
        try {
            FcitZm fcit = new FcitZm(test, scoreObj);
            ...
            fcit.setTimeout(FCITZM_TIMEOUT_MS);
            fcit.setUseMarkFlipEscalation(true);
            fcit.setCommitRoute(COMMIT_ROUTE);
            ...
            if (ERR_PRINTS.incrementAndGet() <= 10) {
                System.err.println("FcitZm error " + modelId + ": " + ex);
            }
            return;
        }

        // Normalize terminal to the canonical observed nodes (FcitZm returns a PAG over
    ...
    private static String violationEntry(String bucket, String modelId, Graph trueMag,
        ...
        if (terminal != null) {
            sb.append("  terminal PAG from FcitZm:\n").append(terminal).append('\n');
        }
        sb.append(detail);
        sb.append("==== end entry ====\n");
        return sb.toString();
    ...
    private static String summarize(Result t, long elapsedMs, String violationLogPath) {
        ...
        sb.append("---- TERMINAL-PAG IDENTITY (FcitZm from GRaSP start, oracle) ----\n");
        sb.append(String.format("EXACT (terminal PAG == G*)         : %d%n", exact));
        sb.append(String.format("VIOLATIONS (terminal PAG != G*)    : %d%s%n", violations,
                violations == 0 ? "   *** clean: FcitZm recovers G* on every distinct PAG ***" : ""));
