...
/**
 * <p><b>FF-DML: Feature-Function Distributional Marginal Likelihood score (GP form, continuous)</b></p>
 *
 * <p>A distributional generalization of FF-ML. FF-ML scores one GP regression of the conditional
 * mean of {@code Y} on {@code Pa(Y)}, so its bump only rewards parents that improve the conditional
 * mean — blind, like a covariance/Fisher-Z score, to parents that move only the conditional variance,
 * skew, or tails. FF-DML instead sums GP log marginal likelihoods over a small battery of orthogonal
 * response channels {@code He_k(Y)/sqrt(k!)} (and optionally normal-scored ranks), all sharing the
 * same parent features. The feature Gram and its Cholesky are computed once and reused across
 * channels, so R channels cost about the same as one FF-ML evaluation when the feature count
 * dominates. Selecting only {@code Channel.MEAN} recovers FF-ML bit-for-bit.</p>
 *
 * <p>
 ...
 * </ul>
 */
public final class FfCordContinuous implements Score, EffectiveSampleSizeSettable {
...
// We later scale by wStd = sqrt(2/bw2Child).
    ...
    // -------------------- distributional (multi-channel) configuration --------------------
    /**
     * Response channels summed into the local score. Each channel is an orthogonal transform of the
     * (already globally z-scored) child column, regressed on the SAME parent features. MEAN reproduces
     * plain FF-ML exactly; the higher channels make the score sensitive to conditional variance / skew /
     * tail dependence that a mean-only score (like FF-ML or Fisher Z) cannot see. Default {MEAN, VAR}.
     */
    private volatile Channel[] channels = {Channel.MEAN, Channel.VAR};
    /**
     * Per-channel weights, aligned with {@link #channels}. Higher moments carry more sampling variance,
     * so down-weighting them (e.g. skew/kurt at 0.5) is often sensible. Default {1.0, 1.0}.
     */
    private volatile double[] channelWeights = {1.0, 1.0};
    // Cached per-child normal-scored ranks (for the RANK channel), lazily filled; NaN preserved.
    private final AtomicReference<ConcurrentHashMap<Integer, double[]>> rankColCacheRef =
            new AtomicReference<>(new ConcurrentHashMap<>());
    /**
     * If true, the no-parent baseline fixes sigma^2=lambda exactly as FF-ML does, so that
     * {@code channels={MEAN}} reproduces FF-ML bit-for-bit on every parent set including the empty one.
     * This reintroduces the baseline mis-calibration for higher-moment channels, so it is intended only
     * for exact FF-ML A/B reproduction with MEAN alone. Default false (profiled, calibrated baseline).
     */
    private volatile boolean exactFfmlBaseline = false;
     ...
     */
    public FfCordContinuous(DataSet dataSet) {
    ...
    /**
     * No-parent baseline with sigma^2 profiled to its MLE (sigma^2 = g'g/n), i.e. the properly
     * normalized marginal likelihood of a zero-feature model. This is the correct baseline for the
     * with-parents path, which also profiles sigma^2: using it makes the edge bump measure genuine
     * conditional structure rather than a mismatch between the response's marginal variance and lambda.
     * FF-ML fixes sigma^2=lambda here, which is only calibrated because Y is z-scored (variance ~1);
     * the higher-moment channels have marginal variance far from 1, so fixing sigma^2 there produces
     * large spurious bumps for any parent, including independent ones.
     */
    private double gpLogMarginalLikelihoodSigmaOnlyProfiled(double[] gCentered) {
        int n = gCentered.length;
        if (n == 0) return Double.NaN;
        double gTg = 0.0;
        for (double v : gCentered) gTg += v * v;
        double s2 = TMath.max(1e-10, gTg / n);
        // -0.5 (g'g/s2 + n log s2) = -0.5 n (1 + log s2); 2*pi constant dropped as in FF-ML.
        return -0.5 * (gTg / s2 + n * TMath.log(s2));
    }
    ...
    public double localScore(int i, int... parents) {
        ...
        return cache.computeIfAbsent(key, k -> {
            try {
                ...
                final Channel[] chans = this.channels;
                final double[] w = this.channelWeights;
                if (chans.length == 0) return Double.NaN;

                double sigma2 = lambda;
                if (!(sigma2 > 0) || !Double.isFinite(sigma2)) return Double.NaN;

                // One (centered) response vector per channel, built from the child column.
                // MEAN == extract1D(i)+center == exactly the FF-ML response, so channels={MEAN}
                // reproduces FF-ML bit-for-bit.
                double[][] g = new double[chans.length][];
                for (int c = 0; c < chans.length; c++) {
                    g[c] = channelVector(chans[c], i, rows, n);
                }

                // Exact closed-form for no parents: C = sigma2 I, summed over channels.
                if (parents.length == 0) {
                    double s = 0.0;
                    for (int c = 0; c < chans.length; c++) {
                        double ll = exactFfmlBaseline
                                ? gpLogMarginalLikelihoodSigmaOnly(g[c], sigma2)   // fixed sigma2=lambda (FF-ML)
                                : gpLogMarginalLikelihoodSigmaOnlyProfiled(g[c]);  // profiled sigma2 (calibrated)
                        if (Double.isNaN(ll)) return Double.NaN;
                        s += w[c] * ll;
                    }
                    return s;
                }
                ...
                // G = Phi^T Phi and the Cholesky are shared across channels; only v_c = Phi^T g_c
                // and the profiled sigma2_c differ. So R channels cost ~1x FF-ML when m dominates.
                return gpLogMarginalLikelihoodRFF_profiledNestedMulti(
                        g, w, parents, rows, n, numFeatures,
    ...
    /**
     * Returns the active response channels.
     *
     * @return a copy of the channel array.
     */
    ...
    /**
     * Sets the response channels summed into each local score. Duplicates are removed, order preserved.
     * This resets the per-channel weights to all 1.0, so call {@link #setChannelWeights} afterward if you
     * want non-unit weights. {@code setChannels(Channel.MEAN)} reduces the score to plain FF-ML.
     *
     * @param channels one or more channels; must be non-empty.
     * @throws IllegalArgumentException if {@code channels} is null or empty.
     */
    public void setChannels(Channel... channels) {
        if (channels == null || channels.length == 0) {
            throw new IllegalArgumentException("channels must be non-empty");
        }
        java.util.LinkedHashSet<Channel> set = new java.util.LinkedHashSet<>(Arrays.asList(channels));
        Channel[] cs = set.toArray(new Channel[0]);
        double[] w = new double[cs.length];
        Arrays.fill(w, 1.0);
        this.channels = cs;
        this.channelWeights = w;
        resetCache();
    }

    /**
     * Returns the per-channel weights.
     *
     * @return a copy of the weight array.
     */
    ...
    /**
     * Sets the per-channel weights. The length must match the current number of channels (set channels
     * first). Weights must be finite and nonnegative.
     *
     * @param weights per-channel weights, aligned with {@link #getChannels()}.
     * @throws IllegalArgumentException if the length is wrong or any weight is negative/non-finite.
     */
    public void setChannelWeights(double... weights) {
        if (weights == null || weights.length != channels.length) {
            throw new IllegalArgumentException("weights length (" + (weights == null ? 0 : weights.length)
                    + ") must match number of channels (" + channels.length + ")");
        }
    ...
    /**
     * Returns whether the exact FF-ML (fixed-sigma^2) no-parent baseline is in effect.
     *
     * @return true if the fixed-sigma^2 baseline is used.
     */
    public boolean isExactFfmlBaseline() {
        return exactFfmlBaseline;
    }

    /**
     * Sets whether to use FF-ML's fixed-sigma^2 no-parent baseline. Enable only for exact FF-ML
     * reproduction with {@code channels={MEAN}}; leave false (default) for calibrated bumps with any
     * higher-moment channel.
     *
     * @param exactFfmlBaseline true to fix sigma^2=lambda at the baseline.
     */
    public void setExactFfmlBaseline(boolean exactFfmlBaseline) {
        this.exactFfmlBaseline = exactFfmlBaseline;
        resetCache();
    }
...
//  - includes a local traceInvFromCholeskyLower(L) helper below (exact trace(B^{-1})).
    ...
    /**
     * Multi-channel variant of {@link #gpLogMarginalLikelihoodRFF_profiledNested}. Accumulates the
     * shared feature Gram {@code G = Phi^T Phi} and each channel's {@code v_c = Phi^T g_c} and
     * {@code g_c^T g_c} in a single pass, then profiles sigma^2 and evaluates the GP log marginal
     * likelihood per channel, returning the weighted sum. With a single MEAN channel this is
     * numerically identical to the single-response method (same accumulation order, same routines).
     *
     * @param gChannels {@code [R][n]}, each channel response already centered.
     * @param weights   {@code [R]} per-channel weights.
     * @return the weighted sum of per-channel GP log marginal likelihoods, or NaN on failure.
     */
    private double gpLogMarginalLikelihoodRFF_profiledNestedMulti(
            double[][] gChannels,
            double[] weights,
            int[] parentIdx,
            int[] rows,
        ...
        final DMatrixRMaj G = new DMatrixRMaj(mFeatures, mFeatures);
        final double[][] v = new double[R][mFeatures];
        final double[] yTy = new double[R];
        final double[] phi = new double[mFeatures];
        ...
        for (int ii = 0; ii < n; ii++) {
            ...
            for (int c = 0; c < R; c++) {
                final double gi = gChannels[c][ii];
                yTy[c] += gi * gi;
                final double[] vc = v[c];
                for (int k = 0; k < mFeatures; k++) vc[k] += phi[k] * gi;
            }
        ...
        double total = 0.0;
        for (int c = 0; c < R; c++) {
            double sigma2Hat = profileSigma2Secant(sigma2Init, n, mFeatures, yTy[c], G, v[c]);
            if (!(sigma2Hat > 0) || !Double.isFinite(sigma2Hat)) return Double.NaN;
            SigmaEval eval = evalSigma2(sigma2Hat, n, mFeatures, yTy[c], G, v[c]);
            if (eval == null) return Double.NaN;
            total += weights[c] * eval.logLik;
        }
        return total;
    }

    // -------------------- channel (distributional) machinery --------------------

    private static final double SQRT2 = Math.sqrt(2.0);
    private static final double SQRT6 = Math.sqrt(6.0);
    private static final double SQRT24 = Math.sqrt(24.0);

    /**
     * Builds the centered response vector for one channel from the (globally z-scored) child column.
     * Moment channels apply the normalized probabilists' Hermite polynomial He_k(Y)/sqrt(k!) — these
     * are marginally orthonormal under Y~N(0,1), so their evidences add cleanly, and He_1(Y)=Y makes
     * MEAN identical to the FF-ML response. RANK uses normal-scored ranks of the child column.
     */
    private double[] channelVector(Channel ch, int child, int[] rows, int n) {
        if (ch == Channel.RANK) {
            double[] rc = getRankColumn(child);   // full-length normal scores, NaN preserved
            double[] out = new double[n];
            if (rows == null) {
                for (int r = 0; r < n; r++) out[r] = rc[r];
            } else {
                for (int r = 0; r < n; r++) out[r] = rc[rows[r]];
            }
            centerInPlace(out);
            return out;
        }

        double[] s = extract1D(child, rows, n);   // already z-scored child values at these rows
        double[] out = new double[n];
        switch (ch) {
            case MEAN -> System.arraycopy(s, 0, out, 0, n);                          // He1(s) = s
            case VAR -> {
                for (int r = 0; r < n; r++) { double x = s[r]; out[r] = (x * x - 1.0) / SQRT2; }      // He2/sqrt(2!)
            }
            case SKEW -> {
                for (int r = 0; r < n; r++) { double x = s[r]; out[r] = (x * x * x - 3.0 * x) / SQRT6; } // He3/sqrt(3!)
            }
            case KURT -> {
                for (int r = 0; r < n; r++) {
                    double x = s[r];
                    out[r] = (x * x * x * x - 6.0 * x * x + 3.0) / SQRT24;            // He4/sqrt(4!)
                }
            }
            default -> throw new IllegalStateException("Unhandled channel: " + ch);
        }
        centerInPlace(out);
        return out;
    }
    ...
    /**
     * Normal-scored ranks of a child column: rank the non-NaN values, map rank i (1-based) to
     * {@code Phi^{-1}(i/(cnt+1))}. Computed once per child (ranks are a stable global transform of the
     * variable, like the global z-scoring FF-ML already does) and cached. Ties break by row order.
     */
    private double[] getRankColumn(int child) {
        return rankColCacheRef.get().computeIfAbsent(child, c -> {
            double[] col = zCols[c];
            int N = col.length;
            int cnt = 0;
            for (double val : col) if (!Double.isNaN(val)) cnt++;

            Integer[] idx = new Integer[cnt];
            int t = 0;
            for (int r = 0; r < N; r++) if (!Double.isNaN(col[r])) idx[t++] = r;
            Arrays.sort(idx, (p, q) -> Double.compare(col[p], col[q]));  // stable: ties keep row order

            double[] out = new double[N];
            Arrays.fill(out, Double.NaN);
            for (int rank = 0; rank < cnt; rank++) {
                double u = (rank + 1.0) / (cnt + 1.0);
                out[idx[rank]] = invNormCdf(u);
            }
            return out;
        });
    }
    ...
    private static double invNormCdf(double p) {
        ...
        final double[] dd = {7.784695709041462e-03, 3.224671290700398e-01, 2.445134137142996e+00,
                3.754408661907416e+00};
        final double plow = 0.02425, phigh = 1.0 - 0.02425;
        double q, r, x;
        if (p < plow) {
    ...
    /**
     * Response channels for the distributional score. Each is an orthogonal transform of the (globally
     * z-scored) child column; the local score is the weighted sum of per-channel GP log marginal
     * likelihoods against the shared parent features. MEAN alone reproduces FF-ML; adding higher
     * channels buys sensitivity to conditional variance / skew / tail dependence.
     */
    public enum Channel {

        /** He1(Y) = Y: conditional-mean channel. Reproduces FF-ML exactly. */
        MEAN,

        /** He2(Y)/sqrt(2!) = (Y^2-1)/sqrt(2): conditional-variance / co-volatility channel. */
        VAR,

        /** He3(Y)/sqrt(3!) = (Y^3-3Y)/sqrt(6): conditional-skew channel. */
        SKEW,

        /** He4(Y)/sqrt(4!) = (Y^4-6Y^2+3)/sqrt(24): conditional-kurtosis / tail channel. */
        KURT,

        /** Normal-scored ranks of Y: rank-robust omnibus channel (CORD-style). */
        RANK
    }
}