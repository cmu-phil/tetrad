...
//    private Algorithm algorithm = Algorithm.SAG;
    ...
    public Graph search() {
        ...
        if (algorithm == FindOneFactorClusters.Algorithm.GAP) {
            clusters = estimateClustersGAP(); // Triples first.
        } else if (algorithm == FindOneFactorClusters.Algorithm.SAG) {
    ...
    private Set<Set<Integer>> estimateClustersGAP() {
        Set<Set<Integer>> threeClusters = findThreeClusters();
        return combineThreeClusters(threeClusters);
...
//            System.out.println("...done.");
        ...
        while ((choice = gen.next()) != null) {
            ...
            for (int o : allVariables) {
                ...
                Set<Integer> cluster = quartet(n1, n2, n3, o);

                if (!(quartetVanishes(cluster))) {
                    continue CHOICE;
                }

//                if (!(avgSumLnP(cluster) > -20)) {
//                    continue CHOICE;
//                }
            }
    ...
    private Set<Set<Integer>> combineThreeClusters(Set<Set<Integer>> threeClusters) {
        ...
        if (algType == AlgType.lax) {
            ...
            int count = 0;
            int total = threeClusters.size();

            do {
                if (!threeClusters.iterator().hasNext()) {
                    break;
                }

                Set<Integer> cluster = threeClusters.iterator().next();
...
//                    }
        ...
        if (algType == AlgType.laxWithSpeedup) {
            ...
            for (Set<Integer> cluster : new HashSet<Set<Integer>>(threeClusters)) {
                Set<Integer> _cluster = new HashSet<Integer>(cluster);
...
//                    System.out.println("Adding " + o  + " to " + cluster);
                ...
                for (Set<Integer> c : new HashSet<Set<Integer>>(threeClusters)) {
                    if (_cluster.containsAll(c)) {
                        threeClusters.remove(c);
                    }
                }
