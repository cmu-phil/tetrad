...
/**
 * Converts a MAG (Directed acyclic graph) into the PAG (partial ancestral graph) which it is in the equivalence class
 * of.
 *
 * @author josephramsey
 * @author peterspirtes
...
//            if (!edge.pointsTowards(x)) continue;
    ...
    public Graph convert() {
        if (history.get(mag) != null) return history.get(mag);
        ...
        orientUnshieldedColliders(graph, this.mag);
    ...
    private Graph calcAdjacencyGraph() {
        List<Node> allNodes = this.mag.getNodes();
        ...
        for (int i = 0; i < measured.size(); i++) {
            for (int j = i + 1; j < measured.size(); j++) {
                ...
                List<Node> inducingPath = this.mag.paths().getInducingPath(n1, n2);
    ...
    private boolean foundCollider(Graph dag, Node a, Node b, Node c) {
        boolean ipba = MagToPag.existsInducingPathInto(b, a, dag);
        boolean ipbc = MagToPag.existsInducingPathInto(b, c, dag);
