 ...
 */
public final class Fges3 implements GraphSearch, GraphScorer {
     ...
     */
    public Fges3(Score score, int parallelism) {
        if (score == null) {
...
// as the "bump".
    ...
    private Set<Node> getNaYX(Node x, Node y) {
        ...
        List<Node> a = graph.getAdjacentNodes(x);
        a.retainAll(graph.getAdjacentNodes(y));

        List<Node> c = graph.getChildren(x);
        c.retainAll(graph.getChildren(y));
