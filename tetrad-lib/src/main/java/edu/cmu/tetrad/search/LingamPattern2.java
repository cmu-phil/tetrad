...
//            System.out.println(getDataModel().getCauseNode(j) + ": " + nf.format(p));
//        }
...
//
//        for (int j = 0; j < getDataModel().getNumColumns(); j++) {
//            TetradLogger.getInstance().log("normalityTests", getDataModel().getCauseNode(j) + ": " + nf.format(scores.get(maxj).pvals[j]));
//        }
