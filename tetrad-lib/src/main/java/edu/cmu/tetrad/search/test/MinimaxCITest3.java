 ...
 * </ul>
 */
public final class MinimaxCITest3 implements IndependenceTest, RowsSettable {
     ...
     */
    public MinimaxCITest3(DataSet data, double alpha) {
    ...
    private double pValueMixedGTest(int ix, int iy,
        ...
        if (verbose) {
            TetradLogger.getInstance().log(
                    "MinimaxCITest mixed: strata=" + strata.length +
                            " plans=" + plans.length +
                            " qMinimax=" + qMinimax +
                            " perms=" + Math.max(50, B)
            );
...
//        return (ge + 1.0) / (valid + 1.0);
    ...
    /**
     * If rng != null, for each plan we shuffle Y categories within the stratum before computing.
     */
...
//    }
     ...
     * (This matches pValueContContOrig, which shuffles Y within strata.)
     */
    private static final class GroupPlan {
        final int[] xObs;     // observed codes 0..Kx-1
        final int[] yObs;     // observed codes 0..Ky-1
        final int Kx, Ky;
        ...
        double statistic(SplittableRandom rng) {
            ...
            // 1) choose Y codes: observed or permuted (within this group only)
            if (rng == null) {
            ...
            for (int i = 0; i < n; i++) {
                int xi = xObs[i];
                int yi = yPerm[i];
                // defensively clamp if ever needed (shouldn’t happen)
