...
//        poolSet.retainAll(reachY);
        ...
        pool.sort(Comparator
                .comparingInt((Node v) -> {
                    ...
                    return TMath.min(endpointDistance(v, shellsFromX), endpointDistance(v, shellsFromY));
                })
...
//        }
    ...
    private int approxEndpointDistance(Node v, Node X, Node Y, Shells sx, Shells sy) {
        ...
        for (int r = 1; r < sx.layers.length; r++) if (sx.layers[r].contains(v)) best = TMath.min(best, r);
        for (int r = 1; r < sy.layers.length; r++) if (sy.layers[r].contains(v)) best = TMath.min(best, r);
