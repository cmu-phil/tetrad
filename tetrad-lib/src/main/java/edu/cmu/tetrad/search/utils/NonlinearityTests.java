...
// ----- helpers for intercept + skipping intercept in z-score -----
    ...
    /**
     * Additive-noise diagnostic: fit an additive mean model y ~ sum_j f_j(x_j),
     * compute residuals ehat = y - ghat(X), then test whether ehat is independent of X.
     *
     * Implementation: approximate HSIC using Random Fourier Features (RBF kernels) and
     * compute a permutation p-value by shuffling residual features across rows.
     *
     * reject=true  => evidence that residual depends on X (violates additive-noise / independent noise)
     * reject=false => no evidence of residual dependence on X (consistent with additive noise)
     */
    public static TestResult additiveNoiseIndependenceTest(double[] y, double[][] X, double alpha) {
        if (y == null || X == null || y.length == 0 || X.length != y.length) {
            return new TestResult(Double.NaN, Double.NaN, false);
        }
        int n = y.length;
        int d = X[0].length;
        if (n < 40 || d == 0) return new TestResult(Double.NaN, Double.NaN, false);

        // ---- knobs (UI-friendly defaults) ----
        ...
        final int rffE = 80;    // features for residual kernel approx

        // Permutations for p-value (trade speed vs stability)
        final int perms = 100;
        ...
        Standardizer stX = new Standardizer(Xs);
        stX.applyInPlace(Xs);

        // ---- fit additive hinge model to get yhat ----
        // Center y for stability (and because we don't want intercept issues)
        double[] yc = Arrays.copyOf(y, n);
        double yMean = mean(yc);
        for (int i = 0; i < n; i++) yc[i] -= yMean;

        AdditiveHingeBasis basis = new AdditiveHingeBasis(Xs, knotsPerVar);
        DMatrixRMaj Phi = basis.designMatrix(Xs);

        // Predict on the same X (this is a diagnostic tool; we’re not doing CV here)
        double[] yhat = ridgePredict(Phi, yc, Phi, ridge);

        // Residuals
        double[] ehat = new double[n];
        for (int i = 0; i < n; i++) ehat[i] = yc[i] - yhat[i];

        // Standardize residuals for kernel bandwidth selection
        double eMean = mean(ehat);
        double eSd = Math.sqrt(Math.max(1e-12, varSample(ehat)));
        for (int i = 0; i < n; i++) ehat[i] = (ehat[i] - eMean) / eSd;

        // ---- build RFF features for X and ehat ----
        double sigmaX = medianPairwiseDistanceND(Xs, Math.min(400, n));
        if (!(sigmaX > 0) || !Double.isFinite(sigmaX)) sigmaX = 1.0;
        sigmaX = sigmaX / Math.sqrt(2.0);

        double sigmaE = medianPairwiseDistance1D(ehat, Math.min(800, n));
        if (!(sigmaE > 0) || !Double.isFinite(sigmaE)) sigmaE = 1.0;
        sigmaE = sigmaE / Math.sqrt(2.0);

        Random rng = new Random(seed);
        RffMap rffMapX = new RffMap(d, rffX, sigmaX, rng, /*useCosSinPairs*/ true);
        DMatrixRMaj Zx = rffMapX.transform(Xs); // n x px

        // 1D RFF for residuals
        RffMap1D rffMapE = new RffMap1D(rffE, sigmaE, new Random(seed ^ 0x9E3779B97F4A7C15L));
        DMatrixRMaj Ze = rffMapE.transform(ehat); // n x pe

        // Optional: z-score feature columns for numeric stability
        zscoreColumnsInPlace(Zx);
        zscoreColumnsInPlace(Ze);

        // ---- compute observed dependence statistic ----
        double statObs = hsicFromFeatures(Zx, Ze);

        // ---- permutation p-value: shuffle residual feature rows ----
        int[] perm = new int[n];
        for (int i = 0; i < n; i++) perm[i] = i;

        int ge = 0;
        for (int b = 0; b < perms; b++) {
            shuffleInPlace(perm, new Random(mix64(seed + 1315423911L * (b + 1L))));
            double statPerm = hsicFromFeaturesPermutedSecond(Zx, Ze, perm);
            if (statPerm >= statObs) ge++;
        }
        double p = (ge + 1.0) / (perms + 1.0);

        boolean reject = Double.isFinite(p) && p < alpha;
        return new TestResult(statObs, p, reject);
    }

    private static double[][] deepCopy(double[][] X) {
        double[][] out = new double[X.length][];
        for (int i = 0; i < X.length; i++) out[i] = Arrays.copyOf(X[i], X[i].length);
        return out;
    }

    /** Median pairwise distance for 1D array (subsample for speed). */
    private static double medianPairwiseDistance1D(double[] x, int limit) {
        int n = Math.min(x.length, limit);
        if (n < 3) return 1.0;

        // sample first n values (already randomized enough for diagnostics)
        ...
        for (int i = 0; i < n; i++) {
            for (int j = i + 1; j < n; j++) {
                double dist = Math.abs(xs[i] - xs[j]);
                if (dist > 0 && Double.isFinite(dist)) dists[idx++] = dist;
            }
        }
        if (idx == 0) return 1.0;
    ...
    /**
     * Approximate HSIC using *linear* HSIC on nonlinear RFF features:
     * HSIC ≈ || Cov(Zx_centered, Zy_centered) ||_F^2.
     */
    private static double hsicFromFeatures(DMatrixRMaj A, DMatrixRMaj B) {
        return hsicFromFeaturesPermutedSecond(A, B, null);
    }

    /**
     * Same as hsicFromFeatures, but optionally permutes rows of B using perm[].
     * If perm is null, uses identity.
     */
    private static double hsicFromFeaturesPermutedSecond(DMatrixRMaj A, DMatrixRMaj B, int[] perm) {
        int n = A.numRows;
        int pa = A.numCols;
        int pb = B.numCols;
        if (B.numRows != n) throw new IllegalArgumentException("Row mismatch");

        if (n < 3 || pa == 0 || pb == 0) return Double.NaN;

        // Column means
        double[] ma = new double[pa];
        double[] mb = new double[pb];

        for (int i = 0; i < n; i++) {
            int bi = (perm == null) ? i : perm[i];
            int aOff = i * pa;
            int bOff = bi * pb;
            for (int j = 0; j < pa; j++) ma[j] += A.data[aOff + j];
            for (int j = 0; j < pb; j++) mb[j] += B.data[bOff + j];
        }
        for (int j = 0; j < pa; j++) ma[j] /= n;
        for (int j = 0; j < pb; j++) mb[j] /= n;

        // Accumulate cross-covariance C = sum_i (a_i - ma)(b_i - mb)^T
        // and return ||C||_F^2 / (n-1)^2.
        double[] C = new double[pa * pb]; // row-major (aIndex * pb + bIndex)
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int ja = 0; ja < pa; ja++) {
                double da = A.data[aOff + ja] - ma[ja];
                int base = ja * pb;
                for (int jb = 0; jb < pb; jb++) {
                    double db = B.data[bOff + jb] - mb[jb];
                    C[base + jb] += da * db;
                }
            }
        }
        ...
        double fro2 = 0.0;
        for (double v : C) fro2 += v * v;

        double denom = (n - 1.0) * (n - 1.0);
        return fro2 / denom;
    }

    /** 1D Random Fourier Features (cos/sin pairs) for an RBF kernel. */
    private static final class RffMap1D {
        final int outM;
        final int mf;
        final double[] w;
        final double[] b;
        ...
        RffMap1D(int m, double sigma, Random rng) {
            if (!(sigma > 0) || !Double.isFinite(sigma)) sigma = 1.0;
            this.mf = Math.max(1, m / 2);
            this.outM = 2 * mf;
            this.w = new double[mf];
            this.b = new double[mf];

            double wStd = 1.0 / sigma;
            for (int i = 0; i < mf; i++) {
                w[i] = rng.nextGaussian() * wStd;
                b[i] = rng.nextDouble() * 2.0 * Math.PI;
            }
            this.scale = Math.sqrt(2.0 / outM);
        }
        ...
        DMatrixRMaj transform(double[] x) {
            int n = x.length;
            DMatrixRMaj Z = new DMatrixRMaj(n, outM);
            for (int i = 0; i < n; i++) {
                int col = 0;
                double xi = x[i];
                for (int f = 0; f < mf; f++) {
                    double t = w[f] * xi + b[f];
                    Z.set(i, col++, scale * Math.cos(t));
                    Z.set(i, col++, scale * Math.sin(t));
                }
            }
            return Z;
        }
