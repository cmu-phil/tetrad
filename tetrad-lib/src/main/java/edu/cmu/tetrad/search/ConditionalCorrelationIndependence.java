...
public final class ConditionalCorrelationIndependence {
    ...
    public double isIndependent(Node x, Node y, List<Node> z) {
        try {
            ...
            List<Integer> rows = this.getRows(dataSet, allNodes, nodesHash);

            if (rows.isEmpty()) return 0;

            double[] rx = this.residuals(x, z, rows);
            double[] ry = this.residuals(y, z, rows);

            // rx _||_ ry ?
            double score = this.independent(rx, ry);
            this.score = score;
    ...
    public double[] residuals(Node x, List<Node> z, List<Integer> rows) {
        ...
        DataSet _dataSet = (dataSet.subsetRowsColumns(_rows, _cols));

        for (int j = 0; j < _dataSet.getNumColumns(); j++) {
            this.scale(_dataSet, j);
        }
        ...
        for (int i = 0; i < _N; i++) {
            Set<Integer> js = this.getCloseZs(_data, _z, i, kernelRegressionSampleSize,
            ...
            for (int j : js) {
                ...
                if (this.getKernelMultiplier() == Kernel.Epinechnikov) {
                    k = this.kernelEpinechnikov(d, h);
                } else if (this.getKernelMultiplier() == Kernel.Gaussian) {
                    k = this.kernelGaussian(d, h);
                } else {
                    throw new IllegalStateException("Unsupported kernel type: " + this.getKernelMultiplier());
                }
    ...
    public void setKernelRegressionSampleSize(int kernelRegressionSapleSize) {
        kernelRegressionSampleSize = kernelRegressionSapleSize;
    }
    ...
    private double independent(double[] x, double[] y) {
        ...
        for (int m = 1; m <= this.getNumFunctions(); m++) {
            for (int n = 1; n <= this.getNumFunctions(); n++) {
                ...
                double score = abs(this.nonparametricFisherZ(_x, _y));
    ...
    private double nonparametricFisherZ(double[] _x, double[] _y) {
        ...
        return z / (sqrt((this.moment22(__x, __y))));
    }
