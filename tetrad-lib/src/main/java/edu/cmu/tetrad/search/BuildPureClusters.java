...
public final class BuildPureClusters {
    ...
    private void initAlgorithm(double alpha, TestType sigTestType) {
        ...
        scoreTestMode = (this.sigTestType == TestType.DISCRETE ||
        ...
        if (sigTestType == TestType.DISCRETE) {
            ...
            numVariables = this.getCovarianceMatrix().getSize();
            independenceTest = new IndTestFisherZ(this.getCovarianceMatrix(), .1);
            TestType type;
            ...
            if (dataSet != null) {
                ...
                tetradTest = new ContinuousTetradTest(this.getCovarianceMatrix(), type, alpha);
            }
        }
        labels = new int[this.numVariables()];
    ...
    public Graph search() {
        ...
        List<int[]> clustering = this.findMeasurementPattern();
    ...
    private boolean uncorrelated(int v1, int v2) {
        ...
        if (this.getCovarianceMatrix() != null) {
            ...
            return this.getIndependenceTest().isIndependent(variables.get(v1),
                    variables.get(v2));

        } else {
            return this.getIndependenceTest().isIndependent(dataSet.getVariable(v1),
    ...
    private List<int[]> findMaximalCliques(int[] elements, int[][] ng) {
        boolean[][] connected = new boolean[numVariables()][numVariables()];
        ...
        this.findMaximalCliquesOperator(numCalls, output, connected,
    ...
    private void findMaximalCliquesOperator(int[] numCalls,
        ...
        for (nod = minnod + nod; nod >= 1; nod--) {
            ...
            if (newce == 0) {
                ...
                numCalls[0]++;
                this.findMaximalCliquesOperator(numCalls, output,
    ...
    private List<List<int[]>> filterAndOrderClusterings(List<List<int[]>> baseListOfClusterings,
        ...
        for (int i = 0; i < baseListOfClusterings.size(); i++) {
            ...
            boolean[][] impurities = new boolean[numVariables()][numVariables()];
            ...
            List<int[]> newClustering2 = this.removeMarkedImpurities(newClustering,
                    impurities);
        ...
        for (int i = 0; i < listOfClusterings.size(); i++) {
            numIndicators[i] = this.clustersize3(listOfClusterings.get(i));
        }
        this.sortClusterings(0, listOfClusterings.size(), listOfClusterings,
                numIndicators);
        for (int i = 0; i < listOfClusterings.size(); i++) {
            numIndicators[i] = this.clustersize(listOfClusterings.get(i));
        }

        int start = 0;

        while (start < listOfClusterings.size()) {
            int size3 = this.clustersize3(listOfClusterings.get(start));
            int end = start + 1;

            for (int j = start + 1; j < listOfClusterings.size(); j++) {
                if (size3 != this.clustersize3(listOfClusterings.get(j))) {
                    break;
                }
                end++;
            }
    ...
    private List<int[]> removeMarkedImpurities(List<int[]> partition, boolean[][] impurities) {
        ...
        boolean[] eliminated = new boolean[numVariables()];
            ...
            if (eliminated[elements[i][0]]) {
                for (int j = i + 1; j < elements.length; j++) {
                    if (!eliminated[elements[j][0]]) {
                        this.swapElements(elements, i, j, temp);
                        break;
                    }
                }
            }
    ...
    private List<int[]> initialMeasurementPattern(int[][] ng, int[][] cv) {
        ...
        for (int v1 = 0; v1 < this.numVariables() - 1; v1++) {
            for (int v2 = v1 + 1; v2 < this.numVariables(); v2++) {
        ...
        for (int v1 = 0; v1 < this.numVariables() - 1; v1++) {
            for (int v2 = v1 + 1; v2 < this.numVariables(); v2++) {
                ...
                for (int v3 = 0; v3 < this.numVariables() - 1 && notFound; v3++) {
                    if (v1 == v3 || v2 == v3 || ng[v1][v3] == EDGE_NONE || ng[v1][v3] ==
                            EDGE_GRAY || ng[v2][v3] == EDGE_NONE || ng[v2][v3] ==
                            EDGE_GRAY) {
                        continue;
                    }
                    for (int v4 = v3 + 1; v4 < this.numVariables() && notFound; v4++) {
        ...
        for (int v1 = 0; v1 < this.numVariables() - 1; v1++) {
            for (int v2 = v1 + 1; v2 < this.numVariables(); v2++) {
                ...
                for (int v3 = 0; v3 < this.numVariables() - 1 && notFound; v3++) {
                    ...
                    for (int v5 = v3 + 1; v5 < this.numVariables() && notFound; v5++) {
                        if (v1 == v5 || v2 == v5 || //ng[v1][v5] != EDGE_BLUE || ng[v3][v5] != EDGE_BLUE ||
                                ...
                                cv[v2][v5] != EDGE_BLACK ||
                                cv[v3][v5] != EDGE_BLACK ||
                                this.clusteredPartial1(v1, v3, v5, v2)) {
                            continue;
                        }

                        for (int v4 = 0; v4 < this.numVariables() - 1 && notFound; v4++) {
                            if (v1 == v4 || v2 == v4 || v3 == v4 || v5 == v4 ||
                                    ...
                                    cv[v3][v4] != EDGE_BLACK ||
                                    cv[v5][v4] != EDGE_BLACK ||
                                    this.clusteredPartial2(v1, v3, v5, v2, v4)) {
                                continue;
                            }

                            for (int v6 = v4 + 1; v6 < this.numVariables() && notFound; v6++) {
                                ...
                                if (this.unclusteredPartial3(v1, v3, v5, v2, v4, v6)) {
                                    notFound = false;
                ...
                if (notFound) {
                    ...
                    for (int v3 = 0; v3 < this.numVariables() && notFound; v3++) {
                        ...
                        for (int v4 = 0; v4 < this.numVariables() - 2 && notFound; v4++) {
                            if (v1 == v4 || v2 == v4 || v3 == v4 ||
                                    ...
                                    cv[v2][v4] != EDGE_BLACK ||
                                    cv[v3][v4] != EDGE_BLACK ||
                                    this.clusteredPartial1(v1, v2, v3, v4)) {
                                continue;
                            }
                            ...
                            for (int v5 = v4 + 1; v5 < this.numVariables() - 1 && notFound; v5++) {
                                if (v1 == v5 || v2 == v5 || v3 == v5 ||
                                        ...
                                        this.clusteredPartial2(v1, v2, v3, v4,
                                                v5)) {
                                    continue;
                                }

                                for (int v6 = v5 + 1; v6 < this.numVariables() && notFound; v6++) {
                                    if (v1 == v6 || v2 == v6 || v3 == v6 ||
                                            ng[v1][v6] == EDGE_GRAY ||
                                            ng[v2][v6] == EDGE_GRAY ||
                                            ng[v3][v6] == EDGE_GRAY ||
                                    ...
                                    if (this.unclusteredPartial3(v1, v2, v3, v4, v5, v6)) {
                                        notFound = false;
                                        ng[v1][v4] = ng[v4][v1] = EDGE_NONE;
        ...
        List<List<int[]>> actualClustering = this.filterAndOrderClusterings(clusterings, ids,
                orderedIds, ng);
        return this.purify(actualClustering, orderedIds);
    }

    private List<int[]> individualPurification(List<int[]> clustering) {
        boolean oldOutputMessage = outputMessage;
            ...
            for (int o2 = o1 + 1; o2 < cset1 - 1; o2++) {
                for (int o3 = o2 + 1; o3 < cset1; o3++) {
                    for (int o4 = 0; o4 < cset2 - 2; o4++) {
                        if (!this.validClusterPairPartial1(cluster1[o1],
                                cluster1[o2], cluster1[o3], cluster2[o4], cv)) {
                            continue;
                        }
                        for (int o5 = o4 + 1; o5 < cset2 - 1; o5++) {
                            if (!this.validClusterPairPartial2(cluster1[o1],
                                    cluster1[o2], cluster1[o3], cluster2[o5],
                                    cv)) {
                                continue;
                            }
                            for (int o6 = o5 + 1; o6 < cset2; o6++) {
                                if (this.validClusterPairPartial3(cluster1[o1],
    ...
    private List<int[]> findMeasurementPattern() {
        ...
        List<int[]> initialClustering = this.initialMeasurementPattern(ng, cv);
        ...
        //Rule 1
        for (int x1 = 0; x1 < this.numVariables() - 1; x1++) {
            outer_loop:
            for (int y1 = x1 + 1; y1 < this.numVariables(); y1++) {
                ...
                for (int x2 = 0; x2 < this.numVariables(); x2++) {
                    ...
                    for (int x3 = 0; x3 < this.numVariables(); x3++) {
                        if (x1 == x3 || x2 == x3 || y1 == x3 ||
                                ...
                                cv[x2][x3] == EDGE_NONE ||
                                cv[y1][x3] == EDGE_NONE ||
                                !this.partialRule1_1(x1, x2, x3, y1)) {
                            continue;
                        }
                        for (int y2 = 0; y2 < this.numVariables(); y2++) {
                            if (x1 == y2 || x2 == y2 || x3 == y2 || y1 == y2 ||
                                    ...
                                    cv[x3][y2] == EDGE_NONE ||
                                    cv[y1][y2] == EDGE_NONE ||
                                    !this.partialRule1_2(x1, x2, y1, y2)) {
                                continue;
                            }
                            for (int y3 = 0; y3 < this.numVariables(); y3++) {
                                if (x1 == y3 || x2 == y3 || x3 == y3 ||
                                        ...
                                        cv[y1][y3] == EDGE_NONE ||
                                        cv[y2][y3] == EDGE_NONE ||
                                        !this.partialRule1_3(x1, y1, y2, y3)) {
                                    continue;
                                }
                ...
                for (int x2 = 0; x2 < this.numVariables(); x2++) {
                    ...
                    for (int y2 = 0; y2 < this.numVariables(); y2++) {
                        if (x1 == y2 || x2 == y2 || y1 == y2 ||
                                ...
                                cv[y1][y2] == EDGE_NONE ||
                                ng[y1][y2] == EDGE_GRAY ||
                                !this.partialRule2_1(x1, x2, y1, y2)) {
                            continue;
                        }
                        for (int x3 = 0; x3 < this.numVariables(); x3++) {
                            if (x1 == x3 || x2 == x3 || y1 == x3 || y2 == x3 ||
                                    ...
                                    cv[y1][x3] == EDGE_NONE ||
                                    cv[y2][x3] == EDGE_NONE ||
                                    !this.partialRule2_2(x1, x2, x3, y2)) {
                                continue;
                            }
                            for (int y3 = 0; y3 < this.numVariables(); y3++) {
                                if (x1 == y3 || x2 == y3 || x3 == y3 ||
                                        ...
                                        cv[y1][y3] == EDGE_NONE ||
                                        cv[y2][y3] == EDGE_NONE ||
                                        !this.partialRule2_3(x2, y1, y2, y3)) {
                                    continue;
                                }
                                ng[x1][y1] = ng[y1][x1] = EDGE_NONE;
                                continue outer_loop;
        ...
        List<int[]> orderedIds = new ArrayList<>();
        List<List<int[]>> actualClusterings = this.filterAndOrderClusterings(clusterings, ids,
                orderedIds, ng);
    ...
    private List<List<int[]>> chooseClusterings(List<int[]> clustering, List<List<Integer>> outputIds,
        ...
        boolean[] buffer = new boolean[numVariables()];
        ...
        if (need3) {
            for (int i = 0; i < clustering.size() - 1; i++) {
                for (int j = i + 1; j < clustering.size(); j++) {
                    compatibility[i][j] = compatibility[j][i] = this.compatibleClusters(
                            clustering.get(i),
                            clustering.get(j), cv);
                }
            }
        ...
        for (int i = 0; i < max; i++) {
            ...
            if (this.isNewClustering(clusterings, newClustering)) {
