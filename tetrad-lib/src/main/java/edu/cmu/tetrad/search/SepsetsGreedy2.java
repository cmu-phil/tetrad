 ...
 */
public class SepsetsGreedy2 implements SepsetProducer {
    ...
    public SepsetsGreedy2(Graph graph, IndependenceTest independenceTest, TeyssierScorer scorer, SepsetMap extraSepsets, int depth) {
        this.graph = graph;
        ...
        for (int d = 0; d <= FastMath.min((this.depth == -1 ? 1000 : this.depth), FastMath.max(adji.size(), adjk.size())); d++) {
            if (d <= adji.size()) {
                ...
                while ((choice = gen.next()) != null) {
                    ...
                    List<Node> after = new ArrayList<>(allCond);
                    after.removeAll(v);

                    List<Node> perm = new ArrayList<>(v);
                    perm.add(i);
                    perm.add(k);
                    perm.addAll(after);
