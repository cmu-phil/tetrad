...
public class EstimateRank {
    ...
    public double[] CanCor(int[] iA, int[] iB, double[][] cov) {
        ...
        RealMatrix S = this.getInverse(covA).multiply(covAB).multiply(this.getInverse(covB)).multiply(covBA);
    ...
    public int Estimate(int[] iA, int[] iB, double[][] cov, int N, double alpha) {
        ...
        this.cov = cov;
        this.N = N;
        double[] Cors = this.CanCor(iA, iB, cov);
        int rank = 0;
        boolean reject = true;
