...
public class BicScore implements LocalDiscreteScore, IBDeuScore {
    ...
    public BicScore(DataSet dataSet) {
        ...
        for (int i = 0; i < variables.size(); i++) {
            numCategories[i] = this.getVariable(i).getNumCategories();
        }
    }
    ...
    public double localScore(int node, int[] parents) {
        ...
        return 2 * lik - penaltyDiscount * params * Math.log(N) + 2 * this.getPriorForStructure(parents.length);
    }
