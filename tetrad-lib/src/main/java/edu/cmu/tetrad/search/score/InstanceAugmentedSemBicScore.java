 ...
 *
 * Local score:
 *   IS-local(i | Pa) = Pop-local(i | Pa) + alpha * logN(x_i | b^T x_Pa, s2)
 *
 * where b = Sigma_{Pa,Pa}^{-1} Sigma_{Pa,i} and
 *       s2 = Sigma_{ii} - Sigma_{i,Pa} Sigma_{Pa,Pa}^{-1} Sigma_{Pa,i}.
 *
 * Supports either:
 *  - centered instance row x (use 2-arg ctor), or
 *  - raw instance row + population means (use 3-arg ctor), in which case the
 *    instance is centered internally using those means.
 */
public final class InstanceAugmentedSemBicScore implements InstanceSpecificScore {
    ...
    /**
     * Use this when the instance comes from a different dataset and is NOT centered.
     * Provide population means (aligned with base.getVariables()) and the class
     * will center the instance internally.
     */
    ...
    @Override public void setAlpha(double alpha) { this.alpha = alpha; }
    ...
    public void setPenaltyDiscount(double d) { base.setPenaltyDiscount(d); }
    public double getPenaltyDiscount() { return base.getPenaltyDiscount(); }
    ...
    public SemBicScore getBase() { return base; }
    public double[] getInstanceRowCentered() { return Arrays.copyOf(xCentered, xCentered.length); }
}