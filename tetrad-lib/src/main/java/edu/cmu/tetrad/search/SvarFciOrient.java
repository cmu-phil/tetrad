...
//        graph.closeInducingPaths();   //to make sure it's a legal PAG
    ...
    private void ruleR1(Node a, Node b, Node c, Graph graph) {
        ...
        if (graph.getEndpoint(a, b) == Endpoint.ARROW && graph.getEndpoint(c, b) == Endpoint.CIRCLE) {
            ...
            orientSimilarPairs(graph, getKnowledge(), c, b, Endpoint.TAIL);
            orientSimilarPairs(graph, getKnowledge(), b, c, Endpoint.ARROW);
        }
    }
    ...
    private void ruleR2(Node a, Node b, Node c, Graph graph) {
        if ((graph.isAdjacentTo(a, c)) &&
            ...
            if ((graph.getEndpoint(a, b) == Endpoint.ARROW) &&
                ...
                orientSimilarPairs(graph, getKnowledge(), a, c, Endpoint.ARROW);
                if (verbose) {
        ...
        for (Node B : nodes) {
            ...
            for (Node D : intoBCircles) {
                ...
                while ((choice = gen.next()) != null) {
                    ...
                    if (!this.isArrowpointAllowed(D, B, graph)) {
                        continue;
                    }

                    graph.setEndpoint(D, B, Endpoint.ARROW);
                    orientSimilarPairs(graph, getKnowledge(), D, B, Endpoint.ARROW);
                    if (verbose) {
...
//        next.put(b, c);
        ...
        while (reachable.size() > 0) {
            ...
            if (e == x) {
                ...
                int _maxPathLength = maxPathLength == -1 ? 1000 : maxPathLength;
...
//                        }
    ...
    private void doDdpOrientation(Node d, Node a, Node b, Node c, Graph graph) {
        ...
        if (sepset.contains(b)) {
            ...
            graph.setEndpoint(a, b, Endpoint.ARROW);
            graph.setEndpoint(c, b, Endpoint.ARROW);
            orientSimilarPairs(graph, getKnowledge(), a, b, Endpoint.ARROW);
            orientSimilarPairs(graph, getKnowledge(), c, b, Endpoint.ARROW);
        ...
        for (Node a : nodes) {
            ...
            for (Node b : adjacents) {
                ...
                for (List<Node> u : ucCirclePaths) {
                    ...
                    orientSimilarPairs(graph, getKnowledge(), a, b, Endpoint.TAIL);
                    graph.setEndpoint(b, a, Endpoint.TAIL);
                    orientSimilarPairs(graph, getKnowledge(), b, a, Endpoint.TAIL);
...
//                    if (graph.isAdjacentTo(a, c)) continue;
    ...
    private void orientTailPath(List<Node> path, Graph graph) {
        for (int i = 0; i < path.size() - 1; i++) {
            ...
            orientSimilarPairs(graph, getKnowledge(), n1, n2, Endpoint.TAIL);
            graph.setEndpoint(n2, n1, Endpoint.TAIL);
            orientSimilarPairs(graph, getKnowledge(), n2, n1, Endpoint.TAIL);
            changeFlag = true;
    ...
    private boolean ruleR8(Node a, Node c, Graph graph) {
        ...
        for (Node b : intoCArrows) {
            ...
            orientSimilarPairs(graph, getKnowledge(), c, a, Endpoint.TAIL);
            changeFlag = true;
            return true;
        }
...
//        Collections.sort(tier_y);
        ...
        for (i = 0; i < tier_x.size(); ++i) {
            if (this.getNameNoLag(x.getName()).equals(this.getNameNoLag(tier_x.get(i)))) {
                indx_comp = i;
                break;
            }
        }
        ...
        for (i = 0; i < tier_y.size(); ++i) {
            if (this.getNameNoLag(y.getName()).equals(this.getNameNoLag(tier_y.get(i)))) {
                indy_comp = i;
                break;
            }
        }
