 ...
 *
 * <p>
 * v3: identical behavior to MinimaxTRffCITest2, with an added design-matrix (Phi) cache.
 * </p>
 */
public final class MinimaxTRffCITest3 implements IndependenceTest, RowsSettable {
    ...
    // cache for design matrices Phi: (parents + useRows signature + rff knobs + childIndexForSeed) -> Phi
    private final ConcurrentHashMap<DesignKey, DesignMap> designCache = new ConcurrentHashMap<>();

    // toggle (in case you want to disable caching quickly)
    private boolean cacheDesigns = true;
    ...
    public MinimaxTRffCITest3(DataSet data, double alpha) {
        ...
        this.ridge = 1e-2;
        this.binsPerContZ = 3;
        this.minStratumSize = 12;
        this.permutations = 500;

        // 	•	increase binsPerContZ from 3 → 6 or 8
        //	•	reduce minStratumSize from 12 → 8 or 10
        //	•	increase permutations from 500 → 1000

        this.binsPerContZ = 6;
        this.minStratumSize = 8;
        this.permutations = 1000;
    }
    ...
    public IndependenceResult checkIndependence(Node x, Node y, Set<Node> z) {
        ...
        if (verbose) {//&& indep) {
            if (indep) {
                ...
                TetradLogger.getInstance().log(LogUtilsSearch.dependenceFactMsg(x, y, z, p));
            }
        }
        return r;
    }
    ...
    public double getPValue(Node x, Node y, Set<Node> z) {
        ...
        // ---------------- DISCRETE BRANCH UNCHANGED ----------------
        if (isDiscrete(iy)) {
            // keep your existing discrete-Y code exactly as you had it
        ...
        // ---------------- CONTINUOUS BRANCH: PERMUTE X WITHIN STRATA(Z) ----------------
        ...
        // Null design (Z) fixed
        DesignMap dmNull = buildDesign(parentsNull, useRows, iy);
        if (dmNull == null) return 0.0;

        // Alt design (Z,X) for observed X fixed -> tObs
        DesignMap dmAltObs = buildDesign(parentsAlt, useRows, iy);
        if (dmAltObs == null) return 0.0;

        double ll0 = fitStudentTRidgeLL(yObs, dmNull.Phi);
        double ll1 = fitStudentTRidgeLL(yObs, dmAltObs.Phi);
        if (!Double.isFinite(ll0) || !Double.isFinite(ll1)) return 0.0;

        double tObs = 2.0 * (ll1 - ll0);
        if (!Double.isFinite(tObs)) return 0.0;

        // Precompute pieces needed to rebuild ONLY the X block of Phi under permutations.
        // We'll copy the null Phi and then append the permuted-X feature columns.
        // This is what avoids refitting the null / rebuilding Z features every time.
        final AltDesignPlan plan = buildAltPlanForPermutingX(ix, iz, useRows, iy);

        SplittableRandom rng = new SplittableRandom(seed);
        int B = Math.max(50, permutations);

        // permutation operates on an index mapping over useRows-space:
        // xSrc[i] tells which original row index supplies X for permuted sample i.
        int[] xSrc = new int[n];
        for (int i = 0; i < n; i++) xSrc[i] = i;

        int ge = 0;
        int valid = 0;

        // scratch PhiAlt: n x (M0 + Mx)
//        double[][] PhiAlt = new double[n][plan.M0 + plan.Mx];
//
//        for (int b = 0; b < B; b++) {
//            // reset mapping to identity then shuffle within strata
//            for (int i = 0; i < n; i++) xSrc[i] = i;
//            shuffleWithinStrataInt(xSrc, strata, rng); // permute X-source indices within strata
//
//            // build PhiAlt for this permutation: [PhiNull | PhiX(permuted)]
//            buildAltPhiFromNullPlusPermutedX(PhiAlt, dmNull.Phi, plan, xSrc);
//
//            double pll1 = fitStudentTRidgeLL(yObs, PhiAlt);
//            if (!Double.isFinite(pll1)) continue;
//
//            double t = 2.0 * (pll1 - ll0); // ll0 fixed across permutations
//            if (!Double.isFinite(t)) continue;
//
//            valid++;
//            if (t >= tObs) ge++;
//        }
        ...
        for (int b = 0; b < B; b++) {
            for (int i = 0; i < n; i++) xSrc[i] = i;
            shuffleWithinStrataInt(xSrc, strata, rng); // permute X-source indices within strata(Z)

            // rebuild full alt design using the permuted X (because RFF mixes continuous parents)
            double[][] PhiAlt = buildDesignWithPermutedX(parentsAlt, useRows, iy, ix, xSrc).Phi;

            double pll1 = fitStudentTRidgeLL(yObs, PhiAlt);
            if (!Double.isFinite(pll1)) continue;

            double t = 2.0 * (pll1 - ll0);
            if (!Double.isFinite(t)) continue;
    ...
    private DesignMap buildDesignWithPermutedX(int[] parentsAlt, int[] useRows, int childIndexForSeed,
                                               int ix, int[] xSrc) {
        // Same as buildDesign(), but whenever it needs X (either continuous or discrete),
        // it uses the value from useRows[xSrc[i]] instead of useRows[i].

        int[] cont = filterContinuous(parentsAlt);
        int[] disc = filterDiscrete(parentsAlt);
        ...
        // Build Zc with possible X override (continuous case)
        double[][] Zc = new double[n][cont.length];
        for (int i = 0; i < n; i++) {
            int row = useRows[i];
            int rowX = useRows[xSrc[i]]; // where X is sourced from after permutation
            for (int j = 0; j < cont.length; j++) {
                int var = cont[j];
                Zc[i][j] = (var == ix) ? zCols[var][rowX] : zCols[var][row];
            }
        }

        long seed = rffSeed ^ (long) childIndexForSeed * 0x9E3779B97F4A7C15L ^ Arrays.hashCode(parentsAlt);
        ...
        for (int i = 0; i < n; i++) {
            ...
            int dataRowX = useRows[xSrc[i]];
            buildPhiRowPermutedX(rowPhi, i, dataRow, dataRowX, Zc, cont.length, W, phase, phiScale, oh, disc, ix);
    ...
    private void buildPhiRowPermutedX(double[] out,
                                      int i, int dataRow, int dataRowX,
                                      double[][] Zc, int dCont,
        ...
        for (int t = 0; t < discParents.length; t++) {
            ...
            int lev = (var == ix)
                    ? data.getInt(dataRowX, var)   // permuted X source row
                    : data.getInt(dataRow,  var);
    ...
    /* ======================================================================================
     * Helpers for "permute X within strata(Z)" continuous branch
     * ====================================================================================== */

    private static final class AltDesignPlan {
        final int M0;          // columns in null Phi
        final int Mx;          // columns contributed by X (intercept+Z already in null)
        final int rffOffX;     // offset of X's RFF block within the X-only block
        final int ohOffX;      // offset of X's one-hot block within the X-only block

        final int[] xCont;     // X is continuous? then length=1 else 0
        final int[] xDisc;     // X is discrete? then length=1 else 0

        final OneHotSpec ohX;  // one-hot spec for X (baseline dropped)
        final double[][] Wx;   // RFF weights for X (D x dContX)
        final double[] phasex; // RFF phase for X (D)
        final double phiScale; // sqrt(2/D)

        final int rffFeatures;

        AltDesignPlan(int M0, int Mx,
                      int[] xCont, int[] xDisc,
                      OneHotSpec ohX,
                      double[][] Wx, double[] phasex, double phiScale,
                      int rffFeatures) {
            this.M0 = M0;
            this.Mx = Mx;
            this.rffOffX = 0;
            this.ohOffX = rffFeatures;
            this.xCont = xCont;
            this.xDisc = xDisc;
            this.ohX = ohX;
            this.Wx = Wx;
            ...
            this.rffFeatures = rffFeatures;
        }
    }

    private AltDesignPlan buildAltPlanForPermutingX(int ix, int[] iz, int[] useRows, int childIndexForSeed) {
        // We need to reproduce exactly the feature mapping for the X parent that buildDesign() uses.
        // buildDesign() seeds RNG with: rffSeed ^ childIndexForSeed * ... ^ Arrays.hashCode(parents)
        // Here the "parents" are parentsAlt = sorted(iz + ix)
        int[] parentsAlt = append(iz, ix);
        Arrays.sort(parentsAlt);

        // Determine if X is continuous or discrete
        int[] xCont = isDiscrete(ix) ? new int[0] : new int[]{ix};
        int[] xDisc = isDiscrete(ix) ? new int[]{ix} : new int[0];

        // OneHot for X (if discrete)
        OneHotSpec ohX = buildOneHotSpec(xDisc);

        // RFF params for X (if continuous)
        int D = rffFeatures;
        long seed = rffSeed ^ (long) childIndexForSeed * 0x9E3779B97F4A7C15L ^ Arrays.hashCode(parentsAlt);
        Random rng = new Random(seed);

        int dCont = xCont.length; // 0 or 1
        double[][] Wx = new double[D][Math.max(1, dCont)];
        for (int k = 0; k < D; k++) {
            for (int j = 0; j < Wx[k].length; j++) {
                Wx[k][j] = rng.nextGaussian() / rffSigma;
            }
        }
        double[] phasex = new double[D];
        for (int k = 0; k < D; k++) phasex[k] = 2.0 * PI * rng.nextDouble();
        double phiScale = sqrt(2.0 / D);

        // Null columns = (intercept + RFF(Zcont) + onehot(Zdisc))
        // We'll infer M0 from a null design build (already built in caller) but here we only need Mx.
        // X-only contributes: RFF block (D) + onehot (K-1) (or zero)
        int Mx = D + ohX.totalCols;

        // We'll fill M0 later using dmNull.Phi[0].length
        // For convenience, compute with placeholder M0 = 0; caller will overwrite when constructing plan.
        // (But we actually pass dmNull.M0 at runtime—see call site.)
        // We'll return with M0=0, and caller will patch it.
        // Simpler: set M0 using current class's design rules by building dmNull here? Not needed.
        return new AltDesignPlan(0, Mx, xCont, xDisc, ohX, Wx, phasex, phiScale, D);
    }

    private static void buildAltPhiFromNullPlusPermutedX(double[][] outPhiAlt,
                                                         double[][] phiNull,
                                                         AltDesignPlan planIn,
                                                         int[] xSrc) {
        final int n = phiNull.length;
        final int M0 = phiNull[0].length;

        // patch plan with correct M0
        // (We keep plan immutable but derive locals.)
        final int D = planIn.rffFeatures;
        final int Mx = planIn.Mx;

        // Safety: outPhiAlt dimensions should match n x (M0+Mx)
        // Copy null block
        for (int i = 0; i < n; i++) {
            System.arraycopy(phiNull[i], 0, outPhiAlt[i], 0, M0);
        }

        // Fill X block
        final double[][] Wx = planIn.Wx;
        final double[] phase = planIn.phasex;
        final double phiScale = planIn.phiScale;

        // X continuous? compute its RFF using zCols[ix][useRows[xSrc[i]]]
        if (planIn.xCont.length == 1) {
            // NOTE: relies on outer class members; so we’ll do this as an instance method in practice.
            // This static helper is called by an instance wrapper below (see next helper).
            throw new IllegalStateException("Use instance wrapper for continuous X; see buildAltPhi... instance overload.");
        }

        // X discrete? set one-hot
        // This static helper also needs data access; handled by instance overload below.
        if (planIn.xDisc.length == 1) {
            throw new IllegalStateException("Use instance wrapper for discrete X; see buildAltPhi... instance overload.");
        }

        // If X has no signal (shouldn't happen), leave zeros.
        for (int i = 0; i < n; i++) {
            Arrays.fill(outPhiAlt[i], M0, M0 + Mx, 0.0);
        }
    }

    // Instance overload that has access to `data` and `zCols`.
    private void buildAltPhiFromNullPlusPermutedX(double[][] outPhiAlt,
                                                  double[][] phiNull,
                                                  AltDesignPlan plan,
                                                  int[] useRows,
                                                  int ix,
        ...
        // Fill X block
        final int D = plan.rffFeatures;
        final int xBlockOff = M0;

        // Initialize X block to zeros
        for (int i = 0; i < n; i++) {
            Arrays.fill(outPhiAlt[i], xBlockOff, xBlockOff + plan.Mx, 0.0);
        }

        // RFF(X) if X continuous
        if (!isDiscrete(ix)) {
            for (int i = 0; i < n; i++) {
                int src = xSrc[i];
                int dataRow = useRows[src];
                double xval = zCols[ix][dataRow]; // globally z-scored already

                // dContX=1, so dot = Wx[k][0]*xval
                for (int k = 0; k < D; k++) {
                    double dot = plan.Wx[k][0] * xval;
                    outPhiAlt[i][xBlockOff + k] = plan.phiScale * cos(dot + plan.phasex[k]);
                }
            }
        }

        // OneHot(X) if X discrete
        if (isDiscrete(ix)) {
            OneHotSpec oh = plan.ohX; // built on {ix}
            int ohOff = xBlockOff + D;

            for (int i = 0; i < n; i++) {
                int src = xSrc[i];
                int dataRow = useRows[src];

                int lev = data.getInt(dataRow, ix);
                if (lev == DiscreteVariable.MISSING_VALUE) continue;
                if (lev <= 0) continue; // baseline dropped

                int col = (lev - 1); // only one disc parent
                if (col >= 0 && col < oh.totalCols) {
                    outPhiAlt[i][ohOff + col] = 1.0;
                }
            }
        }
    }
...
//        Arrays.sort(parentsAlt);
//
//        // Precompute useRows signature once (for design-cache key)
...
//            int[] yObs = extractDiscreteY(iy, useRows);
//
//            DesignMap dmNull = buildDesignCached(parentsNull, useRows, rowsSig, iy);
//            DesignMap dmAlt  = buildDesignCached(parentsAlt,  useRows, rowsSig, iy);
//            if (dmNull == null || dmAlt == null) return 0.0;
//
//            double ll0 = fitMultinomialLogitLL(yObs, K, dmNull.Phi);
//            double ll1 = fitMultinomialLogitLL(yObs, K, dmAlt.Phi);
...
//    }
    ...
    private DesignMap buildDesignCached(int[] parents, int[] useRows, long rowsSig, int childIndexForSeed) {
        if (!cacheDesigns) return buildDesign(parents, useRows, childIndexForSeed);
    ...
    public void setCacheDesigns(boolean cacheDesigns) {
        this.cacheDesigns = cacheDesigns;
        if (!cacheDesigns) designCache.clear();
    }
    ...
    // =========================================================
    // Small utility classes (strata cache + one-hot spec + design cache)
    // =========================================================
    ...
    private record DesignKey(int[] parents, long rowsSig, int childIndexForSeed,
        ...
        @Override
        public int hashCode() {
            int h = Arrays.hashCode(parents);
            h = 31 * h + Long.hashCode(rowsSig);
            h = 31 * h + Integer.hashCode(childIndexForSeed);
        ...
        public boolean equals(Object o) {
            ...
            return rowsSig == k.rowsSig
                    && childIndexForSeed == k.childIndexForSeed
