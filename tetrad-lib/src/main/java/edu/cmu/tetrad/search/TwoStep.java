...
public final class TwoStep {
    ...
    private static double bicForRow(double rss, int n, int k) {
        double nSafe = TMath.max(1, n);
        double rssSafe = TMath.max(rss, 1e-12);
        return nSafe * TMath.log(rssSafe / nSafe) + k * TMath.log(nSafe);
    }
    ...
    private static SimpleMatrix covarianceS(SimpleMatrix X) {
        int n = X.numRows();
        return X.transpose().mult(X).divide(TMath.max(1, n));
    }
    ...
    private static SimpleMatrix standardizeCols(SimpleMatrix X) {
        ...
        for (int j = 0; j < p; j++) {
            ...
            double sd = TMath.sqrt(TMath.max(s2 / TMath.max(n - 1, 1), 1e-12));
    ...
    private static Graph toGraph(SimpleMatrix B, List<Node> vars, double thr) {
        ...
        for (int i = 0; i < p; i++)
            for (int j = 0; j < p; j++) {
                if (i == j) continue;
                if (TMath.abs(B.get(i, j)) > thr) g.addDirectedEdge(vars.get(j), vars.get(i));
            }
        return g;
    }
    ...
    private static double cond2(SimpleMatrix A) {
        ...
        for (int i = 0; i < evd.getNumberOfEigenvalues(); i++) {
            ...
            maxEv = TMath.max(maxEv, ev);
            minEv = TMath.min(minEv, ev);
        }
        if (!(maxEv > 0.0) || !(minEv > 0.0)) return Double.POSITIVE_INFINITY;
        return TMath.sqrt(maxEv / minEv);
    }
    ...
    public void setCoefThreshold(double t) {
        ...
        this.twoCycleMinAbs = TMath.max(this.twoCycleMinAbs, t);
    }
...
//        bicPairwiseCleanupCov(S, n, M, B);
        ...
        if (breakTwoCyclesEnabled) {
            double minAbs = TMath.max(twoCycleMinAbs, coefThreshold);
    ...
    private SimpleMatrix buildMaskWithAdaptiveLassoGram(SimpleMatrix S, int n) {
        ...
        final double lambdaEffRow = normalizeLossByN ? (lambda * TMath.log(TMath.max(8, n)) / 2.0) : lambda;
        ...
        for (int i = 0; i < p; i++) {
            ...
            for (int k = 0; k < P.length; k++) {
                double b = TMath.abs(ols.beta.get(k, 0));
                w[k] = useAdaptiveLasso ? 1.0 / TMath.pow(b + eps, adaptiveGamma) : 1.0;
            }

            SimpleMatrix beta = lassoGram(S, i, P, w, lambdaEffRow, alassoEps, alassoMaxIter, null);

            for (int k = 0; k < P.length; k++) {
                if (TMath.abs(beta.get(k, 0)) > maskThreshold) {
                    M.set(i, P[k], 1.0);
                }
            }
            M.set(i, i, 0.0);
    ...
    private SimpleMatrix lassoGram(SimpleMatrix S, int i, int[] P,
        ...
        for (int j = 0; j < m; j++) Gdiag[j] = TMath.max(G.get(j, j), 1e-12);
        ...
        for (int it = 0; it < maxIter; it++) {
            ...
            for (int j = 0; j < m; j++) {
                ...
                beta[j] = bjNew;
                maxDelta = TMath.max(maxDelta, TMath.abs(bjNew - bjOld));
            }
    ...
    private SimpleMatrix fastIcaMixingUsingFastIca(SimpleMatrix X_nxp, int p) {
        ...
        for (int i = 0; i < p; i++) {
            ...
            double s = 1.0 / TMath.max(TMath.abs(Aperm.get(i, i)), 1e-12);
    ...
    private SimpleMatrix refineByConstrainedLSCov(SimpleMatrix S, int n, SimpleMatrix B0, SimpleMatrix M) {
        ...
        for (int i = 0; i < p; i++) {
            ...
            for (int k = 0; k < P.length; k++) {
                ...
                B.set(i, P[k], TMath.abs(v) < coefThreshold ? 0.0 : v);
            }
        }
    ...
    private SimpleMatrix residualIndependenceCleanup(SimpleMatrix X, SimpleMatrix M, SimpleMatrix B) {
        ...
        java.util.function.BiFunction<double[], double[], Double> depScore = (a, b) -> {
            ...
            for (int t = 0; t < n; t++) {
                meanA += TMath.tanh(a[t]);
                meanB += TMath.tanh(b[t]);
            }
            meanA /= n;
            meanB /= n;
            double num = 0, va = 0, vb = 0;
            for (int t = 0; t < n; t++) {
                double ta = TMath.tanh(a[t]) - meanA;
                double tb = TMath.tanh(b[t]) - meanB;
                num += ta * tb;
                va += ta * ta;
                vb += tb * tb;
            }
            double den = TMath.sqrt(TMath.max(va, 1e-12) * TMath.max(vb, 1e-12));
            return TMath.abs(num / TMath.max(den, 1e-12));
        };
        ...
        for (int i = 0; i < p; i++) {
            for (int j = i + 1; j < p; j++) {
                ...
                double s_none = TMath.max(depScore.apply(ri_none, colAsArray.apply(j)),
                ...
                if (allow_ij && allow_ji) {
                    ...
                    double[] ri = fitResiduals.apply(i, Pi);
                    double[] rj = fitResiduals.apply(j, Pj);
                    double s = TMath.max(depScore.apply(ri, colAsArray.apply(j)),
        ...
        if (breakTwoCyclesEnabled) Bout = breakTwoCyclesStrict(Bout, TMath.max(twoCycleMinAbs, coefThreshold));
        return Bout;
    }
    ...
    private void writeOlsRowCoeffs(SimpleMatrix X, int target, int[] P, SimpleMatrix B) {
        ...
        for (int k = 0; k < P.length; k++) {
            ...
            B.set(target, P[k], TMath.abs(v) < coefThreshold ? 0.0 : v);
        }
    }

    private double prune(double v) {
        return TMath.abs(v) < coefThreshold ? 0.0 : v;
    }
    ...
    private OlsCov olsFromCov(SimpleMatrix S, int i, int[] P, int n) {
        ...
        out.beta = beta;
        out.rss = n * TMath.max(1.0 - explained, 0.0);
        return out;
    }
    ...
    private double rssFromCov(SimpleMatrix S, int i, int[] P, int n) {
        ...
        return n * TMath.max(1.0 - explained, 0.0);
    }
    ...
    private SimpleMatrix breakTwoCyclesStrict(SimpleMatrix B, double minAbs) {
        ...
        for (int i = 0; i < p; i++) {
            for (int j = i + 1; j < p; j++) {
                double a = TMath.abs(out.get(i, j));
                double b = TMath.abs(out.get(j, i));
                if (a < minAbs) a = 0.0;
                if (b < minAbs) b = 0.0;
                if (a == 0.0 && b == 0.0) {
                    out.set(i, j, 0.0);
    ...
    private static final class Hungarian {
        ...
        static int[] maximizeAbsDiagonal(SimpleMatrix C) {
            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    cost[i][j] = -TMath.abs(C.get(i, j)); // maximize abs -> minimize negative abs
