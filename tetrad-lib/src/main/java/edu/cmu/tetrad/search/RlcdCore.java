...
public class RlcdCore {
     ...
     *
     * @param core The {@code RlcdCore} instance to be copied. It serves as the source
     *             from which the new {@code RlcdCore} object is created, replicating
     ...
     * This class is used to store various sets of constraints and essential relationships
     * in the graph being learned.
     *
     * The data members of this class capture mandatory and prohibited edges in
     * the learned graph, as well as information on collider and non-collider constraints,
     * which are essential for correctly determining causal relationships.
     */
     ...
     *
     * @param p       An instance of {@code RlcdParams} encapsulating all the
     *                essential parameters for the RLCD algorithm, such as alpha
     *                level, stage 1 method, and rank test configurations.
     * @param knowledge An optional {@code Knowledge} object that allows specifying
     *                  known causal relations or constraints. If {@code null}, a new
     *                  empty {@code Knowledge} instance is created.
    ...
    /**
     * Executes the first stage of the RLCD structure learning algorithm. This method processes
     * a covariance matrix to generate constraints that can be used for subsequent stages of
     * causal structure learning. It identifies required and prohibited edges in the graph, and
     * sets up collider and non-collider constraints.
     *
     * @param cov The covariance matrix containing the variables and their relationships,
     *            used for generating structure constraints.
     * @param N   The sample size corresponding to the covariance matrix, required for
     *            statistical tests during constraint generation.
     * @return An instance of {@code Stage1Output} containing the generated structure constraints,
     *         including required edges, prohibited edges, and collider/non-collider information.
     */
    ...
    /**
     * Constructs a CPDAG (Completed Partially Directed Acyclic Graph) from the given variables
     * and constraints. The method enforces must-have edges, removes must-not-have edges,
     * applies collider constraints, and orients remaining edges using standard orientation rules
     * as described in Zhang 2008.
     *
     * @param vars A list of nodes representing the variables in the graph.
     * @param s1   An instance of {@code Stage1Output} containing constraints on edges
     *             (must-have, must-not-have) and collider/non-collider constraints.
     * @return An instance of {@code Graph} representing the constructed CPDAG after
     *         incorporating the given constraints.
     */
...
//        return GraphTransforms.undirectedToCpdag(g);
    ...
    /**
     * Applies GIN (General Independence Network) orientation rules to the given CPDAG (Completed Partially Directed Acyclic Graph)
     * using the specified covariance matrix and sample size. This method serves as a hook for the GIN algorithm to further
     * orient the edges in the CPDAG based on the input parameters and configurations.
     *
     * @param cov  The covariance matrix containing information about the variables and their relationships, used to inform edge orientations.
     * @param cpdag The initial CPDAG (Completed Partially Directed Acyclic Graph) to which GIN orientation rules will be applied.
     * @param N    The sample size associated with the covariance matrix, influencing statistical tests during the GIN orientation process.
     * @return The CPDAG after applying the GIN orientation rules, potentially with additional edge orientations performed by the GIN algorithm.
     */
    ...
    /**
     * Tests whether the rank of the cross-covariance matrix extracted from the given covariance matrix
     * is less than or equal to the specified rank threshold using one of the supported methods.
     * The method supports rank determination using either the singular value decomposition (SVD) or
     * other statistical methods like Wilks' lambda test (currently not implemented).
     *
     ...
     * @param N      The sample size associated with the covariance matrix, used in certain statistical methods (e.g., Wilks).
     * @param alpha  The significance level for the rank test, relevant for statistical methods (e.g., Wilks).
     * @return A boolean value indicating whether the rank test passed. Returns true if the rank is less than or equal to the specified threshold, and false otherwise.
     ...
     * The method extracts the relevant entries from the covariance matrix
     * based on the provided index arrays for the two variable subsets.
     *
     ...
     * @return A cross-covariance matrix represented as an instance of {@code DMatrixRMaj},
     *         containing the covariances between the variables in subsets A and B.
     */
     ...
     * The rank is determined based on the number of singular values greater than the specified threshold.
     *
     * @param M   The input matrix for which the numerical rank is to be computed.
     * @param tau The threshold value for singular values. Singular values greater than this threshold are considered significant.
