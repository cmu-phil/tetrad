...
public class ChiSquareTest {
    ...
    public ChiSquareTest(DataSet dataSet, double alpha) {
        ...
        for (int i = 0; i < this.getDims().length; i++) {
            ...
            getDims()[i] = variable.getNumCategories();
        }

        this.dataSet = dataSet;
        this.alpha = alpha;
        cellTable = new CellTable(null);
        getCellTable().setMissingValue(DiscreteVariable.MISSING_VALUE);
    }
    ...
    public synchronized Result calcChiSquare(int[] testIndices) {
        ...
        getCellTable().addToTable(this.getDataSet(), testIndices);
    ...
    public boolean isDetermined(int[] testIndices, double p) {
        ...
        System.arraycopy(this.selectFromArray(this.getDims(), testIndices), 1, condDims, 0,
                condDims.length);

        int[] coords = new int[testIndices.length];
        int numValues = getCellTable().getNumValues(0);
        ...
        while (combinationIterator.hasNext()) {
            ...
            long total = getCellTable().calcMargin(coords, firstVar);
            ...
            for (int i = 0; i < numValues; i++) {
                ...
                long numi = getCellTable().getValue(coords);
