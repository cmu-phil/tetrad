...
public final class FgesOrienter implements GraphSearch, GraphScorer, Reorienter {
    ...
    public void orient(Graph graph) {
        graphToOrient = new EdgeListGraph(graph);
        graphToOrient = GraphUtils.undirectedGraph(graphToOrient);
    ...
    public void setFaithfulnessAssumed(boolean faithfulness) {
        faithfulnessAssumed = faithfulness;
    }
    ...
    public Graph search() {
        ...
        effectEdgesGraph = this.getEffectEdges(nodes);
...
//
                            ...
                            if (FgesOrienter.this.existsKnowledge()) {
                                if (FgesOrienter.this.getKnowledge().isForbidden(x.getName(), y.getName()) && FgesOrienter.this.getKnowledge().isForbidden(y.getName(), x.getName())) {
                                    continue;
                                }

                                if (!FgesOrienter.this.validSetByKnowledge(y, emptySet)) {
                                    continue;
                                }
                            }
                            ...
                            if (covariances != null) {
                                double s1 = FgesOrienter.this.localSemScoreSingleParent(hashIndices.get(y), hashIndices.get(x));
                                double s2 = FgesOrienter.this.localSemScoreSingleParent(hashIndices.get(y));
                                bump = s1 - s2;
                            } else {
                                bump = FgesOrienter.this.scoreGraphChange(y, Collections.singleton(x), emptySet, hashIndices);
                            }
//
//                            if (bump > 0.0) {
                            if (bump > -FgesOrienter.this.getPenaltyDiscount() * Math.log(FgesOrienter.this.sampleSize())) {
//                                if (bump > -getPenaltyDiscount() * Math.log(sampleSize())) {
    ...
    private void fes(Graph graph) {
        ...
        while (!sortedArrows.isEmpty()) {
            ...
            if (!this.validInsert(x, y, arrow.getHOrT(), arrow.getNaYX(), graph)) {
                continue;
            }
            ...
            for (Node node : visited) {
                Set<Node> neighbors = getNeighbors(node, graph);
    ...
    private void bes(Graph graph) {
        ...
        while (!sortedArrows.isEmpty()) {
            ...
            if (!this.validDelete(y, arrow.getHOrT(), arrow.getNaYX(), graph)) {
                continue;
            }
            ...
            protected Boolean compute() {
                if (to - from <= 25) {
                    for (int _w = from; _w < to; _w++) {
                        OrderedPair<Node> p = pairs.get(_w);
                        Node w = p.getFirst();
                        Node x = p.getSecond();
                        ...
                        if (!graph.isAdjacentTo(w, x)) {
                            FgesOrienter.this.calculateArrowsForward(w, x, graph);
                        }
                    }
    ...
    private void calculateArrowsForward(Node a, Node b, Graph graph) {
        if (this.isFaithfulnessAssumed() && !effectEdgesGraph.isAdjacentTo(a, b)) return;
        ...
        int _depth = Math.min(t.size(), depth == -1 ? 1000 : depth);
        ...
        while ((choice = gen.next()) != null) {
            ...
            if (this.existsKnowledge()) {
                if (!this.validSetByKnowledge(b, s)) {
                    continue;
                }
            }


            double bump = this.insertEval(a, b, s, naYX, graph, hashIndices);
            ...
            protected Boolean compute() {
                if (to - from <= chunk) {
                    for (int _w = from; _w < to; _w++) {
                        ...
                        if (!graph.isAdjacentTo(w, x)) {
                            FgesOrienter.this.calculateArrowsBackward(w, x, graph);
                        }

                        if (!graph.isAdjacentTo(w, y)) {
                            FgesOrienter.this.calculateArrowsBackward(w, y, graph);
                        }
                    }
...
//        final int _depth = Math.min(_naYX.size(), depth == -1 ? 1000 : depth);
        ...
        while ((choice = gen.next()) != null) {
            ...
            double bump = this.deleteEval(a, b, h, naYX, graph, hashIndices);
...
//            throw new IllegalArgumentException(x + " and " + y + " are already adjacent in the graph.");
    ...
    private void addRequiredEdges(Graph graph) {
        if (!this.existsKnowledge()) return;
    ...
    private void setCovMatrix(ICovarianceMatrix covarianceMatrix) {
        ...
        variables = covarianceMatrix.getVariables();
    ...
    private void buildIndexing(List<Node> nodes) {
        ...
        for (Node node : nodes) {
            hashIndices.put(node, variables.indexOf(node));
        }
    }
    ...
    private double scoreGraphChange(Node y, Set<Node> parents1,
        ...
        if (this.isDiscrete()) {
            score1 = this.localDiscreteScore(yIndex, parentIndices1);
        } else {
            score1 = this.localSemScore(yIndex, parentIndices1);
        }
        ...
        if (this.isDiscrete()) {
            score2 = this.localDiscreteScore(yIndex, parentIndices2);
        } else {
            score2 = this.localSemScore(yIndex, parentIndices2);
        }
    ...
    private double localSemScoreSingleParent(int i, int parent) {
        ...
        int n = this.sampleSize();
        final int p = 1;
        double covXX = covariances.getValue(parent, parent);
        ...
        if (residualVariance <= 0 && verbose) {
            out.println("Nonpositive residual varianceY: resVar / varianceY = " + (residualVariance / this.getCovMatrix().getValue(i, i)));
            return Double.NaN;
        }
    ...
    private double score(double residualVariance, int n, int p, double c) {
        return -n * Math.log(residualVariance) - c * this.getK(p) * Math.log(n);
    }
...
//                e2.printStackTrace();
    ...
    private void storeGraph(Graph graph) {
        ...
        if (topGraphs.isEmpty() || score > topGraphs.first().getScore()) {
            ...
            if (topGraphs.size() > this.getnumCPDAGsToStore()) {
