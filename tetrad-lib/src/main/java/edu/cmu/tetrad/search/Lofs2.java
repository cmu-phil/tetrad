...
//        }
        ...
        varnames = dataSets.get(0).getVariableNames();
    ...
    public Graph orient() {
        ...
        if (rule == Rule.R1TimeLag) {
            this.ruleR1TimeLag(skeleton, graph);
        } else if (rule == Rule.R1) {
            this.ruleR1(skeleton, graph, nodes);
        } else if (rule == Rule.R2) {
            graph = GraphUtils.undirectedGraph(skeleton);
            this.ruleR2(graph, graph);
        } else if (rule == Rule.R3) {
            graph = GraphUtils.undirectedGraph(skeleton);
            this.ruleR3(graph);
        } else if (rule == Rule.R4) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.ruleR4(graph);
        } else if (rule == Rule.EB) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.entropyBased(graph);
        } else if (rule == Rule.Tanh) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.tanhGraph(graph);
        } else if (rule == Rule.Skew) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.skewGraph(graph, false);
        } else if (rule == Rule.SkewE) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.skewGraph(graph, true);
        } else if (rule == Rule.RSkew) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.robustSkewGraph(graph, false);
        } else if (rule == Rule.RSkewE) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.robustSkewGraph(graph, true);
        } else if (rule == Rule.IGCI) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.igci(graph);
        } else if (rule == Rule.RC) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.resolveEdgeConditional(graph);
        } else if (rule == Rule.Patel) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.patelTauOrientation(graph, Double.NaN);
        } else if (rule == Rule.Patel25) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.patelTauOrientation(graph, 0.25);
        } else if (rule == Rule.Patel50) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.patelTauOrientation(graph, 0.50);
        } else if (rule == Rule.Patel75) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.patelTauOrientation(graph, 0.75);
        } else if (rule == Rule.Patel90) {
            graph = GraphUtils.undirectedGraph(skeleton);
            return this.patelTauOrientation(graph, 0.90);
        } else if (rule == Rule.FastICA) {
    ...
    private void ruleR1(Graph skeleton, Graph graph, List<Node> nodes) {
        List<DataSet> centeredData = DataUtils.center(dataSets);
...
//            }
    ...
    private void ruleR2(Graph skeleton, Graph graph) {
        ...
        for (Edge adj : edgeList1) {
            ...
            this.resolveOneEdgeMax2(graph, x, y, !this.isOrientStrongerDirection());
        }
    }
...
//        neighborsx.remove(y);
        ...
        while ((choicex = genx.next()) != null) {
            ...
            double p2 = this.pValue(x, condxMinus);
            ...
            while ((choicey = geny.next()) != null) {
                ...
                double p3 = this.pValue(y, condyPlus);

//                if (p3 > alpha) {
//                    continue;
//                }

                double p4 = this.pValue(y, condyMinus);
...
//            avgParam.add(0.0);
        ...
        for (int i = 0; i < dataSets.size(); i++) {
            ...
            this.optimizeAllRows(data, range, rows, parameters);

            // Print out the estimated coefficients (I - W) for off-diagonals.
            this.printEstimatedCoefs(nodes, rows, parameters);
        ...
        if (!this.isOrientStrongerDirection()) {
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                for (int _j = 0; _j < rows.get(i).size(); _j++) {
                    ...
                    double param = this.avg(paramsforDataSets, i, _j);
            ...
            for (int i = 0; i < rows.size(); i++) {
                ...
                for (int _j = 0; _j < rows.get(i).size(); _j++) {
                    ...
                    double param1 = this.avg(paramsforDataSets, i, _j);
                    ...
                    for (int _i = 0; _i < rows.get(j).size(); _i++) {
                        ...
                        if (i2 == i) {
                            param2 = this.avg(paramsforDataSets, j, _i);
            ...
            this.printEstimatedCoefs(nodes, rows, avgParams);
    ...
    private void optimizeAllRows(Matrix data, double range,
        ...
        for (int i = 0; i < rows.size(); i++) {
            ...
            try {
                this.optimizeRow(i, data, range, rows, parameters);
    ...
    private void optimizeRow(int rowIndex, Matrix data,
                             double range, List<List<Integer>> rows,
                             List<List<Double>> parameters) {
        System.out.println("A");

        int numParams = rows.get(rowIndex).size();

        double[] dLeftMin = new double[numParams];
        double[] dRightMin = new double[numParams];
        ...
        if (false) { //isEdgeCorrected()) {
            ...
            while ((comb = gen.next()) != null) {
                ...
                double v = this.scoreRow(rowIndex, data, rows, parameters);
            ...
            for (int i = 0; i < numParams; i++) {
                ...
                double vLeft = this.scoreRow(rowIndex, data, rows, parameters);
                double dLeft = -range;
                ...
                parameters.get(rowIndex).set(i, range);
                double vRight = this.scoreRow(rowIndex, data, rows, parameters);
                double dRight = range;
        ...
        MultivariateFunction function = new MultivariateFunction() {
            public double value(double[] values) {
                ...
                double v = Lofs2.this.scoreRow(rowIndex, data, rows, parameters);
    ...
    public double scoreRow(int rowIndex, Matrix data, List<List<Integer>> rows, List<List<Double>> parameters) {
        ...
        for (int i = 0; i < data.rows(); i++) {
            ...
            d += (1.0 - this.getSelfLoopStrength()) * data.get(i, rowIndex);

            col[i] = d;
        }

        return this.score(col);
    }
    ...
    private Graph patelTauOrientation(Graph graph, double cutoff) {
        ...
        for (Edge edge : graph.getEdges()) {
            ...
            List<double[]> ret = this.prepareData(dataSet, _x, _y, false, false);
            double[] xData = ret.get(0);
            double[] yData = ret.get(1);

            double R = this.patelTau(xData, yData, cutoff);
    ...
    private double patelTau(double[] d1in, double[] d2in, double cutoff) {
        double grotMIN = this.percentile(d1in, 10);
        double grotMAX = this.percentile(d1in, 90);
        ...
        grotMIN = this.percentile(d2in, 10);
        grotMAX = this.percentile(d2in, 90);
        ...
        double theta1 = this.dotProduct(d1b, d2b) / XT;
        double theta2 = this.dotProduct(d1b, this.minus(1, d2b)) / XT;
        double theta3 = this.dotProduct(d2b, this.minus(1, d1b)) / XT;
...
//        double[] yData = _data.viewColumn(yIndex).toArray();
    ...
    private List<double[]> prepareData(DataSet concatData, Node _x, Node _y, boolean skewCorrection, boolean coefCorrection) {
        ...
        if (skewCorrection) {
            double xSkew = skewness(xData);
            double ySkew = skewness(yData);
        ...
        if (coefCorrection) {
            double coefX;
            try {
                coefX = this.regressionCoef(xData, yData);
            } catch (Exception e) {
                coefX = Double.NaN;
            }
            ...
            try {
                coefY = this.regressionCoef(yData, xData);
            } catch (Exception e) {
                coefY = Double.NaN;
            }
    ...
    private Graph igci(Graph graph) {
        ...
        for (Edge edge : graph.getEdges()) {
            ...
            double f = this.igci(xCol, yCol, 2, 1);
            ...
            if (f < -epsilon) {
                ...
                if (this.resolveOneEdgeMaxR3(xCol, yCol) < 0) {
                    _graph.addDirectedEdge(x, y);
                } else {
                    _graph.addDirectedEdge(y, x);
                }
    ...
    private double igci(double[] x, double[] y, int refMeasure, int estimator) {
        ...
        if (estimator == 1) {
            ...
            hx = hx / (n1 - 1) + this.psi(n1) - this.psi(1);
            ...
            hy = hy / (n2 - 1) + this.psi(n2) - this.psi(1);
...
//            f = b-a;
    ...
    private double score(Node y, List<Node> parents) {
        if (score == Lofs.Score.andersonDarling) {
            ...
            return abs(kurtosis(this.residuals(y, parents, true, true)));
        } else if (score == Lofs.Score.entropy) {
            return this.entropy(y, parents);
        } else if (score == Lofs.Score.skew) {
            return abs(skewness(this.residuals(y, parents, true, true)));
        } else if (score == Lofs.Score.fifthMoment) {
            return abs(standardizedFifthMoment(this.residuals(y, parents, true, true)));
            ...
            double[] _f = this.residuals(y, parents, true, true);
    ...
    private double meanAbsolute(Node node, List<Node> parents) {
        double[] _f = this.residuals(node, parents, false, true);
    ...
    private double logCoshScore(Node node, List<Node> parents) {
        double[] _f = this.residuals(node, parents, true, true);
    ...
    private double[] residuals(Node node, List<Node> parents, boolean standardize, boolean removeNaN) {
        ...
        Node target = this.getVariable(variables, node.getName());
