...
public class PossibleDsepFci {
    ...
    public PossibleDsepFci(Graph graph, IndependenceTest test, CorrelationMatrix corr) {
        ...
        this.setMaxPathLength(maxReachablePathLength);
    }
    ...
    public SepsetMap search() {
        ...
        for (Edge edge : new ArrayList<>(graph.getEdges())) {
            ...
            List<Node> condSet = this.getSepset(test, x, y);
    ...
    public List<Node> getSepset(IndependenceTest test, Node node1, Node node2) {
        List<Node> condSet = this.getCondSet(test, node1, node2, maxReachablePathLength);

        if (sepset == null) {
            condSet = this.getCondSet(test, node2, node1, maxReachablePathLength);
        }
