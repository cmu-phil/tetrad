...
public final class Fgs3 implements GraphSearch, GraphScorer {
    ...
    private void initializeForwardEdgesFromEmptyGraph(final List<Node> nodes) {
        initializeForwardEdgesFromEmptyGraph2(nodes);
    }
...
//     not canceled by other paths (the "effect edges")
    //original
    private void initializeForwardEdgesFromEmptyGraph1(final List<Node> nodes) {
        ...
        buildIndexing(nodes);
        pool.invoke(new InitializeFromEmptyGraphTask(getMinChunk(nodes.size()), 0, nodes.size()));
    ...
    private void initializeForwardEdgesFromEmptyGraph2(final List<Node> nodes) {
        if (verbose) {
    ...
    private void initializeForwardEdgesFromExistingGraph(final List<Node> nodes) {
        initializeForwardEdgesFromExistingGraph2(nodes);
    }

    private void initializeForwardEdgesFromExistingGraph1(final List<Node> nodes) {
        long start = System.currentTimeMillis();
    ...
    private void initializeForwardEdgesFromExistingGraph2(final List<Node> nodes) {
        if (verbose) {
