/// ////////////////////////////////////////////////////////////////////////////
// PhantomKernelEnumerator13.java  (parallel, PAG-space, end-to-end GraspFci)    //
//                                                                             //
// pke13 = pke12 (PKE8's QUESTION over PKE10's POPULATION) with the SEARCH     //
// UNDER TEST swapped from Fcit to GraspFci.  Same enumeration, same oracle,   //
// same verdict buckets; only the search construction differs (see ORACLE      //
// DRIVE and the GraspFci configuration block).                                //
//                                                                             //
// PKE8 asked the single thing that matters for correctness -- does the actual //
// shipping search, from its own GRaSP start, driven by the m-separation      //
...
// PKE8's question: GraspFci sees only the oracle, and the oracle is a function  //
// of the true PAG's Markov class.  So pke13 enumerates distinct true PAGs a   //
// la PKE10 and runs PKE8's end-to-end check once per PAG:                     //
//                                                                             //
//     terminal PAG returned by GraspFci.search()  ==  G* (the true PAG)?        //
//                                                                             //
...
// is exactly PKE10's analyze() convention).  GraspFci is constructed with the //
// MsepTest on that MAG as test and a GraphScore on the same MAG as score.     //
// GraspFci runs GRaSP for its initial phase by construction, so this is the   //
// Oracle GRaSP path with no startWith flip (that flip was an Fcit detail).    //
//                                                                             //
// SELECTION BIAS: a single EXCLUDE_SELECTION_BIAS constant (default true,     //
// matching PKE10/pke12) is applied consistently to the enumeration            //
// projections AND to GraspFci (setExcludeSelectionBias, inherited from        //
// StarFciCheckPag), since G* and the search must operate under the same       //
// assumption.                                                                 //
//                                                                             //
...
//                         over-deletion.                                      //
//   ERROR              : GraspFci threw (no timeout knob in GraspFci).        //
...
// string over all OBS! relabellings -- so GraspFci runs once per isomorphism      //
// class (PAG up to relabelling), not once per labelling.  This is the lever     //
// that cuts the GraspFci call count (up to OBS!-fold in the limit, less in        //
...
// specific representative that was analyzed.  GraspFci's verdict is relabelling-   //
...
// analyze, pke13 runs GraspFci on each distinct PAG the instant it is first      //
...
//       [4]=violation log path (default pke13_violations.log)                 //
//       [5]=magspace checkpoint path (default pke13_magspace_obs<OBS>_        //
//           checkpoint.txt) -- delete to restart enumeration from scratch     //
//       [6]=magspace distinct-class keys path (default pke13_magspace_obs<OBS>_//
...
/**
 * pke13: enumerate DISTINCT TRUE PAGs (magspace or dagsweep, as in PKE10) and, once per
 * PAG, run the real GraspFci from a GRaSP start, oracle-driven, reporting every PAG whose
 * terminal graph is not exactly G* (PKE8's end-to-end check over PKE10's population).
 */
public final class PhantomKernelEnumerator13 {

    /**
     * Default constructor.
     */
    public PhantomKernelEnumerator13() {
    }

    // ────────────────────────────────────────────────────────────────────────
    // CONFIGURATION (enumeration knobs via args, GraspFci knobs hard-coded)
    // ────────────────────────────────────────────────────────────────────────
    ...
    // ── GraspFci configuration (match the shipping defaults to certify) ──
    // Unlike Fcit, GraspFci exposes no timeout, so a hang stalls the run; there is
    // no ERROR-on-timeout escape hatch here (ERROR now means "threw" only).
    /** Sepset-search depth passed to GraspFci (-1 = unlimited). */
    private static final int DEPTH = -1;
    /** Retained for GraphTransforms.dagToPag in dagsweep enumeration only;
     *  GraspFci has no recursive-depth knob. */
    private static final int RECURSIVE_DEPTH = -1;
    /** Applied consistently to the enumeration projections AND GraspFci
     *  (setExcludeSelectionBias, inherited from StarFciCheckPag); see header. */
    ...
    public static void main(String[] args) throws IOException {
        ...
        String violationLogPath = (args.length > 4) ? args[4] : "pke13_violations.log";
        ...
        String checkpointPath = (args.length > 5) ? args[5]
                : "pke13_magspace_obs" + OBS + "_checkpoint.txt";
        String keysPath = (args.length > 6) ? args[6]
                : "pke13_magspace_obs" + OBS + "_keys.txt";

        CANON = new ArrayList<>();
        for (int i = 0; i < OBS; i++) CANON.add(new GraphNode("V" + (i + 1)));

        CONFIG_LINE = String.format(
                "# pke13 config: mode=%s N=%d latent=%d observed=%d "
                        + "depth=%d recDepth=%d "
                        + "exclSel=%b",
        ...
        String header = CONFIG_LINE + "\n# run started " + new Date()
                + "\n# VIOLATIONS: every distinct true PAG whose terminal GraspFci PAG (GRaSP start,"
                + "\n# oracle-driven) is NOT exactly G*.  Analysis is INLINE: each new distinct"
                + "\n# PAG is run through GraspFci the moment it is found, and any violation below"
                ...
                + "\n#   ERROR                      -- GraspFci threw (GraspFci has no timeout)";
        violationLog = new StreamLog(violationLogPath, VIOLATION_LOG_MAX, header);

        // ── Enumeration + inline analysis (single phase) ──
        // GraspFci runs on each distinct PAG as it is discovered; `total` accumulates the
    ...
    // ────────────────────────────────────────────────────────────────────────
    // ENUMERATION + INLINE ANALYSIS: GraspFci runs on each distinct PAG as found
    // ────────────────────────────────────────────────────────────────────────
     ...
     * time it is seen is immediately run through GraspFci (inline analysis).
     *
        ...
        LongStream.range(0, totalMasks).parallel().forEach(mask -> {
            ...
            while ((latChoice = latGen.next()) != null) {
                ...
                try {
                    ...
                    // Dedup on the canonical (isomorph-invariant) key: one GraspFci run per
     ...
     * PAG (won the {@code seen.putIfAbsent} race), GraspFci runs on it and any violation
    ...
    private static long enumerateAndAnalyzeMagSpace(Result total, String checkpointPath,
        ...
        String enumConfig = String.format(
                "# pke13 magspace-enum config: OBS=%d blockSize=%d", OBS, MAGSPACE_BLOCK_SIZE);

        Set<Long> doneBlocks = loadCheckpoint(checkpointPath, enumConfig);
        // Dedup set keyed by CANONICAL (isomorph-invariant) key: GraspFci runs once per
        ...
        for (long oi = 0; oi < numBlocks; oi++) {
            ...
            LongStream.range(lo, hi).parallel().forEach(code -> {
                try {
                    ...
                    // relabellings), so isomorphic PAGs collapse to one GraspFci run.
     ...
     *  the dedup key, so GraspFci runs once per class up to relabelling. Cost is one OBS!
     *  minimization per surviving candidate -- cheaper than the GraspFci run it saves. */
    ...
    // ────────────────────────────────────────────────────────────────────────
    // INLINE ANALYSIS: PKE8's end-to-end GraspFci check, run the instant a PAG is found
    // ────────────────────────────────────────────────────────────────────────

    /** Run GraspFci on one just-discovered distinct true PAG and record the verdict.
    ...
    private static void analyzeInline(Result r, Graph trueMag, Graph truePag) {
        ...
        try {
            ...
            // the oracle as zhangMagFromPag(G*); the completion could leave the
            // class on circle-rich PAGs (fixed 2026-08 via MCS orientation; see
            // PKE12's ZHANG-ROUNDTRIP-FAIL).  Oracle and reference are now
            // consistent by construction, independent of the transform.
            //
            // Round-trip audit of the Zhang completion, kept as a regression guard:
            // this enumeration is exactly the fuzzer that transform needs.
            try {
            ...
            runGraspFci(r, modelId, trueMag, truePag, CANON);
        } catch (Exception ex) {
            r.skipped.increment();
    ...
    private static void runGraspFci(Result r, String modelId, Graph trueMag, Graph truePag,
                                    List<Node> obs) {
        // Oracle drive: MsepTest on the true MAG as test and a GraphScore on the same
        // MAG as score.  On this branch GraspFci extends StarFciCheckPag: GRaSP supplies
        // a Markov DAG (getMarkovDag), search() projects it to a PAG and runs the gated
        // extra-edge-removal passes.  This IS the oracle GRaSP path -- no startWith flip
        // needed as in Fcit.  A fresh graph per run (searches mutate node types internally).
        ...
        Graph terminal;
        try {
            // Replayability: StarFciCheckPag's edge shuffles now draw from RandomUtil,
            // whose generator is THREAD-LOCAL, seeded per thread by a global counter --
            // so which model sees which seed is scheduling-dependent in this parallel
            // harness.  Seeding the worker's generator from the model id right before
            // the search makes every verdict a deterministic function of the model
            // alone: any violation replays standalone from its log entry.
            RandomUtil.getInstance().setSeed(modelId.hashCode());
            GraspFci gfci = new GraspFci(test, scoreObj);
            gfci.setKnowledge(new Knowledge());
            gfci.setExcludeSelectionBias(EXCLUDE_SELECTION_BIAS);   // via StarFciCheckPag
            gfci.setCompleteRuleSetUsed(true);
            gfci.setDepth(DEPTH);   // sets BOTH GRaSP depth and StarFciCheckPag sepset depth
            gfci.setNumStarts(1);
            gfci.setVerbose(false);
            // Match the INTERFACE defaults: the algcomparison wrapper sets guaranteePag
            // from Params.GUARANTEE_PAG_DEFAULT_TRUE (default true, per the name), so the
            ...
            // sepset needs a non-adjacent vertex (e.g. (V2,V6) sep {V1,V3,V5} in model
            // aaaa..ta.aaat.aata..aaaa) are removable only during order-dependent windows in
            // which other spurious edges inflate the adjacency sets -- exactness becomes a
            // function of the shuffle draw.

            // gfci.setLvHeuristicOnly(true);  // return dagToPag(Markov DAG), no removal
            //
            // CAVEAT (nondeterminism): StarFciCheckPag.search() calls an UNSEEDED
            // Collections.shuffle on the edge list before the removal passes, and gated
            // greedy commits are visitation-order-dependent.  So verdicts here are
            // per-DRAW, not per-model: a violation may pass on rerun (and vice versa).
            // Replay any violation K times before trusting it, and consider seeding
            // that shuffle in the branch for reproducibility.
            terminal = gfci.search();
        } catch (Throwable ex) {
            r.error.increment();
            violationLog.write(violationEntry("ERROR", modelId, trueMag, truePag, null,
                    "GraspFci threw: " + ex));
            if (ERR_PRINTS.incrementAndGet() <= 10) {
                System.err.println("GraspFci error " + modelId + ": " + ex);
            }
            return;
        }

        // Normalize terminal to the canonical observed nodes (GraspFci returns a PAG over
     ...
     *  MAG (audited above); the reference side is the oracle MAG itself,
    ...
    private static String violationEntry(String bucket, String modelId, Graph trueMag,
        ...
        if (terminal != null) {
            sb.append("  terminal PAG from GraspFci:\n").append(terminal).append('\n');
        }
        sb.append(detail);
        sb.append("==== end entry ====\n");
        return sb.toString();
    ...
    private static String summarize(Result t, long elapsedMs, String violationLogPath) {
        ...
        sb.append("---- TERMINAL-PAG IDENTITY (GraspFci from GRaSP start, oracle) ----\n");
        sb.append(String.format("EXACT (terminal PAG == G*)         : %d%n", exact));
        sb.append(String.format("VIOLATIONS (terminal PAG != G*)    : %d%s%n", violations,
                violations == 0 ? "   *** clean: GraspFci recovers G* on every distinct PAG ***" : ""));
