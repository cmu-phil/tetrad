...
public final class ResolveSepsets {
     ...
     */
    public static SepsetMapDci ResolveSepsets(final List<SepsetMapDci> sepsets, final List<IndependenceTest> independenceTests,
                                              final Method method, final SepsetMapDci resolvedIndependent, final SepsetMapDci resolvedDependent) {
        final SepsetMapDci resolvedSepset = new SepsetMapDci();
        // get all variables
        final Set<Node> allVars = new HashSet<>();
        ...
        for (final NodePair pair : ResolveSepsets.allNodePairs(new ArrayList<>(allVars))) {
            ...
            if (testsWithPair.size() < 2) {
                ...
                if (testsWithPair.size() == 1) {
                    ...
                    for (final List<Node> sepset : pairSepsets.get(0)) {
            ...
            final List<List<Node>> allConditioningSets = new ArrayList<>();
            for (final List<List<Node>> conditioningSet : pairSepsets) {
                if (conditioningSet == null) {
                    continue;
                }
                allConditioningSets.addAll(conditioningSet);
            }
            for (final List<Node> conditioningSet : allConditioningSets) {
                final List<IndependenceTest> testsWithSet = new ArrayList<>();
                for (final IndependenceTest independenceTest : testsWithPair) {
                ...
                for (int k = 0; k < testsWithSet.size(); k++) {
                    final IndependenceTest testWithSet = testsWithSet.get(k);
                    if (k == 0) {
     ...
     */
    public static boolean isIndependentPooledFisher(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        ...
        final double p = 1.0 - ProbUtils.chisqCdf(tf, 2 * independenceTests.size());
        return (p > alpha);
    }
...
//            }
     ...
     */
    public static boolean isIndependentPooledTippett(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        double p = -1.0;
        for (final IndependenceTest independenceTest : independenceTests) {
            if (ResolveSepsets.missingVariable(x, y, condSet, independenceTest)) continue;
            ...
            final double newp = independenceTest.getPValue();
            if (newp < p) {
                p = newp;
            }
        }
     ...
     */
    public static boolean isIndependentPooledWilkinson(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet, final int r) {
        final double alpha = independenceTests.get(0).getAlpha();
        final double[] p = new double[independenceTests.size()];
        int k = 0;
        for (final IndependenceTest independenceTest : independenceTests) {
     ...
     * 2000.
     */
    public static boolean isIndependentPooledWorsleyFriston(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        double p = -1.0;
        for (final IndependenceTest independenceTest : independenceTests) {
            final List<Node> localCondSet = new ArrayList<>();
     ...
     */
    public static boolean isIndependentPooledStouffer(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        double ts = 0.0;
        for (final IndependenceTest independenceTest : independenceTests) {
            final List<Node> localCondSet = new ArrayList<>();
        ...
        final double p = 2.0 * (1.0 - RandomUtil.getInstance().normalCdf(0, 1, Math.abs(ts)));
        return (p > alpha);
    }
     ...
     */
    public static boolean isIndependentPooledMudholkerGeorge(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        final double c = Math.sqrt(3 * (5 * independenceTests.size() + 4) / (independenceTests.size() * Math.pow(Math.PI, 2) * (5 * independenceTests.size() + 2)));
        double tm = 0.0;
        for (final IndependenceTest independenceTest : independenceTests) {
            final List<Node> localCondSet = new ArrayList<>();
            for (final Node node : condSet) {
            ...
            independenceTest.isIndependent(independenceTest.getVariable(x.getName()), independenceTest.getVariable(y.getName()), localCondSet);
            final double pk = independenceTest.getPValue();
            if (pk != 0 && pk != 1) {
                tm += -c * Math.log(pk / (1 - pk));
            }
        }
        final double p = 2.0 * (1.0 - ProbUtils.tCdf(Math.abs(tm), 5 * independenceTests.size() + 4));
        return (p > alpha);
    }
    ...
    public static boolean isIndependentPooledMudholkerGeorge2(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        final List<Double> pValues = ResolveSepsets.getAvailablePValues(independenceTests, x, y, condSet);
        final double c = Math.sqrt(3 * (5 * pValues.size() + 4) / (pValues.size() * Math.pow(Math.PI, 2) * (5 * pValues.size() + 2)));
        double tm = 0.0;
        for (final double pk : pValues) {
            tm += -c * Math.log(pk / (1 - pk));
        }
        final double p = 2.0 * (1.0 - ProbUtils.tCdf(Math.abs(tm), 5 * pValues.size() + 4));
        return (p > alpha);
    }
     ...
     */
    public static boolean isIndependentPooledAverage(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        double sum = 0.0;
        int numTests = 0;
        ...
        for (final IndependenceTest independenceTest : independenceTests) {
            ...
            independenceTest.isIndependent(independenceTest.getVariable(x.getName()), independenceTest.getVariable(y.getName()), localCondSet);
            final double p = independenceTest.getPValue();
...
//        }
    ...
    private static boolean isMissing(final Node node, final DataSet dataSet) {
        final Node _node = dataSet.getVariable(node.getName());

        final int col = dataSet.getColumn(_node);
     ...
     * TEST
     */
    public static boolean isIndependentPooledAverageTest(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        double ts = 0.0;
        int df = 0;
        for (final IndependenceTest independenceTest : independenceTests) {
        ...
        final double p = 1.0 - ProbUtils.chisqCdf(ts, df);
        return (p > alpha);
    }
     ...
     */
    public static boolean isIndependentPooledRandom(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        final double alpha = independenceTests.get(0).getAlpha();
        final int r = RandomUtil.getInstance().nextInt(independenceTests.size());
        final IndependenceTest independenceTest = independenceTests.get(r);
        final List<Node> localCondSet = new ArrayList<>();
        for (final Node node : condSet) {
            localCondSet.add(independenceTest.getVariable(node.getName()));
        }
     ...
     */
    public static List<NodePair> allNodePairs(final List<Node> nodes) {
    ...
    private static boolean isIndependentMajorityFdr(final List<IndependenceTest> independenceTests, final Node x, final Node y, final List<Node> condSet) {
        ...
        final boolean independent = c < allPValues.size() / 2;
