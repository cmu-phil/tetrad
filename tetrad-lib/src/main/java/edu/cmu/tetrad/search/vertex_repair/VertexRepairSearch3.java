 ...
 */
public class VertexRepairSearch3 implements IGraphSearch {
     ...
     */
    public VertexRepairSearch3(IndependenceTest test, Graph start, Knowledge knowledge,
    ...
    private static List<VertexRepairSearch3.CandidateEdit> dedupCandidateEdits(List<VertexRepairSearch3.CandidateEdit> edits) {
        if (edits == null || edits.isEmpty()) return List.of();
        Map<String, VertexRepairSearch3.CandidateEdit> seen = new LinkedHashMap<>();
        for (VertexRepairSearch3.CandidateEdit ce : edits) {
            if (ce == null) continue;
            String key = ce.key();
    ...
    private static boolean requiresEdgePresenceCheck(VertexRepairSearch3.CandidateEdit cand) {
    ...
    private static boolean allIntendedNewEdgesPresent(Graph g, VertexRepairSearch3.CandidateEdit cand) {
    ...
    private static boolean isProgress(int baselineViol,
                                      int afterViol,
                                      double baselineWeighted,
                                      double afterWeighted,
                                      int currentEdges,
                                      int afterEdges,
                                      double mpBefore,
                                      double mpAfter) {

        // Primary: weighted violations must decrease meaningfully.
        // A minimum gain threshold avoids chasing noise from borderline violations.
        final double MIN_WEIGHTED_GAIN = 1.0; // ~equivalent to one violation at p=0.37

        if (afterWeighted < baselineWeighted - MIN_WEIGHTED_GAIN) return true;

        // Secondary: if weighted violations tie (within threshold), prefer fewer edges.
        if (Math.abs(afterWeighted - baselineWeighted) < MIN_WEIGHTED_GAIN) {
            if (afterEdges < currentEdges) return true;

            // Tertiary: same edges, meaningful Model-P improvement.
            final double MIN_MP_GAIN = 1e-3;
    ...
    private SearchPack computeCandidatesForNode(Graph g, Node center, RepairGraphType gt) {
        ...
        List<VertexRepairSearch3.CandidateEdit> candidates = enumerateCandidates(base, center, gt);
        candidates = new ArrayList<>(candidates);
        if (candidates.stream().noneMatch(VertexRepairSearch3.CandidateEdit::isNoOp)) {
            candidates.addFirst(VertexRepairSearch3.CandidateEdit.noOp());
        }
        ...
        double baselineWeighted = baseEval.weightedViolations();
        ...
        List<VertexRepairSearch3.ScoredCandidate> scored = new ArrayList<>();

        // PASS 1: violationsAfter + nodeP + edges (Model-P deferred)
        for (VertexRepairSearch3.CandidateEdit cand : candidates) {
            if (stopRequested()) return null;
            ...
            GraphEval afterEval = useLocality
                    ? evalGraphLocality(baseCache, g2, affected, false)
                    : new GraphEval(evalViolationsOnly(g2), 0.0, Double.NaN, 0);

            int after = afterEval.violations();
            double afterWeighted = afterEval.weightedViolations();
            double nodePAfter = nodePValue(g2, center);
            int edgesAfter = g2.getNumEdges();

            scored.add(new VertexRepairSearch3.ScoredCandidate(cand, baseline, after,
                    baselineWeighted, afterWeighted, nodePAfter,
        ...
        List<VertexRepairSearch3.ScoredCandidate> ranked = new ArrayList<>(scored);
        ranked.sort(CANONICAL_TABLE_ORDER);
        ...
        for (int i = 0; i < topK; i++) {
            VertexRepairSearch3.ScoredCandidate sc = ranked.get(i);
            if (sc == null || sc.edit() == null) continue;
            keysToEval.add(sc.edit().key());
        }

        // 2b) all simple reorientation moves
        for (VertexRepairSearch3.ScoredCandidate sc : scored) {
            if (sc == null || sc.edit() == null) continue;
            if (moveType(sc.edit()) == VertexRepairSearch3.MoveType.REORIENT_SIMPLE) {
                keysToEval.add(sc.edit().key());
            }
        }
        ...
        {
            List<VertexRepairSearch3.ScoredCandidate> patched = new ArrayList<>(scored.size());
            for (VertexRepairSearch3.ScoredCandidate sc : scored) {
                Double mpAfter = (sc.edit() == null) ? null : mpAfterByKey.get(sc.edit().key());

                patched.add(new VertexRepairSearch3.ScoredCandidate(
                        sc.edit(),
                        sc.violationsBaseline(),
                        sc.violationsAfter(),
                        sc.weightedViolationsBaseline(),
                        sc.weightedViolationsAfter(),
                        sc.nodePAfter(),
                        mpBefore,
        ...
        {
            List<VertexRepairSearch3.ScoredCandidate> patched2 = new ArrayList<>(scored.size());
            for (VertexRepairSearch3.ScoredCandidate sc : scored) {
                boolean ok = wouldPassGuards(base, sc, gt);

                patched2.add(new VertexRepairSearch3.ScoredCandidate(
                        sc.edit(),
                        sc.violationsBaseline(),
                        sc.violationsAfter(),
                        sc.weightedViolationsBaseline(),
    ...
    private List<VertexRepairSearch3.CandidateEdit> enumerateCandidates(Graph g, Node x, RepairGraphType gt) {
        if (g == null || x == null) return List.of(VertexRepairSearch3.CandidateEdit.noOp());

        List<VertexRepairSearch3.CandidateEdit> out = new ArrayList<>();
        out.add(VertexRepairSearch3.CandidateEdit.noOp());
        ...
        for (Edge e : new ArrayList<>(g.getEdges(x))) {
            out.add(VertexRepairSearch3.CandidateEdit.removeEdge(e));
        }
        ...
        for (Edge e : new ArrayList<>(g.getEdges(x))) {
            ...
            for (Edge v : edgeMenuForPair(x, y, gt)) {
                ...
                out.add(VertexRepairSearch3.CandidateEdit.replaceEdge(e, v));
            }
        }
        ...
        for (Node y : pool) {
            ...
            for (Edge add : addMenuForPair(x, y, gt)) {
                out.add(VertexRepairSearch3.CandidateEdit.addEdge(add));
            }
        }
    ...
    private List<VertexRepairSearch3.CandidateEdit> enumerateIncidentOrientationPatternMoves(Graph g, Node x, RepairGraphType gt) {
        ...
        for (int mask = 0; mask < total; mask++) {
            ...
            out.add(VertexRepairSearch3.CandidateEdit.replaceEdges(label, olds, news));
        }

        return out;
    }

    private List<VertexRepairSearch3.CandidateEdit> enumerateCpdagColliderPairMoves(Graph g, Node x) {
        if (g == null || x == null) return List.of();
        ...
        for (int i = 0; i < adj.size(); i++) {
            ...
            for (int j = i + 1; j < adj.size(); j++) {
                ...
                if (!(endXy == Endpoint.ARROW && endXz == Endpoint.ARROW)) {
                    ...
                    out.add(VertexRepairSearch3.CandidateEdit.replaceEdges(
                            label,
                            List.of(exy, exz),
                            List.of(yToX, zToX)
                    ));
    ...
    private GraphEval evalGraphOnce(Graph g) {
        if (g == null) return new GraphEval(0, 0.0, Double.NaN, 0);

        List<IndependenceFact> facts = MarkovCheck.computeAllImpliedFacts(g, this.conditioningSetType);
        if (facts.isEmpty()) return new GraphEval(0, 0.0, Double.NaN, 0);
        ...
        for (CachedIndependenceQueries.Eval e : evals) {
            if (!e.independent()) {
                ...
                if (Double.isFinite(p) && p > 0.0) {
                    weightedViolations += -Math.log(p);
                } else {
                    weightedViolations += -Math.log(1e-15);
                }
            }
            double p = e.pValue();
        ...
        return new GraphEval(violations, weightedViolations, modelP, evals.size());
    }
    ...
    private GraphEval evalGraphLocality(GlobalEvalCache baseCache,
        ...
        if (candidateGraph == null) return new GraphEval(0, 0.0, Double.NaN, 0);
        ...
        for (Map.Entry<String, Boolean> e : globalViolationByKey.entrySet()) {
            if (e.getValue()) {
                violations++;
                // Weight by -log(p): stronger violations count more.
                Double p = globalPByKey.get(e.getKey());
                if (p != null && Double.isFinite(p) && p > 0.0) {
                    weightedViolations += -Math.log(p);
                } else {
                    // p=0 or missing: use a large but finite penalty
                    weightedViolations += -Math.log(1e-15);
                }
            }
        }
        ...
        return new GraphEval(violations, weightedViolations, modelP, globalViolationByKey.size());
    }
    ...
    private boolean wouldPassGuards(Graph base, ScoredCandidate sc, RepairGraphType gt) {
        ...
        return isProgress(baselineViol, afterViol,
                sc.weightedViolationsBaseline(), sc.weightedViolationsAfter(),
                currentEdges, afterEdges, mpBefore, mpAfter);
    }
    ...
    private interface CandidateEdit {
        ...
        static CandidateEdit noOp() {
            return new VertexRepairSearch3.CandidateEdit() {
                @Override
    ...
    private record ScoredCandidate(
            ...
            int violationsAfter,
            double weightedViolationsBaseline,
            double weightedViolationsAfter,
            double nodePAfter,
            double modelPBefore,
            double modelPAfter,
            int edgesAfter,
        ...
        public double weightedDelta() {
            return weightedViolationsAfter - weightedViolationsBaseline;
        }
    }
    ...
    private static final class CandidateTableModel extends AbstractTableModel {
        ...
        private List<VertexRepairSearch3.ScoredCandidate> rows = new ArrayList<>();

        void set(List<VertexRepairSearch3.ScoredCandidate> rows) {
        ...
        public Object getValueAt(int rowIndex, int columnIndex) {
            VertexRepairSearch3.ScoredCandidate r = rows.get(rowIndex);
    ...
    private record GraphEval(int violations, double weightedViolations, double modelP, int nFacts) {
    }
     ...
     */
    private record SearchPack(String centerName, int baseline, List<VertexRepairSearch3.ScoredCandidate> scored) {
