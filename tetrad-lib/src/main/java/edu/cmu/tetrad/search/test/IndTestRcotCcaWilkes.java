...
public final class IndTestRcotCcaWilkes implements IndependenceTest, RowsSettable {
    ...
    public IndTestRcotCcaWilkes(DataSet dataSet, Parameters params) {
        ...
        this.numFeatZ = TMath.max(1, params.getInt("rcit.numF", 100));
        this.numFeatXY = TMath.max(1, params.getInt("rcit.numF2", 5));
        this.lambda = TMath.max(1e-12, params.getDouble("rcit.lambda", this.lambda));
    ...
    private static SimpleMatrix rff1D(SimpleMatrix x, int numF, double sigma, Random rng) {
        ...
        for (int f = 0; f < numF; f++) {
            ...
            for (int i = 0; i < n; i++) {
                double v = x.get(i, 0);
                feat.set(i, f, scale * TMath.cos(w * v + b));
            }
        }
        return feat;
    }
    ...
    private static SimpleMatrix rff(SimpleMatrix X, int numF, double sigma, Random rng) {
        ...
        for (int i = 0; i < numF; i++)
            for (int j = 0; j < n; j++)
                feat.set(j, i, scale * TMath.cos(WX.get(i, j)));
    ...
    private static double approxMedianPairwiseDistance(SimpleMatrix A, int maxRows, int numPairs, Random rng) {
        int n = TMath.min(A.getNumRows(), maxRows);
        int d = A.getNumCols();
