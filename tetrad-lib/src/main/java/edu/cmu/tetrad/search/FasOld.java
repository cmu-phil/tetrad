 ...
 * @see Knowledge
 */
public class FasOld implements IFas {
    /**
     ...
     */
    public FasOld(IndependenceTest test) {
        this.test = test;
    }
     ...
     *
     * @return An undirected graph that summarizes the conditional independencies in the data. This graph represents the
     * adjacencies that remain after applying conditional independence tests.
     * @throws InterruptedException if any
     */
    @Override
     ...
     * @return A list of Triple objects representing the ambiguous triples involving the specified node, or an empty
    ...
    private boolean searchAtDepth(Map<Edge, Double> scores, List<Edge> edges, IndependenceTest test, Map<Node, Set<Node>> adjacencies, int depth, Thread mainThread) throws InterruptedException {
        ...
        class Task implements Callable<Boolean> {
            ...
            public Task(Edge edge, Map<Edge, Double> scores, IndependenceTest test, Map<Node, Set<Node>> adjacencies, int depth, Knowledge knowledge, SepsetMap sepset, Thread mainThread) {
                this.edge = edge;
                this.scores = scores;
                this.test = test;
            ...
            public Boolean call() {
                ...
                try {
                    boolean b = checkSide(scores, test, adjacencies, depth, x, y, knowledge, sepset, mainThread);

                    if (!b) {
                        checkSide(scores, test, adjacencies, depth, y, x, knowledge, sepset, mainThread);
                    }
                } catch (InterruptedException e) {
                    throw new RuntimeException(e);
                }
        ...
        for (Edge edge : edges) {
            tasks.add(new Task(edge, scores, test, adjacencies, depth, knowledge, sepset, mainThread));
        }
