                ...
                for (int j = i - 1; j >= 0; j--) {
                    ...
                    if (tuck(x, j, scorer)) {
                        if (scorer.score() < sp || violatesKnowledge(scorer.getPi())) {
                            ...
                            if (verbose) {
                                System.out.print("\r# Edges = " + scorer.getNumEdges() + " Score = " + scorer.score() + " (betterMutationTuck2)" + " Elapsed " + ((System.currentTimeMillis() - start) / 1000.0 + " s"));
                            }
                        }
