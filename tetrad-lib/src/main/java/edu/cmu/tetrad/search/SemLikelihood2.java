...
public class SemLikelihood2 {
    ...
    public Ret getLikelihood(int i, int[] parents) {
        ...
        Ret ret1 = this.likelihoodJoint(XPlus);
        Ret ret2 = this.likelihoodJoint(X);
    ...
    private Ret likelihoodJoint(List<ContinuousVariable> X) {
        ...
        double lnL = N * this.gaussianLikelihood(k, cov);
    ...
    private double gaussianLikelihood(int k, Matrix sigma) {
        return -0.5 * this.logdet(sigma) - 0.5 * k * (1.0 + LOG2PI);
    }
