...
public class MimbuildPca {
    ...
    private static void standardizeColumnsInPlace(SimpleMatrix X) {
        ...
        for (int j = 0; j < p; j++) {
            ...
            double sd = FastMath.sqrt(m2 / FastMath.max(1, n - 1));
    ...
    public Graph search() throws InterruptedException {
        ...
        int dof = FastMath.max(1, n - 1);
        SimpleMatrix latentCov = L.transpose().mult(L).divide(FastMath.max(1, n - 1));

        double diagMean = 0.0;
        for (int k = 0; k < B; k++) diagMean += latentCov.get(k, k);
        double eps = 1e-8 * (diagMean / FastMath.max(1, B));
