 ...
 */
public final class RecursiveAdjustmentCopy {
    ...
    private RecursiveAdjustmentCopy() {}
        ...
        for (Node b : graph.getAdjacentNodes(x)) {
            ...
            for (Set<Node> Z : frontier) {
                ...
                List<Set<Node>> blockedVariants = blockBranchEnum(
                        graph, x, b, y, Z, nf, maxPathLength, desc, mask, 100000);// - next.size());
...
//        boolean pag   = graph.paths().isLegalPag();
    ...
    static Set<Node> minimize(Graph g, GraphType graphType, Node x, Node y, Set<Node> Z,
        ...
        for (Node n : order) {
            Set<Node> trial = new HashSet<>(best);
            trial.remove(n);
            Set<Node> check = RecursiveAdjustmentCopy.findAdjustmentSet(g, graphType, x, y, trial, notFollowed, maxLen, mask);
