...
public class Kci2SmokeTest {
    ...
    private static edu.cmu.tetrad.search.test.Kci2 makeKci(SimpleMatrix data, boolean approximate) {
        ...
        edu.cmu.tetrad.search.test.Kci2 k = new edu.cmu.tetrad.search.test.Kci2(data, map, null, rows);
        k.approximate = approximate;
        k.kernelType = edu.cmu.tetrad.search.test.Kci2.KernelType.GAUSSIAN;
        k.epsilon = 1e-3;
        k.scalingFactor = 1.0;
    ...
    public void gammaApprox_independent_vs_dependent() {
        ...
        edu.cmu.tetrad.search.test.Kci2 kInd = makeKci(makeDataVxN(600, Double.POSITIVE_INFINITY), true);
        ...
        edu.cmu.tetrad.search.test.Kci2 kDep = makeKci(makeDataVxN(600, 0.05), true);
...
//        assertTrue(pDep < 1e-3, "dependent should be very small (gamma approx).");
    ...
    public void nonApprox_independent_vs_dependent() {
        ...
        edu.cmu.tetrad.search.test.Kci2 kInd = makeKci(makeDataVxN(600, Double.POSITIVE_INFINITY), /*approximate=*/false);
        ...
        edu.cmu.tetrad.search.test.Kci2 kDep = makeKci(makeDataVxN(600, 0.05), /*approximate=*/false);
    ...
    public void permutation_independent_vs_dependent() {
        double alpha = 0.01;
        edu.cmu.tetrad.search.test.Kci2 kInd = makeKci(makeDataVxN(400, Double.POSITIVE_INFINITY), false);
        kInd.rng = new Random(123); // determinism
        double pInd = kInd.isIndependenceConditional(new GraphNode("X"), new GraphNode("Y"),
                List.of(new GraphNode("Z")), alpha);
        assertTrue(pInd > alpha, "perm: independent should fail to reject");

        edu.cmu.tetrad.search.test.Kci2 kDep = makeKci(makeDataVxN(400, 0.05), false);
        kDep.rng = new Random(123);
        double pDep = kDep.isIndependenceConditional(new GraphNode("X"), new GraphNode("Y"),
                List.of(new GraphNode("Z")), alpha);
