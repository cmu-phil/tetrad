...
public final class PhantomKernelEnumerator3 {
    ...
    // Commit-step Markov guard in the FcitMag re-commit path. ON => the re-commit refuses a
    // removal whose candidate MAG has an unshielded collider x*->c<-*y with an oracle-confirmed
    // separator CONTAINING c (firstUnsoundColliderInMag). OFF => legality-only baseline (the
    // alsoBreaks=2478 number). Enable with -DguardRecommit=true.
    private static boolean GUARD_RECOMMIT = Boolean.getBoolean("guardRecommit");
    ...
    public static void main(String[] args) {
        ...
        System.err.printf("Commit-step Markov guard (re-commit): %s%n",
                GUARD_RECOMMIT ? "ON (firstUnsoundColliderInMag refuses unsound-apex commits)" : "OFF (legality-only baseline)");
        ...
        Result total = LongStream.range(0, TOTAL_DAGS)
                .parallel()
                .collect(Result::new, PhantomKernelEnumerator3::accumulate, Result::merge);
        ...
        try {
            ...
            magDump.println("# Tallies -- alsoBreaks: " + total.magCommitAlsoBreaks
                    ...
                    + " | guardPreventedBreak: " + total.magCommitGuardPreventedBreak
                    + " | guardOvercautious: " + total.magCommitGuardOvercautious
                    + " | guard " + (GUARD_RECOMMIT ? "ON" : "OFF") + "\n");
        ...
        System.out.println("FcitMag re-commit (alsoBreaks=" + total.magCommitAlsoBreaks
                ...
                + " guardPreventedBreak=" + total.magCommitGuardPreventedBreak
                + " guardOvercautious=" + total.magCommitGuardOvercautious
                + " | guard " + (GUARD_RECOMMIT ? "ON" : "OFF") + ") written to: " + magRecheckPath);
...
//                                            + "\n"
                                    ...
                                    MagCommitResult mc = magCommit(h0, e.getNode1(), e.getNode2(), sepsets, oracle);
                                    Graph h1mag = mc.pag();
                                    String magBreak = null, outcome;
                                    if (h1mag == null) {
                                        r.magCommitReverted++;
                                        outcome = "REVERTED (MAG illegal -- FcitMag refuses this step)";
                                    } else if (mc.guardCaught()) {
                                        // Guard would refuse this legal MAG; classify what it prevented.
                                        magBreak = imapWitnessAgainst(h1mag, obs, trueMsep);
                                        if (magBreak != null) {
                                            r.magCommitGuardPreventedBreak++;
                                            outcome = "GUARD-CAUGHT (would have BROKEN -- correct refusal)";
                                        } else {
                                            r.magCommitGuardOvercautious++;
                                            outcome = "GUARD-CAUGHT (would have stayed I-map -- conservative refusal)";
                                        }
                                    } else {
                                        magBreak = imapWitnessAgainst(h1mag, obs, trueMsep);
                                        if (magBreak != null) {
...
//                        + " | " + blocker;
    ...
    static final class Result {
        ...
        long magCommitGuardPreventedBreak, magCommitGuardOvercautious; // guard refusals: correct vs conservative
        ...
        static Result merge(Result a, Result b) {
            ...
            a.magCommitGuardPreventedBreak += b.magCommitGuardPreventedBreak;
            a.magCommitGuardOvercautious += b.magCommitGuardOvercautious;
    ...
    // through it. pag is null iff the MAG was illegal (FcitMag reverts). guardCaught is true iff
    // GUARD_RECOMMIT is on and the commit-step Markov guard would refuse this (legal) MAG; the
    // would-be PAG is still returned so the caller can classify what the refusal prevented.
    private static MagCommitResult magCommit(Graph h0, Node x, Node y, SepsetMap sepsets,
                                             IndependenceTest oracle) throws InterruptedException {
        Graph mag = magOfPag(h0);                 // zhangMagFromPag(h0)
        mag.removeEdge(x, y);
        orientSepsetCollidersInMag(mag, sepsets); // == robust stamp here (R0=0 on this population)
        if (!isLegalMag(mag)) return new MagCommitResult(null, false);   // illegal MAG: FcitMag reverts
        boolean guardCaught = GUARD_RECOMMIT && firstUnsoundColliderInMag(mag, oracle) != null;
        return new MagCommitResult(new MagToPag(mag).convert(false, EXCLUDE_SELECTION_BIAS), guardCaught);
    }

    private record MagCommitResult(Graph pag, boolean guardCaught) {}

    // Commit-step Markov guard (enumerator copy of FcitMag.firstUnsoundColliderInMag). Scans every
    // unshielded def-collider x*->c<-*y in the candidate MAG and asks whether (x,y) has an
    // oracle-confirmed separator CONTAINING the apex c. If so the arrowheads into c are unsound.
    private static Triple firstUnsoundColliderInMag(Graph mag, IndependenceTest oracle)
            throws InterruptedException {
        ...
        for (Node c : mag.getNodes()) {
            ...
            for (int i = 0; i < adj.size(); i++) {
                for (int j = i + 1; j < adj.size(); j++) {
                    ...
                    if (apexHasConfirmedSeparator(mag, oracle, x, y, c)) return t;
                }
            }
        }
        return null;
    ...
    // RB-forced-in apex-containing separator, oracle-confirmed. Mirrors FcitMag's
    // hasConfirmedSeparatorContaining EXACTLY (so the measured guard == the shipped guard), but does
    // NOT touch the ROBUST_R0_* counters. If this under-fires (apex is a collider here, so forcing it
    // in looks path-opening to RB), swap the body for the brute-force oracle variant to get the ceiling.
//    private static boolean apexHasConfirmedSeparator(Graph graph, IndependenceTest oracle,
//                                                     Node x, Node y, Node apex)
//            throws InterruptedException {
//        RecursiveBlocking.BlockingResult rb = RecursiveBlocking.blockPathsRecursively(
//                graph, x, y,
//                Collections.singleton(apex),
//                Collections.emptySet(),
//                RECURSIVE_DEPTH, DEPTH, -1, 3, true,
//                TIMEOUT < 0 ? Long.MAX_VALUE : System.currentTimeMillis() + TIMEOUT);
//        if (rb == null || rb.indeterminate() || rb.blockingSet() == null) return false;
//        Set<Node> z = rb.blockingSet();
//        if (!z.contains(apex)) return false;
//        return oracle.checkIndependence(x, y, z).isIndependent();
//    }

//    private static boolean apexHasConfirmedSeparator(Graph graph, IndependenceTest oracle,
//                                                     Node x, Node y, Node apex)
//            throws InterruptedException {
//        List<Node> pool = new ArrayList<>(graph.getNodes());
//        pool.remove(x); pool.remove(y); pool.remove(apex);
//        int cap = Math.min(MAX_COND, pool.size());
//        for (int k = 0; k <= cap; k++) {
//            SublistGenerator gen = new SublistGenerator(pool.size(), k);
//            int[] choice;
//            while ((choice = gen.next()) != null) {
//                if (choice.length != k) continue;
//                Set<Node> z = new HashSet<>(GraphUtils.asSet(choice, pool));
//                z.add(apex);
//                if (oracle.checkIndependence(x, y, z).isIndependent()) return true;
//            }
//        }
//        return false;
//    }
    ...
    private static boolean apexHasConfirmedSeparator(Graph graph, IndependenceTest oracle,
                                                     Node x, Node y, Node apex)
            throws InterruptedException {
        // UNFORCED RB: ask the answerable question ("block x from y in this graph"), purely to
        // harvest the path-relevant neighborhood. Do NOT force the apex in here -- forcing a node
        // that is a collider in `graph` makes the path look open and RB returns no block (the
        // under-firing we already diagnosed). The apex is forced in only at the oracle test below.
        RecursiveBlocking.BlockingResult rb = RecursiveBlocking.blockPathsRecursively(
                graph, x, y,
                Collections.emptySet(),     // forced-in: none
                Collections.emptySet(),
                RECURSIVE_DEPTH, DEPTH, -1, 3, true,
                TIMEOUT < 0 ? Long.MAX_VALUE : System.currentTimeMillis() + TIMEOUT);

        // Local candidate pool: RB's blocking set (if any) UNION the neighbors of x and y. This is
        // the path-relevant neighborhood, bounded by local degree, not by n. Using the union rather
        // than RB's single blocking set avoids missing a separator that uses a path-relevant node
        // RB happened not to place in that one set.
        Set<Node> pool = new LinkedHashSet<>();
        if (rb != null && !rb.indeterminate() && rb.blockingSet() != null) {
            pool.addAll(rb.blockingSet());
        }
        pool.addAll(graph.getAdjacentNodes(x));
        pool.addAll(graph.getAdjacentNodes(y));
        pool.remove(x);
        pool.remove(y);
        pool.remove(apex);                  // apex is added unconditionally below, not chosen

        List<Node> poolList = new ArrayList<>(pool);
        int cap = Math.min(MAX_COND, poolList.size());
        for (int k = 0; k <= cap; k++) {
            SublistGenerator gen = new SublistGenerator(poolList.size(), k);
            int[] choice;
            while ((choice = gen.next()) != null) {
                if (choice.length != k) continue;
                Set<Node> z = new HashSet<>(GraphUtils.asSet(choice, poolList));
                z.add(apex);                // apex forced in: testing "apex is a non-collider witness"
                if (oracle.checkIndependence(x, y, z).isIndependent()) return true;
            }
        }
        return false;
