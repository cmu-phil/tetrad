    ...
    public Graph search() {
        ...
        for (int i = 0; i < p; i++) {
            ...
            for (int j = 0; j < i; j++) {
                ...
                graph.addNondirectedEdge(v, w);
            }
        }

        for (int i = 0; i < this.variables.size(); i++) {
            Node v = this.variables.get(i);
            Set<Integer> available = new HashSet<>(all);
            available.remove(i);
            ...
            for (Edge edge : graph.getEdges(v)) {
                Node w = edge.getDistalNode(v);
                int j = this.variables.indexOf(w);
                if (! W.contains(j)) graph.removeEdge(v, w);
                else if ((H.contains(j) && edge.getNode1() == w)) edge.setEndpoint1(Endpoint.ARROW);
                else if ((H.contains(j) && edge.getNode2() == w)) edge.setEndpoint2(Endpoint.ARROW);
            }
