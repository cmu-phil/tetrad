...
/**
 * The `EdgePriorsAnchorCheck` class provides a test suite for verifying the functional
 * correctness of the anchoring behavior of the `EdgePriors` class. It runs various tests
 * to validate how edge priors behave when anchored at specific alpha values and compares
 * the results against expectations derived from edge weights and beta thresholds.
 * <p>
 * This class contains several static methods and tests, including:
 * <p>
 * 1. Verification of anchoring behavior when beta values approach zero.
 * 2. Testing that the relationship between beta and alpha values aligns
 * with specified expectations.
 * <p>
 * 3. Validation that prior values saturate at a specific beta threshold.
 * 4. Comparison of behavior differences between anchored priors and
 * BIC bridge priors to assess predictability.
 * <p>
 * 5. Testing of the normalization guard to ensure only valid normalizations
 * are permitted for weights.
 * <p>
 * 6. Checking that the provenance of weights is correctly tracked to declare
 * whether they stem from declared, anchored, or BIC bridge origins.
 * <p>
 * The class uses `ok` as a helper method to assert test outcomes, tracking
 * failures and providing reports on test results. The main method defines
 * the entry point to the test suite and executes predefined test cases.
 */
public class EdgePriorsAnchorCheck {

    /**
     * Default constructor for the EdgePriorsAnchorCheck class.
     */
    public EdgePriorsAnchorCheck() {
    ...
    /**
     * Entry point for the EdgePriorsAnchorCheck program. This method performs a series of tests
     * and validations to verify the behavior of different weighting and anchoring mechanisms in the
     * EdgePriors class.
     *
     * @param a An array of command-line arguments. These arguments are not utilized within the method.
     */
    public static void main(String[] a) {
        ...
        System.out.println("\n5. The guard");
        try {
            lo(0.5).toWeightsViaBicBridge(20.0, 50000, alpha).normalizedToMeanOne();
            ok("bridge weights refuse normalisation", false);
        } catch (IllegalStateException ex) {
            ok("bridge weights refuse normalisation",
                    ex.getMessage().contains("toWeightsAnchoredAtAlpha"));
        }
        try {
            EdgePriors n = lo(0.5).toWeightsAnchoredAtAlpha(alpha).normalizedToMeanOne();
            ok("anchored weights normalise fine", n.getOrigin() == EdgePriors.Origin.NORMALIZED);
        } catch (IllegalStateException ex) {
            ok("anchored weights normalise fine", false);
        }
