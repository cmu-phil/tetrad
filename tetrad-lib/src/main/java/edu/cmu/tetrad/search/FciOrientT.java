...
public final class FciOrientT {
     ...
     */
    public FciOrientT(SepsetProducer sepsets, IndependenceTest test) {
    ...
    public void ruleR0(Graph graph) {
        ...
        this.addColliders(graph, sepsets, knowledge);
    }
...
//
    ...
    private void addColliders(Graph graph, SepsetProducer sepsetProducer, IKnowledge knowledge) {
        List<Triple> colliders = this.findCollidersUsingSepsets(sepsetProducer, graph, verbose);
        ...
        for (Triple collider : colliders) {
            ...
            if (!(this.isArrowpointAllowed(a, b, graph) && this.isArrowpointAllowed(c, b, graph))) {
                continue;
            }
