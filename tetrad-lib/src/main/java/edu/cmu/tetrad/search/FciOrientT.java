...
public final class FciOrientT {
     ...
     */
    public FciOrientT(final SepsetProducer sepsets, final IndependenceTest test) {
    ...
    public void ruleR0(final Graph graph) {
        ...
        addColliders(graph, this.sepsets, this.knowledge);
    }
     ...
     */
    public List<Triple> findCollidersUsingSepsets(final SepsetProducer sepsetProducer, final Graph graph, final boolean verbose) {
...
//
    ...
    private void addColliders(final Graph graph, final SepsetProducer sepsetProducer, final IKnowledge knowledge) {
        final List<Triple> colliders = findCollidersUsingSepsets(sepsetProducer, graph, this.verbose);
    ...
    private boolean isNoncollider(final Node a, final Node b, final Node c) {
     ...
     */
    private void ruleR4A(final Graph graph) {
        final List<Node> nodes = graph.getNodes();
    ...
    private void reachablePathFind(final Node a, final Node b, final Node c,
                                   final LinkedList<Node> reachable, final Graph graph) {
...
//        next.put(b, c);
        ...
        while (reachable.size() > 0) {
            ...
            if (e == x) {
                ...
                final int _maxPathLength = this.maxPathLength == -1 ? 1000 : this.maxPathLength;
     ...
     * arguments.
     */
    private void doDdpOrientation(final Node d, final Node a, final Node b, final Node c, final Graph graph) {
        final List<Node> sepset = getSepset(d, c);
    ...
    private void printDdp(final Node d, final List<Node> path, final Node a, final Node b, final Node c, final Graph graph) {
...
//                    if (graph.isAdjacentTo(a, c)) continue;
     ...
     */
    private boolean ruleR10(final Node a, final Node c, final Graph graph) {
        final List<Node> intoCArrows = graph.getNodesInTo(c, Endpoint.ARROW);
