...
public class BossTuck3 {
    ...
    public BossTuck3(@NotNull Score score) {
        this.score = score;
    ...
    public void betterMutationBossTarget(@NotNull TeyssierScorer2 scorer0, Node target, Map<Node, Set<Node>> keeps, Set<NodePair> pairs) {

        Set<Node> keep = new HashSet<>();
        keep.add(target);
        keep.addAll(scorer0.getAdjacentNodes(target));
        ...
        List<Node> pi = scorer0.getPi();
        List<Node> ref = new ArrayList<>(pi);
        List<Node> newPi = scorer.getPi();

        int[] indices = new int[newPi.size()];
        for (int i = 0; i < indices.length; i++) indices[i] = ref.indexOf(newPi.get(i));

        for (int i = 0; i < indices.length; i++) {
            pi.set(indices[i], newPi.get(i));
        }
