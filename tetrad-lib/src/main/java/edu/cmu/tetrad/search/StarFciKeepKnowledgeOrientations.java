...
/**
 * A variant of StarFciGuaranteePag that KEEPS background-knowledge orientations that go beyond the marks required for
 * a legal (canonical, maximally-informative) PAG, rather than letting the legality machinery reject or erase them.
 * <p>
 ...
 * <p>
 * THE PROBLEM THIS VARIANT SOLVES. Background knowledge (required edges; forbidden edges when selection bias is
 ...
 * the check degenerates to exactly PagLegalityCheck.isLegalPag, so this class then behaves like
 * StarFciGuaranteePag.
 * <p>
 ...
 * <p>
 * Changes from StarFciGuaranteePag (2026-8): (1) all legality gating (per-removal commits, the saturating step) uses
 * legalPagModuloKnowledge instead of PagLegalityCheck.isLegalPag; (2) on the ungated path, guaranteePag runs only if
 * the knowledge-refined graph fails the modulo-knowledge certificate; (3) a final knowledge-refinement step is
 * applied uniformly at the end of search(), so knowledge marks are present in the output even when no edge removal
 * ever committed (in the base class the untouched dagToPag start graph carries no knowledge marks at all).
 * <p>
...
public abstract class StarFciKeepKnowledgeOrientations implements IGraphSearch {
     ...
     */
    public StarFciKeepKnowledgeOrientations(IndependenceTest test) {
    ...
    /**
     * Sets the parallel processing mode for the StarFciKeepKnowledgeOrientations class.
     *
