        ...
        for (int mixedIndex : tiers) {
            ...
            for (int i = 0; i < parameters.getInt(Params.SAMPLE_SIZE); i++) {
                ...
                if (!intercept.containsKey(key)) {
                    ...
                    interceptCoefficients[0] = this.randSign() * RandomUtil.getInstance().nextUniform(interceptLow, interceptHigh);
                ...
                if (!linear.containsKey(key) && !continuousParents.isEmpty()) {
                    ...
                    for (int j = 0; j < parents.length; j++)
                        linearCoefficients[j] = this.randSign() * RandomUtil.getInstance().nextUniform(linearLow, linearHigh);
                ...
                if (!beta.containsKey(key) && !continuousParents.isEmpty()) {
                    ...
                    for (int j = 0; j < parents.length; j++)
                        betaCoefficients[j] = this.randSign() * RandomUtil.getInstance().nextUniform(betaLow, betaHigh);
