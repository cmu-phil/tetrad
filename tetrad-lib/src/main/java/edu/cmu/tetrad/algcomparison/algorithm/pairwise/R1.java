...
public class R1 implements Algorithm, TakesExternalGraph {
    ...
    public R1(final Algorithm algorithm) {
    ...
    public Graph search(final DataModel dataSet, final Parameters parameters) {
        if (parameters.getInt(Params.NUMBER_RESAMPLING) < 1) {
            ...
            final R1 r1 = new R1(this.algorithm);
            if (this.externalGraph != null) {
                r1.setExternalGraph(this.externalGraph);
            }

            final DataSet data = (DataSet) dataSet;
            final GeneralResamplingTest search = new GeneralResamplingTest(data, r1, parameters.getInt(Params.NUMBER_RESAMPLING));
