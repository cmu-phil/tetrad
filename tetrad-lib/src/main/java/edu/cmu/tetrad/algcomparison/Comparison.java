...
//            }
            ...
            statTables = this.calcStatTables(allStats, Mode.MedianCase, numTables, algorithmSimulationWrappers,
                    numStats, statistics);
            ...
            this.printStats(statTables, statistics, Mode.MedianCase, newOrder, algorithmSimulationWrappers, algorithmWrappers,
                    simulationWrappers, utilities, parameters);
        ...
        for (int i = 0; i < simulations.getSimulations().size(); i++) {
            this.saveToFiles(resultsPath + "/simulation" + (i + 1), simulations.getSimulations().get(i), parameters);
        }
...
//    }
    ...
    private class AlgorithmTask extends RecursiveTask<Boolean> {
        ...
        @Override
        protected Boolean compute() {
            Comparison.this.doRun(algorithmSimulationWrappers, algorithmWrappers,
    ...
    private void doRun(List<AlgorithmSimulationWrapper> algorithmSimulationWrappers,
        ...
        this.saveGraph(resultsPath, graphOut, run.getRunIndex(), simIndex, algIndex, algorithmWrapper, elapsed, stdout);
