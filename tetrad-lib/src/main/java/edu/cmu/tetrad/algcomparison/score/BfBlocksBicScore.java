...
public class BfBlocksBicScore implements ScoreWrapper {
    ...
    public Score getScore(DataModel model, Parameters parameters) {
        ...
        int basisType = parameters.getInt(Params.BASIS_TYPE);
        ...
        if (embeddedData.embedding().size() != dataSet.getNumColumns()) {
            throw new IllegalStateException("Embedding size does not match # original columns.");
        }
        for (int i = 0; i < dataSet.getNumColumns(); i++) {
            List<Integer> b = embeddedData.embedding().get(i);
            if (b == null || b.isEmpty()) {
                throw new IllegalStateException("Empty block for original var index " + i);
            }
        }

        DataSet embeddedDataSetFull = embeddedData.embeddedData();
        List<Node> variables = new ArrayList<>(dataSet.getVariables());

        // blocks: one per ORIGINAL variable, in the same order
        List<List<Integer>> blocks = new ArrayList<>(dataSet.getNumColumns());
        for (int i = 0; i < dataSet.getNumColumns(); i++) {
            blocks.add(embeddedData.embedding().get(i));
        }
...
//    }
    @Override
    public Node getVariable(String name) {
        if (this.blockSpec == null) return null;
        return this.blockSpec.dataSet().getVariable(name);
    }
}
