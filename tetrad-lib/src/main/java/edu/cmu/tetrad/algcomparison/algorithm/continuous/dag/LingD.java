...
/**
 * IcaLingD is an implementation of the Algorithm interface that performs the LiNG-D algorithm for discovering
 * causal models for the linear non-Gaussian case where the underlying model might be cyclic.
 *
 * @see edu.cmu.tetrad.search.LingD
 */
...
@Bootstrapping
public class LingD extends AbstractBootstrapAlgorithm implements Algorithm, ReturnsBootstrapGraphs {
    ...
    public Graph runSearch(DataModel dataSet, Parameters parameters) {
        ...
        Matrix W = edu.cmu.tetrad.search.LingD.estimateW(data, maxIter, tol, alpha, parameters.getBoolean(Params.VERBOSE));
        ...
        List<Matrix> bHats = lingD.getScaledBHats(W);
        ...
        for (Matrix bHat : bHats) {
            ...
            if (edu.cmu.tetrad.search.LingD.isStable(bHat)) {
