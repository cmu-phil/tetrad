...
@edu.cmu.tetrad.annotation.Algorithm(
        name = "FCIT-MAG-Markov",
        command = "FCIT-MAG-Markov",
        algoType = AlgType.allow_latent_common_causes
)
@Bootstrapping
@Experimental
public class FcitMagMarkov extends AbstractBootstrapAlgorithm implements Algorithm, TakesScoreWrapper, TakesIndependenceWrapper,
     ...
     * @see Algorithm
     */
    public FcitMagMarkov(IndependenceWrapper test, ScoreWrapper score) {
        this.test = test;
        this.score = score;
    }
    ...
    public Graph runSearch(DataModel dataModel, Parameters parameters) throws InterruptedException {
        ...
        edu.cmu.tetrad.search.FcitMagMarkov search = new edu.cmu.tetrad.search.FcitMagMarkov(test, score);
        ...
        if (parameters.getInt(Params.FCIT_STARTS_WITH) == 1) {
            search.setStartWith(edu.cmu.tetrad.search.FcitMagMarkov.START_WITH.BOSS);
        } else if (parameters.getInt(Params.FCIT_STARTS_WITH) == 2) {
            search.setStartWith(edu.cmu.tetrad.search.FcitMagMarkov.START_WITH.GRASP);
        } else if (parameters.getInt(Params.FCIT_STARTS_WITH) == 3) {
            search.setStartWith(edu.cmu.tetrad.search.FcitMagMarkov.START_WITH.SP);
        } else if (parameters.getInt(Params.FCIT_STARTS_WITH) == 4) {
            search.setStartWith(edu.cmu.tetrad.search.FcitMagMarkov.START_WITH.COMPLETE_GRAPH);
        } else {
    ...
    public String getDescription() {
        return "FCIT-MAG-Markov (FCI Targeted Testing with MAG Waypoints, Markov) using "+ this.test.getDescription() + " and " + this.score.getDescription();
    }
