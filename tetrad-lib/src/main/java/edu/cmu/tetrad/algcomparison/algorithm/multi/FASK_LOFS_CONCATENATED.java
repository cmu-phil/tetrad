 ...
 */
@Bootstrapping
public class FASK_LOFS_CONCATENATED implements MultiDataSetAlgorithm, HasKnowledge {
    static final long serialVersionUID = 23L;
    private final Lofs2.Rule rule;
    private Knowledge knowledge = new Knowledge();

    public FASK_LOFS_CONCATENATED(Lofs2.Rule rule) {
        this.rule = rule;
    }
    ...
    public Graph search(List<DataModel> dataModels, Parameters parameters) {
        if (parameters.getInt(Params.NUMBER_RESAMPLING) < 1) {
            ...
            FASK_LOFS_CONCATENATED algorithm = new FASK_LOFS_CONCATENATED(this.rule);
