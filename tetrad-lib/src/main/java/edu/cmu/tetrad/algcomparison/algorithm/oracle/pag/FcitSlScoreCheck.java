...
/**
 * FCIT-SL-Score-Check: the FCIT-SL algorithm with a MAG BIC SCORE CHECK applied to every candidate the search
 * adjudicates. A candidate that clears FCIT-SL's structural gates is committed only if the MAG it yields does not
 * decrease the RICF Gaussian BIC relative to the currently committed model.
 * <p>
 * NOTE that this wrapper runs {@link edu.cmu.tetrad.search.FcitSlScoreCheck}, which forks
 * {@link edu.cmu.tetrad.search.FcitSl}, whereas the FCIT-SL wrapper runs FcitSlKeepKnowledgeOrientations. Those two
 * differ only in the knowledge-certification stage and are identical when the knowledge is empty, so a comparison
 * of the two wrappers isolates the score check as long as no background knowledge is set.
 *
 * @author josephramsey
...
public class FcitSlScoreCheck extends AbstractBootstrapAlgorithm implements Algorithm, TakesScoreWrapper, TakesIndependenceWrapper,
     ...
     * @see Algorithm
     */
    public FcitSlScoreCheck(IndependenceWrapper test, ScoreWrapper score) {
        this.test = test;
        this.score = score;
    }
    ...
    public Graph runSearch(DataModel dataModel, Parameters parameters) throws InterruptedException {
        ...
        edu.cmu.tetrad.search.FcitSlScoreCheck search = new edu.cmu.tetrad.search.FcitSlScoreCheck(test, score);
        ...
        search.setCommitGate(edu.cmu.tetrad.search.FcitSlScoreCheck.CommitGate.DELETED_PAIR_BATTERY);
        ...
        // MAG SCORE CHECK. The gate's penalty discount is its own parameter (default 1, the classical BIC
        // comparison), independent of the initializer score's penaltyDiscount.
        search.setUseScoreCheck(true);
        search.setScoreCheckPenaltyDiscount(parameters.getDouble(Params.SCORE_CHECK_PENALTY_DISCOUNT));


        if (parameters.getInt(Params.FCIT_STARTS_WITH) == 1) {
            search.setStartWith(edu.cmu.tetrad.search.FcitSlScoreCheck.START_WITH.BOSS);
        } else if (parameters.getInt(Params.FCIT_STARTS_WITH) == 2) {
            search.setStartWith(edu.cmu.tetrad.search.FcitSlScoreCheck.START_WITH.GRASP);
        } else if (parameters.getInt(Params.FCIT_STARTS_WITH) == 3) {
            search.setStartWith(edu.cmu.tetrad.search.FcitSlScoreCheck.START_WITH.SP);
        } else if (parameters.getInt(Params.FCIT_STARTS_WITH) == 4) {
            search.setStartWith(edu.cmu.tetrad.search.FcitSlScoreCheck.START_WITH.COMPLETE_GRAPH);
        } else {
    ...
    public String getDescription() {
        return "FCIT-SL-Score-Check (FCIT-SL with a MAG BIC score check on every candidate) using "+ this.test.getDescription() + " and " + this.score.getDescription();
    }
