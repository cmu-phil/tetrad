    ...
    public class TestFisherZShrinkageCyclicDemo {
        ...
        private static void printCIList(List<?> itemsRaw, int maxShow) {
            ...
            int shown = 0;
            for (Object o : items) {
                if (shown++ >= maxShow) { System.out.println("  …"); break; }
                var ci = (CI) o;
...
// This is a pragmatic filter for accepted-independence lists.
        private static <CI> List<CI> benjaminiHochbergOnLargeP(List<CI> items, double q) {
            ...
            class Pair { final int idx; final double pPrime; Pair(int i, double v){ idx=i; pPrime=v; } }
        ...
        private static Double tryGetLastR(IndTestFisherZ base) {
            try { return (Double) IndTestFisherZ.class.getMethod("getLastR").invoke(base); }
        ...
        private static class RhoPeek extends CachingIndependenceTest {
            public RhoPeek(IndTestFisherZ base) { super(base); }
            public IndTestFisherZ getBaseTest() { return (IndTestFisherZ) super.getBaseTest(); }
        }
    }