...
public final class TestGraph {
    ...
    public void testLegalCpdag() {
        ...
        assertTrue(g5.paths().isLegalCpdag());
    ...
    public void testAdjustmentSet1() {
        ...
        try {
            List<Set<Node>> adjustmentSets = graph.paths().adjustmentSets(x1, x3, "PDAG", 4, 2, 1, 6, false);
    ...
    public void testAdjustmentSet2() {
        ...
        for (int i = 0; i < 20; i++) {
            for (int j = 0; j < 20; j++) {
                ...
                try {
                    List<Set<Node>> adjustmentSetsNearSource = graph.paths().adjustmentSets(x, y, "PDAG", 8, 2, 1, 6, false);
                    List<Set<Node>> adjustmentSetsNearTarget = graph.paths().adjustmentSets(x, y, "PDAG", 8, 2, 2, 6, false);
        ...
        try (PrintWriter out = new PrintWriter(_file)) {
            ...
            for (int i = 0; i < graphNodes.size(); i++) {
                for (int j = 0; j < graphNodes.size(); j++) {
                    ...
                    try {
                        adjustmentSetsNearSource = graph.paths().adjustmentSets(x, y, "PDAG", 4, 4, 1, 8, false);
                    } catch (Exception e) {
                        System.out.println("No adjustment set new source: " + e.getMessage());
                    }
                    List<Set<Node>> adjustmentSetsNearTarget = new ArrayList<>();
                    try {
                        adjustmentSetsNearTarget = graph.paths().adjustmentSets(x, y, "PDAG", 4, 4, 2, 8, false);
                    } catch (Exception e) {
