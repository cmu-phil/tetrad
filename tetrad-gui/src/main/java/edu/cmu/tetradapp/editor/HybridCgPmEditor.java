 ...
 *
 * How to use:
 *   HybridCgPmEditor panel = HybridCgPmEditor.fromWrapper(mixedPmWrapper);
 *   HybridCgPmEditor.showDialog(panel, "Edit Hybrid CG PM"); // modal dialog helper
 *
 * This editor does not mutate HybridCgPm in place (it is effectively immutable).
 * On Apply we *rebuild* a new HybridCgPm with the chosen types/categories and
 * set it back into the wrapper. Existing cutpoints for discrete children with
...
//    }
    ...
    private static RebuildableAccess buildAccess(HybridCgPmWrapper wrapper) {
        ...
        return new RebuildableAccess() {
            ...
            @Override public void setType(Node v, VarType t) { typeMap.put(v, t == null ? VarType.CONTINUOUS : t); }
    ...
    public static final class MixedPmTableModel extends AbstractTableModel {
        ...
        MixedPmTableModel(List<Row> rows) { this.rows = rows; }
    ...
    /**
     * Build an editor panel from your HybridCgPmWrapper.
     * The adapter maintains an editable map of types/categories and rebuilds the PM on Apply.
     */
    public static HybridCgPmEditor fromWrapper(HybridCgPmWrapper wrapper) {
        Objects.requireNonNull(wrapper, "wrapper");
