    ...
    private static final class Model extends AbstractTableModel {
        ...
        Model(HybridCgIm im, HybridCgPm pm, int y) {
            ...
            for (int j = 0; j < contParents.length; j++) {
                ...
                dims[discParents.length + j] = TMath.max(1, bins);
            }
        ...
        void normalizeRow(int row) {
            ...
            double s = 0.0;
            for (int k = 0; k < K; k++) s += TMath.max(0.0, im.getProbability(y, row, k));
            if (s <= 0) { // make uniform
                for (int k = 0; k < K; k++) im.setProbability(y, row, k, 1.0 / K);
            } else {
                for (int k = 0; k < K; k++) im.setProbability(y, row, k, TMath.max(0.0, im.getProbability(y, row, k)) / s);
            }
        }
        void normalizeAll() {
        ...
        void randomizeRow(int row) {
            ...
            for (int k = 0; k < K; k++) { e[k] = -TMath.log(1.0 - TMath.random()); s += e[k]; }
