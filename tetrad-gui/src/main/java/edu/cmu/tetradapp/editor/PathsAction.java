...
//                update(graph, textArea, nodes1, nodes2, method);
        ...
        JComboBox methodBox = new JComboBox(new String[]{"Directed Paths", "Semidirected Paths", "Treks",
                "Adjacents"});
        method = Preferences.userRoot().get("pathMethod", "Directed Paths");
...
//                update(graph, textArea, nodes1, nodes2, method);
        ...
        maxField.setFilter(new Filter() {
            public int filter(int value, int oldValue) {
                try {
                    PathsAction.this.setMaxLength(value);
//                    update(graph, textArea, nodes1, nodes2, method);
                    return value;
                } catch (Exception e) {
                    return oldValue;
        ...
        updateButton.addActionListener(new ActionListener() {
            @Override
            public void actionPerformed(ActionEvent e) {
                PathsAction.this.update(graph, textArea, nodes1, nodes2, method);
            }
        });
        ...
        this.update(graph, textArea, nodes1, nodes2, method);
    }
        ...
        WatchedProcess process = new WatchedProcess(owner) {
            public void watch() {
                if ("Directed Paths".equals(method)) {
                    textArea.setText("");
                    PathsAction.this.allDirectedPaths(graph, textArea, nodes1, nodes2);
                } else if ("Semidirected Paths".equals(method)) {
                    textArea.setText("");
                    PathsAction.this.allSemidirectedPaths(graph, textArea, nodes1, nodes2);
                } else if ("Treks".equals(method)) {
                    textArea.setText("");
                    PathsAction.this.allTreks(graph, textArea, nodes1, nodes2);
                } else if ("Adjacents".equals(method)) {
                    textArea.setText("");
                    PathsAction.this.adjacentNodes(graph, textArea, nodes1, nodes2);
                }
            }
        };
    }
