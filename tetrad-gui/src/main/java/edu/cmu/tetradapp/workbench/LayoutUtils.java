...
public class LayoutUtils {
     ...
     */
    /**
     * Respaces a row-based layout -- knowledge tiers or lag indices -- for the display nodes' label sizes, keeping
     * every node in the row it was put in. The rows are read off the layout itself: nodes the lib placed at the same
     * y are one row. Within a row, nodes keep their left-to-right order and are moved apart until their boxes clear
     * by the workbench's overlap margin; rows are moved apart by their tallest node plus {@link LayoutUtil#ROW_GAP},
     * the wider gap that leaves room for the edges drawn between one row and the next. The arrangement is placed with
     * its top-left at the overlap margin.
     *
     * <p>This preserves the tier assignment, which is the point of laying a graph out by knowledge in the first
     * place -- redoing such a graph as a circle would discard exactly the structure the user asked to see.</p>
     *
     * @param graph          the graph, already laid out in rows by the lib
     * @param layoutEditable the editable, for the display node sizes
     */
    static void respaceRowsForLabels(Graph graph, LayoutEditable layoutEditable) {
        if (graph == null) return;

        LayoutUtil.NodeSize sizes = displayNodeSizes(layoutEditable);
        final int margin = AbstractWorkbench.NODE_OVERLAP_MARGIN;

        // Group by the y the lib assigned; within a row, keep the left-to-right order.
        java.util.TreeMap<Integer, List<Node>> rows = new java.util.TreeMap<>();
        for (Node node : graph.getNodes()) {
            rows.computeIfAbsent(node.getCenterY(), k -> new ArrayList<>()).add(node);
        }
        if (rows.isEmpty()) return;

        for (List<Node> row : rows.values()) {
            row.sort(Comparator.comparingInt(Node::getCenterX));
        }
        ...
        for (List<Node> row : rows.values()) {
            ...
            for (Node node : row) {
                double h = sizes.height(node);
                if (Double.isFinite(h) && h > 0) maxHeight = TMath.max(maxHeight, h);
            }

            if (firstRow) {
                y = margin + maxHeight / 2.0;
                firstRow = false;
            } else {
                y += prevHalfHeight + LayoutUtil.ROW_GAP + maxHeight / 2.0;
            }
            ...
            for (int i = 0; i < row.size(); i++) {
                ...
                if (i == 0) {
                    ...
                    x += prevHalfWidth + margin + halfWidth;
                }
                prevHalfWidth = halfWidth;

                node.setCenterX((int) TMath.round(x));
                node.setCenterY((int) TMath.round(y));
            }
        }
    }
