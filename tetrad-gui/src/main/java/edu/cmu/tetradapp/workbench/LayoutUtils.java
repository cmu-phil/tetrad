...
public class LayoutUtils {
    ...
    public static void copyLag0LayoutLeftToRight(final LayoutEditable layoutEditable) {
        ...
        final int lag0XDiff = maxLag0X - minLag0X;
        final int xSpace = lag0XDiff + 25 < 90 ? 90 : lag0XDiff + 25;

        int minX = Integer.MAX_VALUE;

        for (final Node node : lag0Nodes) {
            int x = node.getCenterX();
            final int y = node.getCenterY();
            final TimeLagGraph.NodeId id = graph.getNodeId(node);
