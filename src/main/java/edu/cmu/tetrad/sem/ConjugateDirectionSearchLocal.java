 ...
 */
public class ConjugateDirectionSearchLocal extends MultivariateMinimum {
    //
    // Public stuff
    //
     ...
     * constructor
     */
    public ConjugateDirectionSearchLocal() {
     ...
     * freeParameters are very different, scbd should be set to
    ...
    public void optimize(MultivariateFunction f, double[] xvector, double tolfx, double tolx, MinimiserMonitor monitor) {
        ...
        if (prin > 1) {
            ...
            System.out.println("... getModel parameter settings ...");
        ...
        while (true) {
            ...
            for (k = 0; k < dim; k++) {
                ...
                do {
                    kl = k;
                    df = 0.0;
                    if (illc) { // illc) {        /* random step to get off resolution valley */
                        for (i = 0; i < dim; i++) {
//                            z[i] = (0.1 * ldt + t2 * Math.pow(10.0, (double) kt)) * (RandomUtil.getInstance().nextDouble() - 0.5);

                            double range = 1;
                            z[i] = (0.1 * ldt + t2 * Math.pow(10.0, (double) kt)) * (range * RandomUtil.getInstance().nextDouble() - range / 2.0);
                            s = z[i];
                            for (j = 0; j < dim; j++) {
                                x[j] += s * v[j][i];
                            }
...
//                    break;
    ...
    // these will be set by praxis to the global control freeParameters
    private double h, t;
    ...
    private void print() /* print a line of traces */ {
        ...
        vecprint("... getModel values of x ...", x);
    }
...
//        if (Double.isNaN(l)) {    // Took that out jdramsey 2014/11/12
//            return f;
//        }
//        if (Double.isNaN(l)) {
