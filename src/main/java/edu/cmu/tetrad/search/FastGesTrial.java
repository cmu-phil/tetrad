 ...
 */
public final class FastGesTrial implements GraphSearch, GraphScorer {
     ...
     */
    public FastGesTrial(DataSet dataSet) {
     ...
     */
    public FastGesTrial(ICovarianceMatrix covMatrix) {
        out.println("GES constructor");
    ...
    public FastGesTrial(GesScore gesScore) {
...
//
    ...
    private void calculateArrowsForward(final Node a, final Node b, final Graph graph) {
        ...
        allSubsetsForward(a, b, graph, naYX, t);
//        greedySubsetsForward(a, b, graph, naYX, t);
    }

    private void allSubsetsForward(Node a, Node b, Graph graph, Set<Node> naYX, List<Node> t) {
        final int _depth = Math.min(t.size(), getDepth() == -1 ? 1000 : getDepth());
        ...
        while ((choice = gen.next()) != null) {
            Set<Node> _t = GraphUtils.asSet(choice, t);

            Set<Node> union = new HashSet<>(_t);
            union.addAll(naYX);
            ...
            if (existsKnowledge()) {
                if (!validSetByKnowledge(b, _t)) {
                    continue;
                }
            }

            double difference = insertEval(a, b, _t, naYX, graph, hashIndices);

            if (difference > 0.0) {
                Arrow arrow = new Arrow(difference, a, b, _t, naYX);
    ...
    // See Alonso-Barba et al. (2013), Scaling up the Greedy Equivalence Search algorithm
    // by constraining the search space of equivalence classes
    private void greedySubsetsForward(Node a, Node b, Graph graph, Set<Node> naYX, List<Node> t) {
        Set<Node> _t = new HashSet<Node>();
        double d = insertEval(a, b, _t, naYX, graph, hashIndices);
        ...
        while (true) {
            ...
            for (Node n : t) {
                if (_t.contains(n)) continue;
                Set<Node> __t = new HashSet<>(_t);

                __t.add(n);

                Set<Node> union = new HashSet<>(__t);
                union.addAll(naYX);
                ...
                if (existsKnowledge()) {
                    if (!validSetByKnowledge(b, __t)) {
                        continue;
                    }
                }

                double _d = insertEval(a, b, __t, naYX, graph, hashIndices);
        ...
        if (d > 0.0) {
            Arrow arrow = new Arrow(d, a, b, _t, naYX);
            sortedArrows.add(arrow);
            addLookupArrow(a, b, arrow);
        }
    }
    ...
    private void calculateArrowsBackward(Node a, Node b, Graph graph) {
        ...
        allSubsetsBackwards(a, b, graph, naYX);
//        sbesStepBackwards(a, b, graph, naYX);
    }

    private void allSubsetsBackwards(Node a, Node b, Graph graph, Set<Node> naYX) {
        List<Node> _naYX = new ArrayList<>(naYX);
        final int _depth = Math.min(naYX.size(), depth == -1 ? 1000 : depth);
        DepthChoiceGenerator gen = new DepthChoiceGenerator(_naYX.size(), _depth);
        int[] choice;
        ...
        while ((choice = gen.next()) != null) {
            ...
            double difference = deleteEval(a, b, h, naYX, graph, hashIndices);

            if (difference > 0.0) {
                Arrow arrow = new Arrow(difference, a, b, h, naYX);
                sortedArrows.add(arrow);
                addLookupArrow(a, b, arrow);
            }
        }
    ...
    private void sbesStepBackwards(Node a, Node b, Graph graph, Set<Node> naYX) {
        Set<Set<Node>> maxCliques = maximalCliques(graph, naYX);

        for (Set<Node> clique : maxCliques) {
            List<Node> Ci = new ArrayList<>(clique);

            Set<Node> H0 = new HashSet<>(naYX);
            H0.removeAll(Ci);

            final int _depth = Math.min(clique.size(), getDepth() == -1 ? 1000 : getDepth());
            DepthChoiceGenerator gen0 = new DepthChoiceGenerator(clique.size(), _depth);
            int[] choice0;

            while ((choice0 = gen0.next()) != null) {
                Set<Node> C = GraphUtils.asSet(choice0, Ci);
                Set<Node> H = new HashSet<>(H0);
                H.addAll(C);
                ...
                if (existsKnowledge()) {
                    if (!validSetByKnowledge(b, C)) {
                        continue;
                    }
                }

                double difference = deleteEval(a, b, C, naYX, graph, hashIndices);

                if (difference > 0.0) {
                    Arrow arrow = new Arrow(difference, a, b, C, naYX);
                    sortedArrows.add(arrow);
                    addLookupArrow(a, b, arrow);
                }
            }
    ...
    public Set<Set<Node>> maximalCliques(Graph graph, Set<Node> nodes) {
        Set<Set<Node>> report = new HashSet<>();
        BronKerbosh1(new HashSet<Node>(), new HashSet<>(nodes), new HashSet<Node>(), report, graph);
        return report;
    }
    ...
    //                   P := P \ {v}
    //                   X := X â {v}
    public void BronKerbosh1(Set<Node> R, Set<Node> P, Set<Node> X, Set<Set<Node>> report, Graph graph) {
        ...
        for (Node v : new HashSet<>(P)) {
            ...
            BronKerbosh1(_R, _P, _X, report, graph);
            P.remove(v);
            X.add(v);
        }
    }
...
//        // Try parallelizing this.
//        while ((choice = gen.next()) != null) {
//            Set<Node> s = DataGraphUtils.asSet(choice, t);
//
...
//
//        while ((choice = gen.next()) != null) {
//            Set<Node> h = DataGraphUtils.asSet(choice, _naYX);
//
//            Set<Node> diff = new HashSet<>(naYX);
//            diff.removeAll(h);
//
