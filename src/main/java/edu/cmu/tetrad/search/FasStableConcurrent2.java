 ...
 */
public class FasStableConcurrent2 implements IFas {
     ...
     */
    public FasStableConcurrent2(Graph graph, IndependenceTest test) {
        this.graph = graph;
        this.test = test;
    }

    public FasStableConcurrent2(IndependenceTest test) {
...
//                                //need this for empty has different meaning from not existing --AJ
...
//        getIndependenceTest().setAlpha(alpha);
    ...
    private boolean searchAtDepth(final List<Node> nodes, final IndependenceTest test, final Map<Node,
            Set<Node>> adjacencies, final int depth) {
        if (verbose) {
