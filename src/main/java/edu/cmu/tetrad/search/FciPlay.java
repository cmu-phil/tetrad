 ...
 */
public final class FciPlay implements GraphSearch {
     ...
     */
    public FciPlay(IndependenceTest independenceTest) {
    ...
    public FciPlay(Graph dag) {
        this.dag = new EdgeListGraphSingleConnections(dag);
        this.independenceTest = new IndTestDSep(dag);
     ...
     * search over.
     */
    public FciPlay(IndependenceTest independenceTest, List<Node> searchVars) {
        if (independenceTest == null || knowledge == null) {
            throw new NullPointerException();
        }
...
//        // Step FCI B.  (Zhang's step F2.)
//        IFas fas = new FasICov(graph, independenceTest);
//        Fas fas = new Fas(graph, independenceTest);
//        FasStableConcurrent fas = new FasStableConcurrent(graph, independenceTest);
...
//        fas.setFci(true);
            ...
            if (!(rfciUsed)) {
                ...
                if (isPossibleDsepSearchDone()) {
                    ...
                    System.out.println("Starting possible dsep seach");
...
//        logger.log("info", "Elapsed time orientation search = " + (stop2 - start2) / 1000L + "s");
    ...
    private void calcAdjacencyGraph() {
        ...
        for (int i = 0; i < measured.size(); i++) {
            for (int j = i + 1; j < measured.size(); j++) {
                ...
                if (sepset != null && dag.isAdjacentTo(n1, n2)) {
                    System.out.println("disrepency 4");
                }
            }
        }
    }
...
//    }
    ...
    public void setRfciUsed(boolean used) {
        this.rfciUsed = used;
    }
    ...
    private void ruleR0() {
        ...
        for (Node b : nodes) {
            ...
            while ((combination = cg.next()) != null) {
                ...
                List<Node> sepset2 = getSepset(a, c);
                ...
                if (!sepset2.contains(b)) {
                    ...
                    printWrongColliderMessage(a, b, c, location);
                }
            }
        }
    }
...
//        }
     ...
     * a DDP consists of colliders that are parents of c.
     */
    public void ddpOrient(Node a, Node b, Node c) {
        ...
        while (!Q.isEmpty()) {
            ...
            if (e == null || e == t) {
                e = t;
                distance++;
                if (distance > 0 && distance > maxPathLength) return;
            }
            ...
            for (Node d : nodesInTo) {
                ...
                if (p != null && !graph.isDefCollider(d, t, p)) {
                    continue;
                }
                ...
                if (!graph.isAdjacentTo(d, c)) {
                    if (doDdpOrientation(d, a, b, c, previous)) {
                        return;
                    }
                }
     ...
     * arguments.
     */
    private boolean doDdpOrientation(Node d, Node a, Node b, Node c, Map<Node, Node> previous) {
        if (graph.isAdjacentTo(d, c)) {
            throw new IllegalArgumentException();
        }

        List<Node> path = getPath(d, previous);
        boolean ind1 = getSepset(c, d) != null && !getSepset(c, d).contains(b); //getIndependenceTest().isIndependent(c, d, path);

        List<Node> pathPlusB = new ArrayList<Node>(path);
        pathPlusB.add(b);
        boolean ind2 = getSepset(c, d) != null && getSepset(c, d).contains(b);// / getIndependenceTest().isIndependent(c, d, pathPlusB);

        if (ind2) {
            if (rfciUsed && path.size() > 0 && removeLastEdge(d, path.get(0), pathPlusB)) {
                return false;
            }
            ...
            if (rfciUsed && path.size() > 0 && removeLastEdge(d, path.get(0), path)) {
                return false;
            }
            ...
            graph.setEndpoint(c, b, Endpoint.ARROW);
            logger.log("colliderOrientations", SearchLogUtils.colliderOrientedMsg("Definite discriminating path.. d = " + d, a, b, c));
            printWrongColliderMessage(a, b, c, "DDP");
            changeFlag = true;
            return true;
        }
...
//        changeFlag = true;
    ...
    private boolean removeLastEdge(Node r, Node q, List<Node> sepset) {
        List<Node> l = new ArrayList<Node>(sepset);

        DepthChoiceGenerator gen = new DepthChoiceGenerator(l.size(), l.size());
        int[] choice;

        while ((choice = gen.next()) != null) {
            List<Node> s = GraphUtils.asList(choice, l);

            if (getIndependenceTest().isIndependent(r, q, s)) {
                graph.removeEdge(r, q);
                ...
                System.out.println("Removing edge " + r + " --- " + q + " location 4");
                graph.removeEdge(r, q);
                changeFlag = true;
